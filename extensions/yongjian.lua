...
function CardIsPresent(id)
	...
	return c:getSkillName()=="" and table.contains(sgs.Sanguosha:getEngineCard(id):property("CharTag"):toStringList(),"present_card")
end
                ...
                for _,skill in sgs.list(effect.to:getSkillList(true,false))do
		        	...
		        	then
		            	...
		            	if skill:isEnabledAtResponse(effect.to,"zl_caochuanjiejian")
	                 	then table.insert(can,"zl_caochuanjiejian") end
		        	end
	        	end
       	        if #can>0
		        then
...
--	damage_card = true,
	...
	filter = function(self,targets,to_select,source)
		...
		else return targets[#targets]:objectName()~=to_select:objectName() end
	end,
	...
	on_effect = function(self,effect)
		...
		if target:isAlive()
		then
			local dc = room:askForExchange(effect.to,"zd_shengdongjixi",2,2,true,"zd_shengdongjixi1:"..target:objectName())
...
--	damage_card = true,
	...
	on_effect = function(self,effect)
		...
		then else room:askForDiscard(effect.to,"zd_zengbingjianzao",2,2,false,true) end
		return false
	end,
}
...
--	damage_card = true,
	...
	on_effect = function(self,effect)
		...
		then table.insert(choices,"ZdQijia1") end
		if hasCard(effect.to,"Armor,DefensiveHorse")
		then table.insert(choices,"ZdQijia2") end
...
zd_fulei = sgs.CreateTrickCard{--锦囊牌
    ...
    available = function(self,player)
		...
		else return self:cardIsAvailable(player) end
    end,
	about_to_use = function(self,room,use)
       	if use.to:isEmpty() then use.to:append(use.from) end
	...
	on_effect = function(self,effect)
		...
		else room:removeTag("ZdFulei"..self:toString()) end
		return false
	end,
}
...
ZhongdanOnTrigger = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data,room)
		...
		then
			...
			or use.from==player then return end
...
WenheOnTrigger = sgs.CreateTriggerSkill{
	...
	can_trigger = function(self,target)
		...
		BanWenhe = sgs.Sanguosha:currentRoom():getMode()~="08p" or table.contains(sgs.Sanguosha:getBanPackages(),"wenheluanwu")
		return not BanWenhe
	end,
	on_trigger = function(self,event,player,data,room)
		local log = sgs.LogMessage()
