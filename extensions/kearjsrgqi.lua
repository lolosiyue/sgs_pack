					...
					if room:askForSkillInvoke(cc, self:objectName(), data) then
						...
						if (player:getMark("banzhenglue-Clear") == 0) then
							if (player:getState() ~= "online") then
								local getlies = room:askForPlayersChosen(cc, players, self:objectName(), math.min(2,players:length()), 2, "keqizhenglue-ask", true, true)
								...
								local getlies = room:askForPlayersChosen(cc, players, self:objectName(), 0, 2, "keqizhenglue-ask", true, true)
								for _,p in sgs.qlist(getlies) do
									if (p:getMark("&keqilue") == 0) then
									    p:gainMark("&keqilue")
									end
							...
							local one = room:askForPlayerChosen(cc, players, self:objectName(), "keqizhenglue-ask", true, true)
							if one then
...
keqipingtaoCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room, player, targets)
		...
		and (not target:isNude()) then
			...
			local card = room:askForExchange(target, self:objectName(), 1,1, true, "#keqipingtao:".. player:getGeneralName(),true)
			if card then
				room:broadcastSkillInvoke("keqipingtao")
			...
			if (use.to:length() == 1) and (not use.to:contains(use.from)) and (not use.card:isKindOf("SkillCard")) then
				...
				if (player:getChangeSkillState("keqishichong") == 1) then
					...
					if (use == 0) and target:isYourFriend(player) and (target:getState() ~= "online") and (not target:isKongcheng()) then
						local card = room:askForExchange(target, self:objectName(), 1,1, false, "#keqishichongg:".. player:getGeneralName(),false)
						if card then
							room:broadcastSkillInvoke(self:objectName())
							local log = sgs.LogMessage()
							log.type = "$keqishichonguse"
			...
			if (player:getPhase() == sgs.Player_Finish) and (not player:isKongcheng()) then
				if room:askForSkillInvoke(player, self:objectName(), data) then
					...
					if (player:getState() ~= "online") then
						...
						targets = room:askForPlayersChosen(player, room:getOtherPlayers(player), self:objectName(), math.min(num,aienys:length()), num, "keqizhaobing-ask", false, true)
					else
						targets = room:askForPlayersChosen(player, room:getOtherPlayers(player), self:objectName(), 0, num, "keqizhaobing-ask", false, true)
					end
					for _,p in sgs.qlist(targets) do
						if (p:getState() ~= "online") then
							...
							local card = room:askForExchange(p, self:objectName(), 1,1, true, "#keqizhaobing:".. player:getGeneralName(),true,table.concat(pattern, ","))
							if card then
	...
	on_trigger = function(self, event, player, data)
		...
		if (event == sgs.Damage) then
			...
			if damage.from:getState() ~= "online" then
				...
				room:setPlayerMark(damage.from,"keqiguanhuospade",0)  room:setPlayerMark(damage.from,"keqiguanhuoclub",0)
				room:setPlayerMark(damage.from,"keqiguanhuoheart",0)  room:setPlayerMark(damage.from,"keqiguanhuodiamond",0)
			end
			--end
			end
		end
		if (event == sgs.CardFinished) then
			...
			if use.card:hasFlag("keqiguanhuocard") then
				...
				if use.from:getState() ~= "online" then
					...
					room:setPlayerMark(use.from,"keqiguanhuospade",0)  room:setPlayerMark(use.from,"keqiguanhuoclub",0)
					room:setPlayerMark(use.from,"keqiguanhuoheart",0)  room:setPlayerMark(use.from,"keqiguanhuodiamond",0)
				end
				--end
				...
				for _,kr in sgs.qlist(krs) do
					...
					and (kr:getCardCount() >= 2) then
						if kr:getState() == "online" then
							local card = room:askForExchange(kr,self:objectName(), 2,2, true, "#keqilirang:".. player:getGeneralName(),true)
							if card then
								room:broadcastSkillInvoke(self:objectName())
								local log = sgs.LogMessage()
		...
		if (event == sgs.EventPhaseStart) then
			if (player:getPhase() == sgs.Player_Start) then
				if room:askForSkillInvoke(player, self:objectName(), data) then
					...
					for _,p in sgs.qlist(room:getOtherPlayers(player)) do
						--统计一下是否有队友需要回血
						local eny = 0 local fri = 0
						for _,oth in sgs.qlist(room:getOtherPlayers(p)) do
							if oth:isYourFriend(p) and ((oth:getHp() + oth:getHp() + oth:getCardCount()) <= 8)  then fri = fri + 1 else eny = eny + 1 end 
						end
...
keqitongjueCard = sgs.CreateSkillCard{
	...
	filter = function(self, selected, to_select)
		return (#selected == 0) and (to_select:objectName() ~= sgs.Self:objectName()) and (to_select:getKingdom() == "qun")
	end ,
	on_use = function(self, room, source, targets)
...
keqitongjueex = sgs.CreateProhibitSkill{
	name = "keqitongjueex",
	is_prohibited = function(self, from, to, card)
		return ((from:getMark("usekeqitongjue-Clear")>0) and (to:getMark("beusekeqitongjue-Clear")>0)) and (not card:isKindOf("SkillCard"))
	end
}
...
keqitaipingyaoshupro = sgs.CreateProhibitSkill{
	...
	is_prohibited = function(self, from, to, card)
		return (from:getState() ~= "online") and (card:isKindOf("ThunderSlash") or card:isKindOf("FireSlash")) and (to:getArmor() ~= nil) and (to:getArmor():objectName() == "_keqi_taipingyaoshu")
	end
}
...
keqiwendaotwo = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if (judge.who:objectName() == player:objectName()) then
			...
			if player:getState() == "online" then
				local daomeidans = room:askForPlayersChosen(player, players, self:objectName(), 0, 2, "keqiwendao-ask", false, true)
		...
		if (event == sgs.EventPhaseStart) then
			if (player:getPhase() == sgs.Player_Start) then
				if room:askForSkillInvoke(player, "keqixuanhuatwofirst", data) then 
					...
					if (player:getMark("keqixuanhuahit") == 0) then
						local target = room:askForPlayerChosen(player, room:getAllPlayers(), "keqixuanhuaco_ask", "keqixuanhuaco-ask", true, true)
						if target then
			...
			if (player:getPhase() == sgs.Player_Finish) then
				if room:askForSkillInvoke(player, self:objectName(), data) then 
					...
					if (player:getMark("keqixuanhuahit") == 0) then
						local target = room:askForPlayerChosen(player, room:getAllPlayers(), "keqixuanhuada_ask", "keqixuanhuada-ask", true, true)
						if target then
