...
f_huoxinGPCCard = sgs.CreateSkillCard { --给出拼点牌
	...
	will_throw = false,
	filter = function(self, targets, to_select)
		return #targets == 0 and to_select:hasFlag("f_huoxin_pindiantargets") and
...
f_hanzhongwangCard = sgs.CreateSkillCard {
	...
	target_fixed = false,
	filter = function(self, targets, to_select)
		return to_select:getKingdom() == "shu" and to_select:objectName() ~= sgs.Self:objectName() and
...
f_shengong = sgs.CreateTriggerSkill {
	...
	can_trigger = function(self, player)
		return player and player:hasSkill(self:objectName()) and player:getMark("f_shengong_triggered") == 0 and
...
getFShenJianSkillVS = sgs.CreateViewAsSkill {
	...
	enabled_at_play = function(self, player)
		return player:hasSkill("f_dingjun") and not player:hasUsed("#getFShenJianSkillCard") and
			player:getPile("ShenJian"):length() >= 4
	end,
...
getOTFShenJianSkillVS = sgs.CreateViewAsSkill {
	...
	enabled_at_play = function(self, player)
		return player:hasSkill("f_newdingjun") and not player:hasUsed("#getOTFShenJianSkillCard") and
			player:getPile("ShenJian"):length() >= 1
	end,
...
f_luansheX = sgs.CreateTriggerSkill {
		...
		elseif event == sgs.Damage then
			...
			if card:isKindOf("ArcheryAttack") and card:getSkillName() == "f_luanshe" then
				...
				if not player:isKongcheng() then
					...
					if card_id then
						card_id:deleteLater()
						card_id = room:askForExchange(player, "f_luanshe", damage.damage, damage.damage, false,
							"f_luansheXPush")
					end
					player:addToPile("ShenJian", card_id)
				end
	...
	can_trigger = function(self, player)
		return player and player:hasSkill("f_luanshe")
	end,
}
...
f_jiutianContinue = sgs.CreateTriggerSkill {
	...
	can_trigger = function(self, player)
		return player and player:hasSkill("f_jiutian")
	end,
}
		...
		if not effect.to:isKongcheng() then
			...
			if result == "discard" then
				if sgs.Sanguosha:getCard(card_id):getSuit() == sgs.Card_Heart then
					...
					local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_DISMANTLE, effect.from:objectName(),
						nil, "fcgongxin", nil)
...
fclongming = sgs.CreateViewAsSkill {
	...
	view_filter = function(self, selected, to_select)
		...
		elseif sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_RESPONSE or sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_RESPONSE_USE then
			...
			if string.find(pattern, "peach") then
				return to_select:getSuit() == sgs.Card_Heart or
					(to_select:getSuit() == sgs.Card_Spade and sgs.Self:hasFlag("Global_Dying"))
	...
	enabled_at_play = function(self, player)
		...
		return player:isWounded() or sgs.Slash_IsAvailable(player)
			or player:usedTimes("Analeptic") <=
			sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_Residue, player, newana)
	end,
	enabled_at_response = function(self, player, pattern)
		return pattern == "slash" or pattern == "jink"
			or (string.find(pattern, "peach") and not player:hasFlag("Global_PreventPeach")) or
			string.find(pattern, "analeptic")
	end,
}
...
fclongming_Audio = sgs.CreateTriggerSkill {
	...
	can_trigger = function(self, player)
		return player and player:hasSkill("fclongming")
	end,
}
----
...
fcjieying_GMS = sgs.CreateTriggerSkill {
	...
	can_trigger = function(self, player)
		return player and player:hasSkill("fcjieying")
	end,
}
	...
	on_trigger = function(self, event, player, data, room)
		if player:getPhase() == sgs.Player_Finish and room:askForSkillInvoke(player, self:objectName(), data) then
			...
			if not targets:isEmpty() then
				local target = room:askForPlayerChosen(player, targets, self:objectName(), "fcjieying-invoke", false,
					true)
				if target then
					room:broadcastSkillInvoke(self:objectName())
...
fczhitiFlag = sgs.CreateTriggerSkill {
	...
	can_trigger = function(self, player)
		return player and player:hasSkill("fczhiti")
	end,
}
...
fczhiti_obtainEquipArea = sgs.CreateTriggerSkill { --恢复装备区
	...
	can_trigger = function(self, targets)
		return targets and targets:getMark("EquipArea_lose") > 0
	end,
}
...
f_gengzhongCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		---
		local basic = { "slash", "normal_slash", "fire_slash", "thunder_slash", "ice_slash", "peach", "analeptic",
			"cancel" }
...
sp_weizhen = sgs.CreateViewAsSkill {
	...
	n = 1,
	view_filter = function(self, selected, to_select)
		return to_select:isBlack() and
			(to_select:getTypeId() == sgs.Card_TypeBasic or to_select:getTypeId() == sgs.Card_TypeTrick)
	end,
	view_as = function(self, cards)
		if #cards == 1 then
		...
		if n >= 1 then
			...
			elseif choice3 == "sp_guamuONEput" then
				...
				if cardd then
					...
					local reasonx = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_PUT, source:objectName(), nil,
						"sp_guamu", nil)
					...
					if tszg:getMark(self:objectName() .. "engine") > 0 then
						...
						elseif choice == "remove2szmarks" then
							...
							elseif suit == sgs.Card_Spade then --对目标造成1点雷电伤害，然后弃置其1张牌，再自弃2张牌
								...
								room:damage(sgs.DamageStruct(self:objectName(), tszg, victim2s, 1,
						...
						elseif choice == "remove3szmarks" then
							...
							elseif suit == sgs.Card_Spade then --对目标造成2点雷电伤害，然后弃置其1张牌，再自弃3张牌
								...
								room:broadcastSkillInvoke("sp_qitian", 4)
								room:damage(sgs.DamageStruct(self:objectName(), tszg, victim3s, 2,
									sgs.DamageStruct_Thunder))
						...
						elseif choice == "remove4szmarks" then
							...
							elseif suit == sgs.Card_Spade then --对目标造成3点雷电伤害，然后弃置其2张牌，再自弃4张牌
								...
								room:broadcastSkillInvoke("sp_qitian", 4)
								room:damage(sgs.DamageStruct(self:objectName(), tszg, victim4s, 3,
									sgs.DamageStruct_Thunder))
...
sp_zhujiuCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select)
		return #targets == 0 and to_select:objectName() ~= sgs.Self:objectName() and
			to_select:getMark("&qingmei_zhujiu") > 0 and not to_select:isKongcheng()
	end,
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
...
sp_zhujiu = sgs.CreateZeroCardViewAsSkill {
	...
	enabled_at_play = function(self, player)
		return player:getMark("&qingmei_zhujiu") > 0 and not player:isKongcheng() and
			not player:hasFlag("sp_zhujiulimited")
	end,
}
			...
			if pindian.reason == "sp_zhujiu" and pindian.from:objectName() == player:objectName() then
				...
				if fromNumber > toNumber then
					...
					if (player:getMark("&sp_zhujiuFQC") <= 10 and player:getMark("@duangexing") > 0) then
						room:addPlayerMark(player, "&sp_zhujiuFQC")
					end
					room:broadcastSkillInvoke("sp_zhujiu")
				else
					...
					elseif player:hasFlag("sp_zhujiuPDlose") and player:faceUp() then
						if not player:isNude() then
							local id = room:askForCardChosen(pindian.to, player, "he", "sp_zhujiu", false,
...
sp_gexingVS = sgs.CreateZeroCardViewAsSkill {
	...
	enabled_at_play = function(self, player)
		return player:getMark("@duangexing") > 0 and player:getMark("&sp_zhujiuFQC") >= 10 and
			not player:hasFlag("sp_gexing_limited") and not player:getPhase() == sgs.Player_Play
	end,
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.PreCardUsed then
			...
			if choice == "1" then
				...
				room:askForUseCard(player, "@@sp_hengsaoqianjunBUFF",
					"@sp_hengsaoqianjunBUFF:" .. use.card:objectName() .. "::" .. 2, -1, sgs.Card_MethodNone)
...
sp_danqi = sgs.CreateViewAsSkill {
	...
	enabled_at_play = function(self, target)
		local newana = sgs.Sanguosha:cloneCard("analeptic", sgs.Card_SuitToBeDecided, 0)
		return not target:hasFlag("sp_danqiCardUsedTwice") and
			not (target:getMark("&canuse_danqi") == 0 and not target:hasSkill("sp_lingyun")) and
			not (target:getMark("&canuse_danqi") == 0 and target:getMark("@sp_lingyunAI") > 0 and target:getMaxHp() <= 1)
			and
			(target:isWounded() or sgs.Slash_IsAvailable(target) or target:usedTimes("Analeptic") <= sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_Residue, target, newana))
	end,
	enabled_at_response = function(self, target, pattern)
		return pattern == "slash" or pattern == "jink" or
			(string.find(pattern, "peach") and not target:hasFlag("Global_PreventPeach")) or
			string.find(pattern, "analeptic")
	end,
}
			...
			if card then
				...
				if card:isKindOf("Jink") and (card:getSkillName() == "sp_danqi" or card:getSkillName() == "sp_danqi_buffs") and data:toCardResponse().m_who then
					...
					elseif choiceJ == "jink2" then
						...
						if data:toCardResponse().m_who:isAlive() and not data:toCardResponse().m_who:isNude() then
							local id1 = room:askForCardChosen(player, data:toCardResponse().m_who, "he",
								self:objectName())
							room:throwCard(id1, data:toCardResponse().m_who, player)
						end
						if data:toCardResponse().m_who:isAlive() and not data:toCardResponse().m_who:isNude() then
							local id2 = room:askForCardChosen(player, data:toCardResponse().m_who, "he",
								self:objectName())
					...
					elseif choiceJ == "jink3" then
						...
						if data:toCardResponse().m_who:isAlive() and not data:toCardResponse().m_who:isNude() then
							local id3 = room:askForCardChosen(player, data:toCardResponse().m_who, "he",
								self:objectName())
...
sp_zhengbianCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		local caoshuang = room:askForPlayerChosen(source, room:getOtherPlayers(source), "sp_zhengbian",
			"sp_zhengbianToDo")
				...
				if can_invoke then
					...
					if to then
						room:moveCardTo(card, from, to, place,
							sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_TRANSFER, source:objectName(), "sp_kongju", ""))
					end
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.PreCardUsed then
			...
			if use.card:isKindOf("Dismantlement") and use.card:getSkillName() == "sp_lvezhen" then
				...
				room:askForUseCard(player, "@@sp_lvezhen_SkillCardBuff",
					"@sp_lvezhen_SkillCardBuff:" .. use.card:objectName() .. "::" .. 1, -1, sgs.Card_MethodNone)
				local adds = sgs.SPlayerList()
				for _, p in sgs.qlist(extra_targets) do
					room:setPlayerFlag(p, "-sp_lvezhen_SkillCardBuff")
...
sp_xiyingSettle = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.EventPhaseChanging then
			...
			if caoying then
				room:setFixedDistance(player, caoying, -1)
			end
...
fcj_kannanCard = sgs.CreateSkillCard {
	...
	filter = function(self, targets, to_select)
		return #targets == 0 and to_select:objectName() ~= sgs.Self:objectName() and
			not to_select:hasFlag("fcj_kannanSelected") and not to_select:isKongcheng()
	end,
	on_effect = function(self, effect)
		local room = effect.from:getRoom()
...
fcj_pingcaiCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		elseif choice == "xuanjian" then
			...
			log.from = source
			room:sendLog(log)
			if not room:askForUseCard(source, "@@fcj_pingcaiXuanjian", "@fcj_pingcaiXuanjian-card") and source:getState() == "online" then
				return false
...
fcj_pingcaiShuijingCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		local to = room:askForPlayerChosen(source, tos, "fcj_pingcaiShuijing",
			"@fcj_pingcaiShuijing_Equip-to:" .. card:objectName())
		if to then
			room:moveCardTo(card, targets[1], to, place,
				sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_TRANSFER, source:objectName(), "fcj_pingcaiShuijing", ""))
			room:broadcastSkillInvoke("fcj_pingcai", 4)
		end
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			if use.from:objectName() == player:objectName() and use.card and not use.card:isKindOf("SkillCard") and player:getPhase() == sgs.Player_Play then
				...
				if player:hasFlag("fcj_wanglie_cantchooseHit") and player:hasFlag("fcj_wanglie_cantchooseDamage")
					and player:hasFlag("fcj_wanglie_cantchooseBeishui") then
					return false
				end
				if room:askForSkillInvoke(player, self:objectName(), data) then
					room:broadcastSkillInvoke(self:objectName())
		...
		elseif event == sgs.EventPhaseEnd then
			if player:getPhase() == sgs.Player_Play then
				...
				if player:hasFlag("fcj_wanglie_cantchooseHit") then
					room:setPlayerFlag(player,
						"-fcj_wanglie_cantchooseHit")
				end
				if player:hasFlag("fcj_wanglie_cantchooseDamage") then
					room:setPlayerFlag(player,
						"-fcj_wanglie_cantchooseDamage")
				end
					...
					if will_use then
						...
						if not player:isNude() --[[and player:objectName() ~= to:objectName()]] then
							...
							if player:getCardCount() > 1 then
								card = room:askForCard(player, "..!", "@fcj_kangkai-give:" .. to:objectName(), data,
									sgs.Card_MethodNone);
								if not card then
							...
							elseif not card:isKindOf("BasicCard") and not card:isKindOf("Nullification")
								...
								if will_use then
									if to:getState() == "robot" and card:isKindOf("EquipCard") then
										...
										if #pattern > 0 then
											room:askForUseCard(to, table.concat(pattern, ","),
												"@fcj_kangkai_ut:" .. card:objectName(), -1)
										end
									end
								end
							end
...
fcj_jueyanCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		local log = sgs.LogMessage()
		if x == 0 then
			log.type = "$fcj_jueyan-0"
		elseif x == 1 then
			log.type = "$fcj_jueyan-1"
		elseif x == 2 then
			log.type = "$fcj_jueyan-2"
		elseif x == 4 then
			log.type = "$fcj_jueyan-4"
		end
		log.from = source
		room:sendLog(log)
	end,
...
ps_huairouCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		room:moveCardTo(self, source, nil, sgs.Player_DiscardPile,
			sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_RECAST, source:objectName(), self:objectName(), ""))
