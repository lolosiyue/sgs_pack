		...
		while n > 0 do
			for _, p in pairs(targets) do
				if math.random() <= 0.5 then
					room:broadcastSkillInvoke("f_mobile_efs", 1)
				else
					room:broadcastSkillInvoke("f_mobile_efs", 2)
				end
...
f_mobile_efs_flowerCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		while n > 0 do
			for _, p in pairs(targets) do
				if math.random() <= 0.5 then
					room:broadcastSkillInvoke("f_mobile_efs", 3)
				else
					room:broadcastSkillInvoke("f_mobile_efs", 4)
				end
							...
							if player:getWeapon() ~= nil then
								...
								elseif jbs == 3 then
									if otr:getKingdom() == "wei" then
										otr:speak("" ..
										sgs.Sanguosha:translate(player:getGeneralName()) .. "，到许昌了指定没你好果汁吃嗷")
									elseif otr:getKingdom() == "shu" then
										otr:speak("" ..
										sgs.Sanguosha:translate(player:getGeneralName()) .. "，到成都了指定没你好果汁吃嗷")
									elseif otr:getKingdom() == "wu" then
										otr:speak("" ..
										sgs.Sanguosha:translate(player:getGeneralName()) .. "，到建业了指定没你好果汁吃嗷")
									elseif isSpecialOne(otr, "董卓") then
										otr:speak("" ..
										sgs.Sanguosha:translate(player:getGeneralName()) .. "，到长安了指定没你好果汁吃嗷")
									else
										otr:speak("" ..
										sgs.Sanguosha:translate(player:getGeneralName()) .. "，到洛阳了指定没你好果汁吃嗷")
									end
								end
							end
						else
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			...
			if use.card and use.card:getSkillName() == "tenyearzhiheng" then
				...
				if qcsj == 0 then
					if qcsjj == 1 then
						player:speak("感觉不如界孙权，精品第一、史诗质检员")
					elseif qcsjj == 2 then
						player:speak("回合内小制衡找顺拆卡距离随便打")
					elseif qcsjj == 3 then
						player:speak("不好打的制衡找无懈闪桃")
					elseif qcsjj == 4 then
						player:speak("下回合找顺拆继续打")
					elseif qcsjj == 5 then
						player:speak("对爆也是界权优")
					end
				elseif qcsj == 1 then
					if qcsjj == 1 then
						player:speak("君言甚妙，但余窃以为差界权甚矣")
					elseif qcsjj == 2 then
						player:speak("全扩之鳌首、阴间之巅峰")
					elseif qcsjj == 3 then
						player:speak("待大制衡神技，顺拆乐马五中等尽入囊中")
					elseif qcsjj == 4 then
						player:speak("卡敌之距离，去敌之珍牌")
					elseif qcsjj == 5 then
						player:speak("神郭、神荀、神甘俯首帖耳，\
					况区区文鸯、大宝、杨彪、杜预之流！")
					end
				elseif qcsj == 2 then
					if qcsjj == 1 then
						player:speak("卿言是，然知不若界权")
					elseif qcsjj == 2 then
						player:speak("于精品榜首，乃史诗之质检")
					elseif qcsjj == 3 then
						player:speak("自回合内彼伺其技大制衡")
					elseif qcsjj == 4 then
						player:speak("寻顺拆，卡敌之击距，轻肆殴之")
					elseif qcsjj == 5 then
						player:speak("与敌之多力，权仍胜之")
					end
				elseif qcsj == 3 then
					if qcsjj == 1 then
						player:speak("你说对，但不如，界孙权")
					elseif qcsjj == 2 then
						player:speak("精品中，数第一，史诗将，质检员")
					elseif qcsjj == 3 then
						player:speak("大制衡，找拆牵，卡距离，随便打")
					elseif qcsjj == 4 then
						player:speak("如不行，找闪桃，下回合，继续打")
					elseif qcsjj == 5 then
						player:speak("对爆时，权亦优")
					end
				end
			end
		...
		elseif event == sgs.EventPhaseStart then
			...
			if player:getPhase() == sgs.Player_Finish then
				if player:getKingdom() == "wu" and isSpecialOne(room:getLord(), "界孙权") then
					...
					if sq == 0 then
						player:speak("思权拳 思如泉涌!👊😭👊")
					elseif sq == 1 then
						player:speak("念权剑 念念不忘!!🗡😭🗡")
					elseif sq == 2 then
						player:speak("界权掌 生生世世!!!✋😭✋")
					elseif sq == 3 then
						player:speak("会玩、会玩、会玩！")
					elseif sq == 4 then
						player:speak("璀璨中的凋零、制衡联合！😊👉👈😊")
					elseif sq == 5 then
						player:speak("极冻中的炽烈、纵横捭阖！😚☺👦👧")
					elseif sq == 6 then
						player:speak("虚无中的真言、容我三思！😁👍💪😁")
					elseif sq == 7 then
						player:speak("冰霜中的独舞、则吴盛可期！😍👼👼😍")
					end
				end
		...
		elseif event == sgs.Dying then
			...
			if dying.who and dying.who:objectName() == player:objectName() and isSpecialOne(player, "张春华") and (player:getSeat() == 2 or player:getSeat() == room:alivePlayerCount())
				...
				if math.random() > 0.75 then
					player:speak("明反不搞，把我往死里盲") --实际可能会有些牛头不对马嘴
				else
					player:speak("我不知道我哪里跳反了")
				end
			end
		elseif event == sgs.Death then
			local death = data:toDeath()
