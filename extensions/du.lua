		...
		if (move.to_place == sgs.Player_DiscardPile) then
			...
			if (bit32.band(move.reason.m_reason, sgs.CardMoveReason_S_MASK_BASIC_REASON) == sgs.CardMoveReason_S_REASON_DISCARD) then
				for _, card_id in sgs.qlist(move.card_ids) do
					if (move.to_place == sgs.Player_DiscardPile) and room:getCardOwner(card_id) and (room:getCardOwner(card_id):objectName() == player:objectName())
...
jfTakeCard = sgs.CreateSkillCard {
	...
	on_effect = function(self, effect)
		...
		if not jin:isEmpty() then
			...
			local choice = room:askForChoice(target, "jinfan",
				"jinfanTake_allow=" .. source:objectName() .. "+jinfanTake_disallow=" .. source:objectName())
...
jinfanTake = sgs.CreateViewAsSkill {
	...
	enabled_at_play = function(self, player)
		return not player:hasUsed("#jfTaken")
	end,
...
du_zhouxuanCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		local move1 = sgs.CardsMoveStruct(a:handCards(), b, sgs.Player_PlaceHand,
			sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_SWAP, a:objectName(), b:objectName(), "du_zhouxuan", ""))
		local move2 = sgs.CardsMoveStruct(b:handCards(), a, sgs.Player_PlaceHand,
			sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_SWAP, b:objectName(), a:objectName(), "du_zhouxuan", ""))
...
yaochong = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardsMoveOneTime then
			...
			if move.to and move.from and (move.from:objectName() == player:objectName()) and move.to and move.to:isAlive()
				and (move.from:objectName() ~= move.to:objectName())
				and (move.card_ids:length() >= 2)
				and (move.reason.m_reason ~= sgs.CardMoveReason_S_REASON_PREVIEWGIVE) then
				local _moveto
...
duoDaoAndMa = sgs.CreateTriggerSkill {
	...
	events = { sgs.EventPhaseStart },
	on_trigger = function(self, event, player, data)
		if player:getPhase() == sgs.Player_Start and (player:getMark("BladeUsed") == 0 or player:getMark("ChiTuUsed") == 0) then
			local room = player:getRoom()
		...
		if killer:objectName() == player:objectName() and (effect.slash:hasFlag("drank")) then
			...
			if not target:isNude() then
				if player:askForSkillInvoke(self:objectName(), data) then
					...
					local choice = room:askForChoice(player, self:objectName(), "jwTake+jwDrop")
					if choice == "jwTake" then
...
tongpaoSlashvs = sgs.CreateViewAsSkill {
	...
	enabled_at_response = function(self, player, pattern)
		return pattern == "slash" and
			...
			and not player:hasFlag("Global_tongpaoFailed")
	end,
	view_as = function(self, cards)
...
tongpaoSlash = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if data:toStringList()[2] == "@tongpao-slash" then return false end
		if not player:askForSkillInvoke("tongpao_slash", data) then return false end
		local room = player:getRoom()
		local log = sgs.LogMessage()
		log.type = "#tongpao"
		log.from = player
...
tongpaoJink = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, xiahoujuan, data)
		...
		if data:toStringList()[2] == "@tongpao-jink" then return false end
		if not xiahoujuan:askForSkillInvoke("tongpao_jink", data) then return false end
		...
		log.from = xiahoujuan
		room:sendLog(log)
		local lieges = room:getLieges("wu", xiahoujuan)
		local dest = sgs.QVariant()
		dest:setValue(xiahoujuan)
		for _, p in sgs.qlist(lieges) do
			local jink = room:askForCard(p, "jink", "@tongpao-jink", dest, sgs.Card_MethodResponse, nil, false, "", true)
			if jink then
				room:provide(jink)
				return true
			end
...
sgs.LoadTranslationTable {
	...
	["du_jieying"] = "劫营",
	[":du_jieying"] = "<font color=\"purple\"><b>觉醒技，</b></font>回合开始阶段，若“锦”数不小于3，你回复1点体力，然后减1点体力上限，获得“奇袭”",
	["duYinling"] = "银铃",
	["duyinling"] = "银铃",
	["duYinlingCard"] = "银铃",
	[":duYinling"] = "摸牌阶段开始时，你可以获得一名其他角色一张手牌。若如此做，弃牌阶段结束后，你需要弃置一张牌。",
	["@duYinlingCard"] = "是否发动技能【银铃】",
	["~duYinling"] = "请选择一名角色,然后点击确定",
	["jfTakeCard"] = "锦帆",
	["jinfanTake"] = "锦帆",
	[":jinfanTake"] = "出牌阶段，若甘宁允许，你可以取一张“锦”作为手牌",
	["jinfan"] = "锦帆",
	["jftake"] = "锦帆",
	[":jinfan"] = "当你的牌因弃置进入弃牌堆时，你可以将其置于武将牌上，称为“锦”。吴国势力角色的出牌阶段限一次，其可选择一张“锦”，你可以将之交给其。",
	["du_jin"] = "锦",
	...
	["duXiaoguo"] = "骁果",
	[":duXiaoguo"] = "当你对一名其他角色造成伤害时，你可以与其拼点，若你赢则伤害+1。",
	["tongpao"] = "同袍",
	[":tongpao"] = "当吴势力角色需要使用或打出【杀】或【闪】时，其他吴势力角色可以代为使用或打出【杀】或【闪】",
	["tongpao_jink"] = "【同胞】技能被触发，请吴国势力代为出“闪”",
	["tongpao_slash"] = "【同胞】技能被触发，请吴国势力代为出“杀”",
	["#tongpao"] = "%from 请吴国势力代为打出【杀】或【闪】",

	["du_zhouxuan"] = "周旋",
	[":du_zhouxuan"] = "出牌阶段，若你拥有“宠”标记，你可以弃一张手牌，与一名男性角色交换其余手牌，或交换两名男性角色的手牌。若如此做，你将失去一枚“宠”标记。",
	["du_zhouxuanCard"] = "周旋",
	["yaochong"] = "邀宠",
	...
	["duWuhun"] = "武魂",
	[":duWuhun"] = "回合开始时，你获得场上、牌堆或弃牌堆里的一张【青龙偃月刀】或【赤兔马】。觉醒技，回合开始时，若你已经装备【青龙偃月刀】则获得“虎啸”，若已装备【赤兔马】则获得“马术”。获得上述两技能后，你回复一点体力，减少一点体力上限，并“武魂”改为失效。。",
	["jieyou"] = "解忧",
	[":jieyou"] = "你可以将一张♠2~9手牌当【酒】使用。",

	["du_tongque"] = "铜雀",
	[":du_tongque"] = "<font color=\"orange\"><b>主公技，</b></font>当魏势力角色导致场上女性角色濒死并求桃失败后，你可以将一张手牌交给其，令其复活。该角色摸三张牌，恢复1点体力，减少1点体力上限，将势力改为魏，并改变身份为“内奸”",
	...
	["xixing"] = "庸纳",
	[":xixing"] = "每当你收到伤害，你可以获得伤害来源的全部手牌",

	["du_jiyu"] = "积郁",
	[":du_jiyu"] = "回合开始阶段，若你的手牌数量大于手牌上限，你减少一点体力上限",
	["duBaonue"] = "暴虐",
	[":duBaonue"] = "<font color=\"orange\"><b>主公技，</b></font>当其他群势力角色造成伤害后，其可以判定，若结果为♠，你增加1点体力上限。",
