	...
	on_trigger = function(self,event,player,data,room)
		...
		if event==sgs.TargetConfirmed
	   	then
			...
			or (use.from and (player:objectName()==use.from:objectName()))
		    then return end
...
ov_yingjiCard = sgs.CreateSkillCard{
	...
	on_validate = function(self,use)
		...
		NotifySkillInvoked("ov_yingji",yuji,use.to,false)
		yuji:drawCards(1,"ov_yingji")
		local to_guhuo = self:getUserString()
		to_guhuo = room:askForChoice(yuji,"ov_yingji",to_guhuo)
...
ov_mouli = sgs.CreateViewAsSkill {
	...
	view_as = function(self,cards)
		...
		then pattern = sgs.Self:getTag("ov_mouli"):toCard():objectName() end
...
ov_dingzhenbf = sgs.CreateProhibitSkill{
	name = "#ov_dingzhenbf",
	is_prohibited = function(self,from,to,card)
		if card:getTypeId()~=0 and to and to:hasSkill("ov_dingzhen")
...
ov_yuhua = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data,room)
		...
		then
			...
			and (not move.to or move.to:objectName()~=player:objectName() or move.to_place~=sgs.Player_PlaceHand and move.to_place~=sgs.Player_PlaceEquip)
			then
				local n = false
				for i,id in sgs.list(move.card_ids)do
					i = move.from_places:at(i)
...
ov_pingting = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data)
		...
		then
			...
			if dy.who==player or not (player:getPhase() ~= sgs.Player_NotActive) then return end
			room:sendCompulsoryTriggerLog(player,"ov_pingting")
			player:drawCards(1,"ov_pingting")
			local card = room:askForCard(player,"..!","ov_pingting0:",sgs.QVariant(),sgs.Card_MethodNone)
			if card then player:addToPile("ov_xingwu",card) end
...
--[[
ov_jxtpguyong = sgs.General(extension,"ov_jxtpguyong","wu",3)
...
ov_fujian = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data,room)
		...
		then
			...
			and (player:getPhase() == sgs.Player_NotActive) and move.from:objectName()==player:objectName()
...
--		Log_message("#ov_kaizeng",use.from,nil,nil,msg)
        if ls:isYourFriend(use.from) then room:setPlayerFlag(ls,"wantgivekaizeng") end
		local dc = room:askForExchange(ls,"ov_kaizeng",ls:getCardCount(),1,true,"ov_kaizeng0:"..use.from:objectName()..":"..msg,true)
		room:setPlayerFlag(ls,"-wantgivekaizeng")
		if dc
		then
...
ov_zhanghong = sgs.General(extension,"ov_zhanghong","wu")
