...
f_huishiContinue = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if use.card:getSkillName() == "f_huishi" and player:getMaxHp() < 10 and not player:hasFlag("DaoZhang_ZhuoYao") then
			...
			--第二次判定
			if player:getMaxHp() < 10 and room:askForSkillInvoke(player, "@f_huishi_continue", data) and not player:hasFlag("DaoZhang_ZhuoYao") then
				room:broadcastSkillInvoke("f_huishi")
				local judge = sgs.JudgeStruct()
				judge.pattern = "."
				judge.good = true
				judge.play_animation = false
...
f_qizhengxiangsheng = sgs.CreateTrickCard {
	...
	on_effect = function(self, effect)
		...
		if choicee == "buyaosong" then
			local choice1 = room:askForChoice(effect.to, self:objectName(), "yingduiqibing+yingduizhengbing+wobuyingdui",
				data)
			if choice1 == "yingduiqibing" then
				room:askForCard(effect.to, "slash", "f_qizhengxiangsheng-slash:" .. effect.from:objectName(),
					sgs.QVariant(), sgs.Card_MethodResponse)
		...
		elseif choicee == "touxi!" then
			local choice2 = room:askForChoice(effect.to, self:objectName(), "yingduiqibing+yingduizhengbing+wobuyingdui",
				data)
			...
			elseif choice2 == "yingduizhengbing" then
				room:askForCard(effect.to, "jink", "f_qizhengxiangsheng-jink:" .. effect.from:objectName(),
					sgs.QVariant(), sgs.Card_MethodResponse)
				if effect.to:isAlive() then
					local damage = sgs.DamageStruct()
					damage.from = effect.from
					...
					if not room:askForUseCard(player, "@@ngCRemover", "@ngCRemover") then --玩家选择是否加入【神武将专属卡牌】
						...
						for _, id in sgs.qlist(room:getDrawPile()) do
							...
							if sgs.Sanguosha:getCard(id):isKindOf("Fqizhengxiangsheng") then
								...
								local move = sgs.CardsMoveStruct(id, nil, nil, sgs.Player_DrawPile, sgs
									.Player_PlaceTable,
		...
		elseif event == sgs.MarkChanged then
			...
			if mark.name == "&tyHuang" and mark.who:hasSkill(self:objectName()) and mark.who:objectName() == player:objectName() then
				...
				elseif m >= 400 and m < 500 then
					m = m - 400
				elseif m >= 500 and m < 600 then
					m = m - 500
				elseif m >= 600 and m < 700 then
					m = m - 600
				elseif m >= 700 and m < 800 then
					m = m - 700
				elseif m >= 800 and m < 900 then
					m = m - 800
				elseif m >= 900 and m < 1000 then
					m = m - 900
					--......
				end
				...
				elseif m >= 70 and m < 80 then
					x = 7
				elseif m >= 80 and m < 90 then
					x = 8
				elseif m >= 90 and m < 100 then
					x = 9
				end
...
tytuoyuVS = sgs.CreateZeroCardViewAsSkill {
	...
	enabled_at_response = function(self, player, pattern)
		...
		then
			...
			return pattern == "slash" or pattern == "jink" or pattern == "peach" or pattern == "nullification"
		end
	end,
}
--查看【丰田】
	...
	on_trigger = function(self, event, player, data)
		...
		for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
			...
			if (p:getMark("@tyty_Fengtian") == 0 or p:getMark("@tyty_Qingqu") == 0 or p:getMark("@tyty_Junshan") == 0)
				and not p:canWake(self:objectName()) then
				continue
			end
			room:broadcastSkillInvoke(self:objectName())
	...
	on_trigger = function(self, event, player, data)
		...
		if n > 0 and player:getMark("@tyjuanjia_equiparea") > 0 then
			...
			if not cds:isEmpty() then
				...
				room:setPlayerProperty(player, "tyqiexie_jjarea_lost_record",
					sgs.QVariant(table.concat(jqxs_tolose, ",")))
				--[[for _, m in sgs.list(player:getMarkNames()) do
			...
			if to then
				...
				if math.mod(player:getHandcardNum(), 2) == 1 then
					while true do
						...
						to = room:askForPlayerChosen(player, players, self:objectName(), self:objectName() .. "-invoke",
							true, true)
					end
				else
					while true do
			...
			if not cards:isEmpty() then
				...
				if equipA then
					...
					if player:getEquip(equip_index) == nil and player:hasEquipArea(equip_index) then
						room:sendCompulsoryTriggerLog(player, self:objectName())
						room:broadcastSkillInvoke(self:objectName())
						Moves:append(sgs.CardsMoveStruct(equipA:getId(), player, sgs.Player_PlaceEquip,
							sgs.CardMoveReason()))
					else
						Moves:append(sgs.CardsMoveStruct(equipA:getId(), player, sgs.Player_PlaceHand,
							sgs.CardMoveReason()))
					end
	...
	on_trigger = function(self, event, player, data)
		...
		if player:hasSkill(self:objectName()) and room:askForSkillInvoke(player, self:objectName(), data) then
			...
			if suit == sgs.Card_Heart then
				log.type = "$joymeihun_heart"
			elseif suit == sgs.Card_Diamond then
				log.type = "$joymeihun_diamond"
			elseif suit == sgs.Card_Club then
				log.type = "$joymeihun_club"
			elseif suit == sgs.Card_Spade then
				log.type = "$joymeihun_spade"
			end
			log.from = player
			room:sendLog(log)
			...
			local MHto
			if MH:length() > 1 then
				MHto = room:askForPlayerChosen(player, MH, self:objectName())
			...
			if dummy_card:subcardsLength() > 0 then
				local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_TRANSFER, player:objectName(),
					MHto:objectName(), self:objectName(), nil)
			...
			if pindian.reason == "joyhuoxin" then
				...
				for _, lsr in sgs.qlist(losers) do
					...
					if choice == "gh=" .. jsdc:objectName() or choice == "gd=" .. jsdc:objectName() or choice == "gc=" .. jsdc:objectName() or choice == "gs=" .. jsdc:objectName() then
						local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_TRANSFER, jsdc:objectName(),
							lsr:objectName(), "joyhuoxin", nil)
				...
				while n > 0 do
					...
					if choice == "w" then
						...
						if weapon_movers:length() > 1 then
							move_from = room:askForPlayerChosen(player, weapon_movers, self:objectName(),
								"joyyizheng_movefrom")
					...
					elseif choice == "a" then
						...
						if armor_movers:length() > 1 then
							move_from = room:askForPlayerChosen(player, armor_movers, self:objectName(),
								"joyyizheng_movefrom")
					...
					elseif choice == "h" then
						...
						if horse_movers:length() > 1 then
							move_from = room:askForPlayerChosen(player, horse_movers, self:objectName(),
								"joyyizheng_movefrom")
					...
					if move_from then
						...
						local move_to = room:askForPlayerChosen(player, tos, self:objectName(),
							"joyyizheng_moveto:" .. card:objectName())
						if move_to then
							room:moveCardTo(card, move_from, move_to, place,
								sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_TRANSFER, player:objectName(),
									self:objectName(), ""))
						end
...
joyseventytwoCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		if CZ:isKindOf("BasicCard") then
			room:setPlayerFlag(source, "joyseventytwoBasic")
		elseif CZ:isKindOf("TrickCard") then
			room:setPlayerFlag(source, "joyseventytwoTrick")
		elseif CZ:isKindOf("EquipCard") then
			room:setPlayerFlag(source, "joyseventytwoEquip")
		end
	end,
}
		...
		elseif event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_RoundStart then
				for _, skw in sgs.qlist(room:findPlayersBySkillName("qt_huoyan")) do
					local hyjj = room:askForPlayersChosen(skw, room:getOtherPlayers(skw), "qt_huoyan", 0, 999,
						"@qt_huoyan-tosee", false, true)
					if not hyjj:isEmpty() then
...
joyseventytwoEXCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		if CZ:isKindOf("BasicCard") then
			room:setPlayerFlag(source, "joyseventytwoEXBasic")
		elseif CZ:isKindOf("TrickCard") then
			room:setPlayerFlag(source, "joyseventytwoEXTrick")
		elseif CZ:isKindOf("EquipCard") then
			room:setPlayerFlag(source, "joyseventytwoEXEquip")
		end
...
joyqitianEX = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		sgs.Sanguosha:playAudioEffect("audio/skill/joyqitianEXs.ogg", false)
		room:broadcastSkillInvoke(self:objectName())
		room:doLightbox("joyqitianAnimate")
		room:loseMaxHp(player, 1)
		room:addPlayerMark(player, self:objectName())
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.Damaged then
			...
			if damage.to and damage.to:isAlive() and damage.to:objectName() == player:objectName() and player:hasSkill(self:objectName())
				...
				if spk == 0 then
					player:speak("苦肉一次，弓无闪有，弓啊在哪")
				elseif spk == 1 then
					player:speak("苦肉无数，苦了没奶")
				elseif spk == 2 then
					player:speak("还拿桃哦，你自己打~")
				end
				room:broadcastSkillInvoke(self:objectName(), 3)
	...
	on_use = function(self, room, source, targets)
		...
		for _, p in sgs.qlist(room:getOtherPlayers(source)) do
			...
			if p:getState() == "robot" then
				...
				local data = sgs.QVariant()
				data:setValue(p)
				local ph_card = room:askForCard(p, "peach", "@jlsgguiyuan-wtgy:" .. source:objectName(), data,
					sgs.Card_MethodNone)
				if ph_card then
					source:obtainCard(ph_card)
				else
...
oflhuishiCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, target)
		...
		if source:isAlive() and slash:subcardsLength() > 0 then
			...
			local to = room:askForPlayerChosen(source, room:getAlivePlayers(), "oflhuishi", "@oflhuishi-give", true,
				false)
			room:clearAG(source)
...
ofllingce = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.TargetConfirming then
			if use.card and not use.card:isKindOf("SkillCard") and use.from and use.from:objectName() ~= player:objectName() then
				local names, name = player:property("SkillDescriptionRecord_ofldinghan"):toString():split("+"),
					use.card:objectName()
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.GameStart then
			if player:hasSkill("ofldinghan") then
				...
				room:setPlayerProperty(player, "SkillDescriptionRecord_ofldinghan",
...
ofldinghan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		local phase = player:getPhase()
		if phase ~= sgs.Player_Start then return false end
		local record, other, dinghan, tricks = sgs.IntList(), sgs.IntList(),
			player:property("SkillDescriptionRecord_ofldinghan"):toString():split("+"), {}
...
oflyuxueEX = sgs.CreateOneCardViewAsSkill {
	...
	enabled_at_play = function(self, player)
		...
		return player:getPile("oflAngry"):length() > 0
			and
			(player:isWounded() or player:usedTimes("Analeptic") <= sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_Residue, player, newana))
	end,
	enabled_at_response = function(self, player, pattern)
		return player:getPile("oflAngry"):length() > 0
...
ofljuwuCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, source:objectName(), sidi:objectName(),
			"ofljuwu", "")
...
ofljuwuEXCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, source:objectName(), friend:objectName(),
			"ofljuwuEX", "")
					...
					for _, oc in sgs.qlist(room:getOtherPlayers(player)) do
						if oc:hasFlag(self:objectName()) then
							...
							if choice == "1" then
								...
								local card = room:askForExchange(oc, self:objectName(), 999, 2, true,
									"#f_xiongyan:" .. player:getGeneralName())
								if card then
									room:obtainCard(player, card,
										sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, player:objectName(),
											oc:objectName(), self:objectName(), ""), false)
		...
		for _, p in sgs.qlist(room:getAllPlayers()) do
			if n > 0 and source:canDiscard(p, "ej") and p:getEquips():length() + p:getJudgingArea():length() > 0 then
				...
				while n > 0 and m > 0 and p:getEquips():length() + p:getJudgingArea():length() > 0 do
					local ruins = room:askForCardChosen(source, p, "ej", "f_qiandu", false, sgs.Card_MethodDiscard,
						sgs.IntList(), true)
...
f_mizong = sgs.CreateMasochismSkill {
	...
	on_damaged = function(self, player, damage)
		...
		if room:askForSkillInvoke(player, self:objectName(), data) then
			...
			room:broadcastSkillInvoke(self:objectName(), 2)
			if player:hasSkill(self:objectName()) then
				room:addPlayerMark(player, "&f_mizongRMC")
			end
		end
	end,
}
		...
		if event == sgs.TargetConfirmed then
			...
			if use.card:isKindOf("Indulgence") then
				...
				for _, sls in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					...
					if not room:askForSkillInvoke(sls, self:objectName(), data) then
						room:setPlayerFlag(sls, "f_lejiNI")
						continue
					end
					room:broadcastSkillInvoke(self:objectName())
				...
				if can_invoke then
					if room:askForSkillInvoke(player, self:objectName(), data) then
						...
						if choice == "1" then
							...
							local Indulgence = sgs.Sanguosha:cloneCard("indulgence", IdgCard:getSuit(),
								IdgCard:getNumber())
						...
						elseif choice == "2" then
							...
							room:moveCardTo(card, BSS, player, place,
								sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_TRANSFER, player:objectName(),
									self:objectName(), ""))
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.CardsMoveOneTime then
			if move.from and move.from:objectName() == player:objectName() then
				...
				if can_invoke then
					if room:askForSkillInvoke(player, self:objectName(), data) then
						local smz = room:askForPlayerChosen(player, room:getAllPlayers(), self:objectName(),
							"f_danshaSiMaZhao")
						room:broadcastSkillInvoke(self:objectName())
						room:loseHp(smz, 2)
		...
		if move.to and move.to:objectName() == player:objectName() and move.reason.m_skillName == "f_yonghun" then
			for _, id in sgs.qlist(move.card_ids) do
				if room:getCardOwner(id):objectName() == player:objectName() and room:getCardPlace(id) == sgs.Player_PlaceHand then
					local card = sgs.Sanguosha:getCard(id)
					if card:isKindOf("Slash") then
						room:setCardFlag(card, "f_yonghunSlash")
					elseif card:isKindOf("Jink") then
						room:setCardFlag(card, "f_yonghunJink")
					elseif card:isKindOf("Peach") then
						room:setCardFlag(card, "f_yonghunPeach")
					elseif card:isKindOf("Analeptic") then
						room:setCardFlag(card, "f_yonghunAnaleptic")
					end
				end
			end
			--1.寻找其中的【酒】并询问发动效果
			...
			for _, cd in sgs.qlist(player:getHandcards()) do --清除标志
				if cd:hasFlag("f_yonghunSlash") then
					room:setCardFlag(cd, "-f_yonghunSlash")
				elseif cd:hasFlag("f_yonghunJink") then
					room:setCardFlag(cd, "-f_yonghunJink")
				elseif cd:hasFlag("f_yonghunPeach") then
					room:setCardFlag(cd, "-f_yonghunPeach")
				elseif cd:hasFlag("f_yonghunAnaleptic") then
					room:setCardFlag(cd, "-f_yonghunAnaleptic")
				end
			end
		end
	end,
				...
				for _, p in sgs.qlist(room:getOtherPlayers(player)) do
					if p:hasFlag("f_JYuan_real") then
						...
						if p:getWeapon() ~= nil or not table.contains(JianYuanLu, p:getGeneralName()) then
							...
							if choice == "x" then
								room:addPlayerMark(player, "&f_jianyuanX")
							else
								room:addPlayerMark(player, "&f_jianyuanY")
							end
						end
						if not table.contains(JianYuanLu, p:getGeneralName()) and room:askForSkillInvoke(player, self:objectName(), FCToData("JianYuanLu:" .. p:objectName())) then
							...
							room:setPlayerProperty(player, "SkillDescriptionRecord_f_jianyuan",
								sgs.QVariant(table.concat(JianYuanLu, "+")))
...
f_miaodao = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.EventPhaseChanging then
			local change = data:toPhaseChange()
			if change.to ~= sgs.Player_NotActive then return false end
			local n = player:getPile("f_syjDao"):length()
			if n > 4 then n = 4 end
			if n > 0 then
				room:sendCompulsoryTriggerLog(player, self:objectName())
				room:broadcastSkillInvoke(self:objectName())
...
f_yifaCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_PUT, source:objectName(), nil, self:objectName(),
			nil)
				...
				if room:askForSkillInvoke(p, self:objectName(), data) then
					...
					if choice == "handcard" then
						...
						if hc then
							...
							local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_PUT, p:objectName(), nil,
								self:objectName(), nil)
					...
					if choicee == "move" then
						...
						if from and (from:getEquips():length() > 0 or from:getJudgingArea():length() > 0) then
							...
							if card_id >= 0 then
								...
								local to = room:askForPlayerChosen(p, tos, self:objectName(),
									"@f_yifa_Move-to:" .. card:objectName())
								if to then
									room:moveCardTo(card, from, to, place,
										sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_TRANSFER, p:objectName(),
											self:objectName(), ""))
									room:broadcastSkillInvoke(self:objectName())
								end
					...
					elseif choicee == "get" then
						...
						room:obtainCard(p, sgs.Sanguosha:getCard(card_id), reason,
							room:getCardPlace(card_id) ~= sgs.Player_PlaceHand)
						room:broadcastSkillInvoke(self:objectName())
					end
				else
		...
		if event == sgs.GameStart then
			...
			elseif choice == "JJ" then
				...
				local sbk = { "_hunduwanbi", "_shuibojian", "_liecuidao", "_hongduanqiang",
					...
					"wutiesuolian", "wuxinghelingshan",
					"_f_linglongshimandai", "_f_hongmianbaihuapao", "_f_guofengyupao", "_f_qimenbazhen", "huxinjing",
					"heiguangkai",
		...
		if damage.card:isKindOf("Slash") and damage.from:objectName() == player:objectName() and damage.to then
			if room:askForSkillInvoke(player, self:objectName(), data) then
				...
				if choice == "1" then
					...
					local card = room:askForCardChosen(player, damage.to, "he", self:objectName(), false,
						sgs.Card_MethodDiscard)
					sgs.Sanguosha:playAudioEffect("audio/equip/Halberd.ogg", false)
					room:setEmotion(player, "weapon/halberd")
	...
	on_use = function(self, room, source, targets)
		...
		while n > 0 do
			...
			if card:isKindOf("EquipCard") then
				success = success + 1
				local to = room:askForPlayerChosen(source, room:getAllPlayers(), "f_qigong",
					"f_qigongUse:" .. card:objectName())
				local equip_index = card:getRealCard():toEquipCard():location()
		...
		elseif event == sgs.Damage then --“毒杀”效果
			if damage.card and damage.card:objectName() == "slash" and (damage.card:getSkillName() == "f_lingqix_poison" or damage.card:hasFlag("poison_slash")) then
				...
				if lhp >= 5 or (lhp < 5 and math.random() <= n) then --5*20%=100%
					if damage.to:isMale() then
						sgs.Sanguosha:playAudioEffect("audio/system/poison_injure1.ogg", false)
					elseif damage.to:isFemale() then
						sgs.Sanguosha:playAudioEffect("audio/system/poison_injure2.ogg", false)
					end
	...
	on_effect = function(self, effect)
		...
		if effect.from:getMark("&fXiaJiang") > 0 then
			...
			if rcard then
				...
				room:moveCardTo(rcard, effect.from, nil, sgs.Player_DiscardPile,
					sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_RECAST, effect.from:objectName(), "f_shenjiang", ""))
				local log = sgs.LogMessage()
				log.type = "#UseCard_Recast"
...
f_yanfa = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.GameStart then
			--统计隐藏卡牌（后续还会视情况扩增）
			local hidden_cards = { "_qizhengxiangsheng", "_hunduwanbi", "_shuibojian", "_liecuidao", "_hongduanqiang",
				"_tianleiren",
				"_piliche", "_secondpiliche", "_tenyearpiliche", "_feilunzhanyu", "_sichengliangyu", "_tiejixuanyu",
				"_jinshu", "_qiongshu", "_xishu",
				"_f_wushuangfangtianji", "_f_guilongzhanyuedao", "_f_chixieqingfeng", "_f_bingtieshuangji",
				...
				"_ov_lingbaoxianhu", "_ov_taijifuchen", "_ov_chongyingshenfu", "_ov_tiaojiyanmei", "_ov_binglinchengxia",
				"_ov_mantianguohai",
			...
			for _, smy in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
				...
				if zmto then
					...
					if math.random() <= 0.5 then
						room:doLightbox("$f_zhengmie1")
					else
						room:doLightbox("$f_zhengmie2")
					end
					--
...
f_diyun = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		if math.random() <= 0.5 then
			room:doLightbox("$f_diyun1")
		else
			room:doLightbox("$f_diyun2")
		end
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseStart then
			if player:getPhase() == sgs.Player_Draw then
				...
				if math.random() <= 0.5 then
					room:doLightbox("$dy_shemi1")
				else
					room:doLightbox("$dy_shemi2")
				end
			end
		elseif event == sgs.DrawNCards then
		...
		elseif event == sgs.EventPhaseChanging then
			...
			if player:isKongcheng() and player:getEquips():length() > 0 and player:canDiscard(player, "e") then
				...
				local yangche = room:askForCardChosen(player, player, "e", self:objectName(), false,
					sgs.Card_MethodDiscard)
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardAsked then
			...
			if pattern ~= "jink" or string.find(prompt, "@f_skysson-jink")
				or player:getMark(self:objectName()) > 0 or not player:hasSkill("f_skysson") then
				return false
			end
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.MarkChanged then
			...
			if mark.name == self:objectName() and mark.gain > 0 and mark.who and mark.who:objectName() == player:objectName() then
				...
				room:setPlayerProperty(player, "SkillDescriptionRecord_f_danggu",
					sgs.QVariant(table.concat(ten_tojd, "+")))
				if player:getMaxHp() ~= mhp then room:setPlayerProperty(player, "maxhp", sgs.QVariant(mhp)) end
				if player:getHp() ~= hp then room:setPlayerProperty(player, "hp", sgs.QVariant(hp)) end
	...
	on_use = function(self, room, source, targets)
		...
		if #hl_qiaosi_cards > 0 then
			...
			elseif choice == "give" then
				local target = room:askForPlayerChosen(source, room:getOtherPlayers(source), "hl_qiaosi",
					"hl_qiaosi-invoke", false, true)
				local hl_qiaosi_give_cards = room:askForExchange(source, "hl_qiaosi", #hl_qiaosi_cards, #hl_qiaosi_cards,
					true, "hl_qiaosi_exchange")
...
mobileGOD_SkinChange_ButtonCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, player, targets)
		...
		if player:getGeneralName() == "f_shenguojia" or player:getGeneralName() == "f_shenguojia_c" or player:getGeneralName() == "f_shenguojia_joy"
			...
			if player:getMark("f_tianyi") > 0 then
				table.removeOne(choices, "sgj_yxzy_first")
			else
				table.removeOne(choices, "sgj_yxzy_second")
			end
			local choice = room:askForChoice(player, self:objectName(), table.concat(choices, "+"))
			if choice == "sjyh" then
		...
		if player:getGeneralName() == "ty_shenmachao" or player:getGeneralName() == "ty_shenmachao_1" or player:getGeneralName() == "ty_shenmachao_1dt"
			...
			local choice = room:askForChoice(player, self:objectName(),
				"sjyh+smc_xwjl+smc_xwjldt+smc_xyxc+smc_lwfc+smc_ymlt")
			if choice == "sjyh" then
