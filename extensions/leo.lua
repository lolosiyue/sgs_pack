							...
							if second:hasSkill(self:objectName()) then
								for i = 1, effect.card:getSubcards():length(), 1 do
									slash = room:askForCard(first, "slash",
										string.format("@xinwushuang-slash:%s:%s", second:objectName(),
											effect.card:getSubcards():length() - i), data, sgs.Card_MethodResponse,
										second);
									if slash == nil then
										break
									end
...
luafanfu_card = sgs.CreateSkillCard
	{
		...
		on_effect = function(self, effect)
			...
			if kdfrom ~= kdto then
				...
				local choice = nil
				local k = room:askForKingdom(from)
				if (from:getKingdom() ~= k) then
					room:setPlayerProperty(from, "kingdom", sgs.QVariant(k))
				elseif (to:getKingdom() ~= "qun") then
					room:setPlayerProperty(from, "kingdom", sgs.QVariant("qun"))
				else
					room:setPlayerProperty(from, "kingdom", sgs.QVariant("wei"))
				end
...
luayijuedestCard = sgs.CreateSkillCard {
	...
	on_validate_in_response = function(self, user)
		...
		if not guanyu:isAlive() or guanyu:isNude() then
			room:setPlayerFlag(player, "Global_luayijueFailed")
			return false
		end
		local aocaistring = self:getUserString()
