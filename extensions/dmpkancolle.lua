...
se_zhikongcard = sgs.CreateSkillCard{
	...
	room:throwCard(self, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_REMOVE_FROM_PILE, "", source:objectName(), self:objectName(), ""), nil)
	end
}
...
se_zhikong=sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data)
		...
		if event == sgs.EventPhaseChanging then
			...
			if change.to == sgs.Player_Start and not player:hasFlag("se_zhikong_on") and not isFire(player) then
			local playerdata = sgs.QVariant()
				playerdata:setValue(player)
				room:setTag("se_zhikong", playerdata)
				if Akagi:getHp() > 1 and Akagi:getPile("akagi_lv"):length() > 0  and room:askForUseCard(Akagi, "@@akagi_lv", "akagi_lv_remove:remove", -1, sgs.Card_MethodNone) then
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardsMoveOneTime then
			...
			if places:contains(sgs.Player_DrawPile) then
				...
				if num - move.card_ids:length() < 1 then
					...
					local target = room:askForPlayerChosen(kita, room:getOtherPlayers(kita), self:objectName(), "se_leimu-invoke", true, true)	
		...
		elseif event == sgs.GameStart then
			if player:hasSkill(self:objectName()) then
				local target = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "se_leimu-invoke", true, true)	
					if not target then return false end
					room:broadcastSkillInvoke(self:objectName())
					room:doLightbox("se_leimu$", 1200)
		...
		if target then
			if target:isAlive() and target:hasSkill(self:objectName()) then
				if target:getPhase() == sgs.Player_Start then
					if target:getMark("se_emeng") == 0 then
						return target:getHp() <= 2
					end
				end
			end
		end
...
se_qianlei = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		choicelist = string.format("%s+%s", choicelist, "se_qianlei_second")
		end
		if not buki:isNude()  then
		choicelist = string.format("%s+%s", choicelist, "se_qianlei_first")
		end
	...
	on_trigger = function(self, event, player, data) 
		...
		elseif event == sgs.EventPhaseEnd then
			if player:getPhase() == sgs.Player_Play then
				local buki = room:findPlayerBySkillName(self:objectName())
				if not buki then return end
				if buki and  buki:getPhase() == sgs.Player_Play  then return end
				if not buki:hasFlag("sonzaikan_aru") then
...
se_hongzha = sgs.CreateViewAsSkill{
	...
	enabled_at_play = function(self, player)
		return sgs.Slash_IsAvailable(player) and not sgs.Self:hasUsed("#se_hongzha") and sgs.Self:getHp() >= 2 and not sgs.Self:getPile("Kansaiki"):isEmpty() and not isFire(player) 
	end,
}
...
fanqianVS = sgs.CreateOneCardViewAsSkill{
	...
	view_filter = function(self,to_select)
		return not to_select:isKindOf("Collateral") and not to_select:isKindOf("Jink") and not to_select:isKindOf("Nullification") and not to_select:isKindOf("DelayedTrick")
	end, 
	view_as = function(self, originalCard) 
...
zheyi = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		local zuikaku = room:findPlayerBySkillName(self:objectName())
		local dying = data:toDying()
		if dying.who and dying.who:getMark("@EryuMark") > 0 and zuikaku and zuikaku:getMark("zheyi") == 0 and zuikaku:getMark("@EryuMark") > 0 then
		...
		if event == sgs.TurnStart then
			if player:hasSkill(self:objectName()) then
				...
				if not targets:isEmpty() then
					local target =  room:askForPlayerChosen(player, targets, self:objectName(), "youdiz-invoke",true, true)
					if target then
						room:broadcastSkillInvoke(self:objectName())
		...
		elseif event == sgs.DamageInflicted then
			...
			if damage.from and damage.from:getMark("@Youdi") > 0 then
				if damage.to and damage.to:hasSkill(self:objectName()) then
					local target =  room:askForPlayerChosen(damage.to, room:getOtherPlayers(damage.to), self:objectName(), "youdi_draw")
					target:drawCards(1)
				else
					local zuikaku = room:findPlayerBySkillName(self:objectName())
					local log= sgs.LogMessage()
...
nuequ = sgs.CreateViewAsSkill{
	...
	enabled_at_play = function(self, player)
		return not sgs.Self:hasUsed("#nuequ")
	end,
}
