...
moujizhuoo = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if phase == sgs.Player_Start and player:hasSkill(self:objectName()) then
			if room:askForSkillInvoke(player, self:objectName(), data) then
				local XLto = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "@XL-jizhuo")
				local choice = room:askForChoice(player, self:objectName(), "XL_tongchouu+XL_bingjinn+XL_shucaii+XL_lulii")
...
moulongdannEXEXCard = sgs.CreateSkillCard{
	...
	on_validate = function(self, card_use)
		...
		if self:getUserString() == "slash" and sgs.Sanguosha:getCurrentCardUseReason() == sgs.CardUseStruct_CARD_USE_REASON_RESPONSE_USE then
			...
			to_moulongdannEXEX = room:askForChoice(player, "moulongdannEXEX_slash", table.concat(moulongdannEXEX_list, "+"))
		end
		local card = nil
		if self:subcardsLength() == 1 then card = sgs.Sanguosha:cloneCard(sgs.Sanguosha:getCard(self:getSubcards():first())) end
		local user_str
	...
	on_validate_in_response = function(self, user)
		...
		if user_str == "peach+analeptic" then
			...
			to_moulongdannEXEX = room:askForChoice(user, "moulongdannEXEX_saveself", table.concat(moulongdannEXEX_list, "+"))
		elseif user_str == "slash" then
			local moulongdannEXEX_list = {}
			table.insert(moulongdannEXEX_list, "slash")
			table.insert(moulongdannEXEX_list, "fire_slash")
...
moujizhuooEX = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if phase == sgs.Player_Start and player:hasSkill(self:objectName()) then
			if room:askForSkillInvoke(player, self:objectName(), data) then
				local XLto = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "@XL-jizhuoEX")
				local choice = room:askForChoice(player, self:objectName(), "XL_tongchouu+XL_bingjinn+XL_shucaii+XL_lulii")
				if choice == "XL_tongchouu" then
					room:addPlayerMark(player, "&XL_tongchou")
					room:addPlayerMark(XLto, "&XL_tongchou")
...
mouxiejii = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseProceeding then
			...
			if phase == sgs.Player_Start and player:hasSkill(self:objectName()) then
				if room:askForSkillInvoke(player, self:objectName(), data) then
					local XLto = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "@XL-xieji")
					local choice = room:askForChoice(player, self:objectName(), "XL_tongchouu+XL_bingjinn+XL_shucaii+XL_lulii")
					if choice == "XL_tongchouu" then
						room:addPlayerMark(player, "&XL_tongchou")
						room:addPlayerMark(XLto, "&XL_tongchou")
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.GameStart then
			if player:hasSkill(self:objectName()) then
				local husband = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "@moujieyinn-start")
				...
				if p:getMark("&mouHusband") > 0 and not p:hasFlag("cantchoiceMJYnow") then
					...
					if choice == "1" then
						if p:getHandcardNum() > 2 then
							local caili = room:askForExchange(p, self:objectName(), 2, 2, false, "#moujieyinn:".. player:getGeneralName())
							if caili then
					...
					elseif choice == "2" then
						...
						if choicee == "4" then
							local Next = room:askForPlayerChosen(player, room:getOtherPlayers(p), self:objectName(), "@moujieyinn-markmove")
				...
				for _, p in sgs.qlist(mingxuaners) do
					...
					if not use_slash then
						if not p:isNude() then
							local card = room:askForExchange(p, self:objectName(), 1, 1, true, "#moumingxuann:".. player:getGeneralName())
							if card then
		...
		if event == sgs.Damaged then
			...
			if room:askForSkillInvoke(player, self:objectName(), data) then
				...
				if room:askForCard(machao, "..", "@mouxianchouu-slash", data, self:objectName()) then
					local slash = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)
					slash:setSkillName(self:objectName())
					if not machao:canSlash(wangyi, nil, false) then slash = nil
						return false
					end
		...
		if room:askForSkillInvoke(player, self:objectName(), data) then
			...
			if choice == "1" then
				...
				if choicee == "3" and not to:isKongcheng() then
					local baohufei = room:askForExchange(to, self:objectName(), 1, 1, false, "#moushipoo:".. player:getGeneralName())
					if baohufei then
						room:obtainCard(player, baohufei, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, player:objectName(), to:objectName(), self:objectName(), ""), false)
						if room:askForSkillInvoke(player, "@moushipoo-give", data) then
							local gt = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "moushipoo_givecards")
			...
			elseif choice == "2" then
				...
				for _, p in sgs.qlist(DiJun) do
					...
					if choicee == "3" and not p:isKongcheng() then
						local baohufei = room:askForExchange(p, self:objectName(), 1, 1, false, "#moushipoo:".. player:getGeneralName())
						if baohufei then
							room:obtainCard(player, baohufei, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, player:objectName(), p:objectName(), self:objectName(), ""), false)
...
mouxieyuann = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.Damaged then
			...
			else room:setPlayerMark(player, "mouxieyuann_hujiaCount", 0) end
		elseif event == sgs.RoundStart then
			for _, p in sgs.qlist(room:getAllPlayers()) do
...
moujieyuee = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if room:askForSkillInvoke(player, self:objectName(), data) then
			room:broadcastSkillInvoke(self:objectName())
			local tzs = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "moujieyuee-givehujia")
			tzs:gainHujia(1)
			room:drawCards(tzs, 2, self:objectName())
			local card = room:askForExchange(tzs, self:objectName(), 2, 2, true, "#moujieyuee:".. player:getGeneralName())
			if card then
				room:obtainCard(player, card, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, player:objectName(), tzs:objectName(), self:objectName(), ""), false)
			end
		end
	end,
}
...
fcchuifengCard = sgs.CreateSkillCard{
	...
	filter = function(self, targets, to_select)
	    local duel = sgs.Sanguosha:cloneCard("duel", sgs.Card_NoSuit, 0)
		return #targets == 0 and to_select:objectName() ~= sgs.Self:objectName() and not to_select:isProhibited(to_select, duel)
	end,
	on_effect = function(self, effect)
	    local room = effect.to:getRoom()
		local duel = sgs.Sanguosha:cloneCard("duel", sgs.Card_NoSuit, 0)
...
laobao_heaiC = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed then
			...
			or not player:hasFlag("mouliegongf_RMnaturelimit") then return false end
		...
		if event == sgs.CardUsed or event == sgs.CardResponded then
			...
			if card and card:getHandlingMethod() == sgs.Card_MethodUse and player:hasSkill(self:objectName()) then
				...
				elseif card:isBlack() then
					...
					if to then
						local card = room:askForCardChosen(player, to, "he", self:objectName(), false, sgs.Card_MethodDiscard)
			...
			if s >= m then
				if room:askForSkillInvoke(player, self:objectName(), data) then
					...
					for _, c in sgs.qlist(other:getHandcards()) do
						if c:getSuit() == sgs.Card_Heart and not player:hasFlag("mouqz_heart") then room:setPlayerFlag(player, "mouqz_heart") end
						if c:getSuit() == sgs.Card_Diamond and not player:hasFlag("mouqz_diamond") then room:setPlayerFlag(player, "mouqz_diamond") end
						if c:getSuit() == sgs.Card_Club and not player:hasFlag("mouqz_club") then room:setPlayerFlag(player, "mouqz_club") end
						if c:getSuit() == sgs.Card_Spade and not player:hasFlag("mouqz_spade") then room:setPlayerFlag(player, "mouqz_spade") end
					...
					if not to_throw:isEmpty() then
						...
						local reasonn = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_DISMANTLE, player:objectName(), nil, self:objectName(), nil)
	...
	on_trigger = function(self, event, player, data)
		...
		if effect.card:isKindOf("Dismantlement") and not effect.card:isVirtualCard() and effect.to:objectName() == player:objectName() then
			...
			if room:askForSkillInvoke(MGN, "mouqixii", data) then
				...
				local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_DISMANTLE, MGN:objectName(), nil, "mouqixii", nil)
...
mouqixirCard = sgs.CreateSkillCard{
	...
	target_fixed = false,
	filter = function(self, targets, to_select)
		return #targets == 0 and to_select:objectName() ~= sgs.Self:objectName() and not to_select:isAllNude() --对着区域没牌的人发动技能有卵用，明牌吗
	end,
	on_effect = function(self, effect)
	    local room = effect.to:getRoom()
		...
		while not effect.from:hasFlag("mouqixirEND") and n < 4 do
			...
			or (choice == "clubMAX" and effect.from:hasFlag("mqx_clubMAX")) or (choice == "spadeMAX" and effect.from:hasFlag("mqx_spadeMAX")) then --猜对
				...
				if room:askForUseCard(effect.from, "@@mouqixirAgain", "@mouqixirAgain") then --令其再次猜测
					if choice == "heartMAX" then table.removeOne(choices, "heartMAX")
					elseif choice == "diamondMAX" then table.removeOne(choices, "diamondMAX")
					elseif choice == "clubMAX" then table.removeOne(choices, "clubMAX")
					elseif choice == "spadeMAX" then table.removeOne(choices, "spadeMAX")
					end
				end
			end
		end
		if n > 0 then
			...
			while n > 0 and not effect.to:isAllNude() do
				local card = room:askForCardChosen(effect.from, effect.to, "hej", "mouqixir", false, sgs.Card_MethodDiscard)
...
moufenweirPUTCard = sgs.CreateSkillCard{ --将牌置于目标武将牌上，称为“威”
	...
	will_throw = false,
	filter = function(self, targets, to_select)
		return #targets == 0 and to_select:hasFlag("moufenweirTargets") and to_select:objectName() ~= sgs.Self:objectName()
	end,
	on_use = function(self, room, source, targets)
		local card_id = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)
		if card_id then
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getPhase() == sgs.Player_Play then
			if room:askForSkillInvoke(player, "mouyanyuu", data) then
				...
				local zhangfei = room:askForPlayerChosen(player, room:getOtherPlayers(player), "mouyanyuu", "mouyanyuu-GiveCardsNum:" .. n)
			...
			if choice == "judge" then
				...
				if not players:isEmpty() and n > 0 then
					...
					while n > 0 and not simawei:isNude() do
						local card = room:askForCardChosen(player, simawei, "he", "kjmoulijin", false, sgs.Card_MethodDiscard, sgs.IntList(), true)
						if card < 0 then
							n = 0
						else
...
moufanjiannCard = sgs.CreateSkillCard{
	...
	on_effect = function(self, effect)
		...
		if choice == "heart" then room:setPlayerFlag(effect.from, "moufanjiann_chooseHeart")
		elseif choice == "diamond" then room:setPlayerFlag(effect.from, "moufanjiann_chooseDiamond")
		elseif choice == "club" then room:setPlayerFlag(effect.from, "moufanjiann_chooseClub")
		elseif choice == "spade" then room:setPlayerFlag(effect.from, "moufanjiann_chooseSpade")
		end
		--------------------
		...
		if effect.from:hasFlag("moufanjiann_chooseDiamond") then room:setPlayerFlag(effect.from, "-moufanjiann_chooseDiamond") end
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.CardUsed or event == sgs.CardResponded then
			...
			if card and card:getHandlingMethod() == sgs.Card_MethodUse and not card:isKindOf("SkillCard") and player:hasSkill(self:objectName()) then
				if player:getMark("mouzhaxianggCardUsed") < player:getMark("&mouzhaxiangg") then room:broadcastSkillInvoke(self:objectName()) end
...
mourendeeCard = sgs.CreateSkillCard{
	...
	will_throw = false,
	filter = function(self, targets, to_select)
		return #targets == 0 and not to_select:hasFlag("mourendeeCardGet") and to_select:objectName() ~= sgs.Self:objectName()
	end,
	on_use = function(self, room, source, targets)
		local hxd = targets[1]
		local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, source:objectName(), targets[1]:objectName(), "mourendee", "")
			...
			for _, p in sgs.qlist(room:getAllPlayers()) do
				if p:getMark("&mourdTOzw") > 0 then
					...
					if p:getHandcardNum() + p:getEquips():length() > m then
						...
						if card then
							room:obtainCard(source, card, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, source:objectName(), p:objectName(), "mouzhangwuu", ""), false)
						end
						...
						local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_TRANSFER, source:objectName(), p:objectName(), "mouzhangwuu", nil)
...
moujijiangg = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.EventPhaseEnd and player:getPhase() == sgs.Player_Play and player:hasLordSkill(self:objectName()) then
			...
			local jjfrom
			if SJ:length() > 1 then jjfrom = room:askForPlayerChosen(player, SJ, self:objectName(), "moujijianggfrom")
		...
		elseif event == sgs.EventPhaseChanging then
			...
			if change.to == sgs.Player_Play and player:getMark("moujijianggSL") > 0 then
				if room:findPlayerBySkillName(self:objectName()) then room:sendCompulsoryTriggerLog(player, self:objectName()) end
				room:broadcastSkillInvoke(self:objectName())
		...
		if (player:getPhase() == sgs.Player_Start) or (player:getPhase() == sgs.Player_Finish) then
			if room:askForSkillInvoke(player, self:objectName(), data) then
				...
				if (suit == sgs.Card_Diamond) or (suit == sgs.Card_Heart) then
					room:broadcastSkillInvoke(self:objectName(), 2)
					local person = room:askForPlayerChosen(player, room:getAllPlayers(), self:objectName(), "fcmzj_jiezhuangshengod-ask", true, true)
					if person then
		...
		for _, p in sgs.qlist(room:getAllPlayers()) do
			if p:getMark("&mXuan") > 0 then
				if player:getMark("mouxuanhuoo-"..p:objectName().."-GCSource") >= 3 then
					...
					if n > 3 then
						local card = room:askForExchange(player, self:objectName(), 3, 3, true, "#mouenyuann:".. p:getGeneralName())
						if card then
						...
						local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_TRANSFER, p:objectName(), player:objectName(), "mouzhangwuu", nil)
...
moulianhuannCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room, source, targets)
		...
		if choice == "1" then
			...
			local card_id = room:askForCard(source, ".|club|.|hand", "@moulianhuann-recast", sgs.QVariant(), sgs.Card_MethodRecast)
			if card_id then
				room:moveCardTo(card_id, source, nil, sgs.Player_DiscardPile, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_RECAST, source:objectName(), "moulianhuann", ""))
				local log = sgs.LogMessage()
	...
	on_trigger = function(self, event, player, data)
					...
					if room:askForSkillInvoke(player, "moulianhuann", data) then
						...
						if player:isAlive() then
							for _, p in sgs.qlist(use.to) do
								...
								local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_NATURAL_ENTER, p:objectName(), "moulianhuann", "")
		...
		elseif event == sgs.PreCardUsed then
			...
			and use.card and use.card:isKindOf("IronChain") then
				...
				room:askForUseCard(player, "@@moulianhuanns", "@moulianhuanns-excard:" .. use.card:objectName() .. "::" .. 1, -1, sgs.Card_MethodNone)
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.PreCardUsed then
			...
			and use.card and use.card:isKindOf("Duel") then
				...
				room:askForUseCard(player, "@@moujiangd", "@moujiangd-excard:" .. use.card:objectName() .. "::" .. 1, -1, sgs.Card_MethodNone)
				if not player:isAlive() then return false end
				local adds = sgs.SPlayerList()
				for _, p in sgs.qlist(extra_targets) do
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.PreCardUsed then
			...
			and (use.card:isRed() and use.card:isKindOf("Slash")) or use.card:isKindOf("Duel") then
				...
				room:askForUseCard(player, "@@fcmoujiang", "@fcmoujiang-excard:" .. use.card:objectName() .. "::" .. 1, -1, sgs.Card_MethodNone)
			...
			if can_invoke then
				...
				if room:askForUseCard(player, "@@mouliulii", prompt, -1, sgs.Card_MethodDiscard) then
					...
					if player:hasFlag(self:objectName()) then
						...
						local sunce = room:askForPlayerChosen(player, players, self:objectName(), "mouliulii-extraEffect", true, true)
