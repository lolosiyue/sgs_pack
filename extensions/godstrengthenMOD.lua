...
GODsurroundingMaxCards = sgs.CreateMaxCardsSkill { --手牌上限-2
	...
	extra_func = function(self, target)
		...
		if target:getMark("GODlessMaxCards") > 0 then
			return -2
		else
			return 0
		end
...
voidRuleReverseCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		--1.0 人理烧却
		local va = room:askForPlayerChosen(source, room:getAllPlayers(), "voidRuleReverse", "voidRuleReverse-invokeA",
			true, true)
		if va then
		...
		--2.0 人理冻结
		local vb = room:askForPlayerChosen(source, room:getAllPlayers(), "voidRuleReverse", "voidRuleReverse-invokeB",
			true, true)
		if vb then
			vb:turnOver()
		...
		--2.1永久冻土帝国/2.2无间冰焰世纪
		local vc = room:askForPlayerChosen(source, room:getAllPlayers(), "voidRuleReverse", "voidRuleReverse-invokeC",
			true, true)
		if vc then
		...
		--2.3人智统合真国/2.4创世灭亡轮回
		local vd = room:askForPlayerChosen(source, room:getAllPlayers(), "voidRuleReverse", "voidRuleReverse-invokeD",
			true, true)
		if vd then
		...
		if from and from:getEquips():length() > 0 then
			local card_id = room:askForCardChosen(source, from, "e", "voidRuleReverse", false, sgs.Card_MethodNone,
				sgs.IntList(), true)
			if card_id >= 0 then
				...
				local to = room:askForPlayerChosen(source, tos, "voidRuleReverse",
					"@voidRuleReverse_Equip-to:" .. card:objectName())
				if to then
					room:moveCardTo(card, from, to, place,
						sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_TRANSFER, source:objectName(), "voidRuleReverse",
							""))
				end
		...
		if from and from:getJudgingArea():length() > 0 then
			local card_id = room:askForCardChosen(source, from, "j", "voidRuleReverse", false, sgs.Card_MethodNone,
				sgs.IntList(), true)
			if card_id >= 0 then
				local card = sgs.Sanguosha:getCard(card_id)
				local place = room:getCardPlace(card_id)
				...
				local to = room:askForPlayerChosen(source, tos, "voidRuleReverse",
					"@voidRuleReverse_Judge-to:" .. card:objectName())
				if to then
					room:moveCardTo(card, from, to, place,
						sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_TRANSFER, source:objectName(), "voidRuleReverse",
							""))
		...
		--星间都市山脉:击落神明之日
		local ve = room:askForPlayerChosen(source, room:getAllPlayers(), "voidRuleReverse", "voidRuleReverse-invokeF",
			true, true)
		if ve then
			local x = ve:getMaxHp()
			local y = ve:getHp()
		...
		----
		--2.6 妖精圆桌领域
		local vf = room:askForPlayerChosen(source, room:getAllPlayers(), "voidRuleReverse", "voidRuleReverse-invokeG1",
			true, true)
		if vf then
			local vg = room:askForPlayerChosen(source, room:getOtherPlayers(vf), "voidRuleReverse",
				"voidRuleReverse-invokeG2")
			room:swapSeat(vf, vg)
			room:loseMaxHp(source, 1)
		else
			room:setPlayerFlag(source, "voidRuleReverse_notdoAll")
		...
		--2.7 黄金树海纪行
		local vh = room:askForPlayerChosen(source, room:getAllPlayers(), "voidRuleReverse", "voidRuleReverse-invokeH",
			true, true)
		if vh then
			vh:throwEquipArea()
...
voidtongxin = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.EventPhaseStart then
			...
			if player:getMark("voidtongxinTurn") < 6 then
				room:addPlayerMark(player, "voidtongxinTurn")
			else
				return false
			end
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.DamageCaused then
			if damage.from:objectName() ~= player:objectName() or player:getMark("&AngelsRelics") == 0 or player:getMark("@voidtongxin_WSFY") == 0
				or damage.to:getHujia() == 0 or math.random() > 0.2 then
				return false
			end
...
voidtongxinTwo = sgs.CreateTriggerSkill {
	...
	can_trigger = function(self, player)
		return player and player:getMark("&AngelsRelics") > 0 and player:getMark("@voidtongxin_BJLv") > 0
	end,
}
	...
	on_trigger = function(self, event, player, data)
		...
		for _, p in sgs.qlist(use.to) do
			if not p:isNude() and player:canDiscard(p, "he") then
				local card = room:askForCardChosen(player, p, "he", self:objectName(), false, sgs.Card_MethodDiscard)
...
voidtongxinFour = sgs.CreateTriggerSkill {
	...
	can_trigger = function(self, player)
		return player and player:getMark("&AngelsRelics") > 0 and player:getMark("@voidtongxin_BJWL") > 0
	end,
}
...
do
	...
	f_MODon["MOD_AngelsRelics"] = function(player)
		...
		local MOD_AngelsRelics_b = { "yuanshu", "bgm_zhaoyun", "yuanshao", "yin_xuyou", "nos_zhangchunhua", "nos_madai",
			"nos_caochong", "nos_fuhuanghou",
			"tenyear_guanyu", "machao", "tenyear_sunquan", "tenyear_lvbu", "ol_zhangjiao", "ol_weiyan", "ol_xunyu",
			"ol_zuoci",
			...
			for _, p in sgs.qlist(room:getAllPlayers()) do
				...
				elseif mdbt:getMark("MOD_AngelsRelics_renli") == 4 or mdbt:getMark("MOD_AngelsRelics_boss") == 4 then --人理的守护者 A+
					...
					if mode == 6 or mode == 8 or mode == 10 then
						...
						local s_choices = {}
						if p:getState() ~= "robot" then table.insert(s_choices, "cancel") end
						local s = 5
						while s > 0 do
				...
				elseif mdbt:getMark("MOD_AngelsRelics_renli") == 5 or mdbt:getMark("MOD_AngelsRelics_boss") == 5 then --人理的守护者 EX
					...
					if mode == 4 then --room:getMode() == "04p" then
						local e_choices = {}
						if p:getState() ~= "robot" then table.insert(e_choices, "cancel") end
						local e = 6
						while e > 0 do
					...
					elseif mode == 6 or mode == 8 or mode == 10 then
						...
						local ex_choices = {}
						if p:getState() ~= "robot" then table.insert(ex_choices, "cancel") end
						local ex = 7
						while ex > 0 do
						...
						while ex < 2 do
							local ex_choice = room:askForChoice(p, "@MOD_AngelsRelics_renli",
								table.concat(ex_choices, "+"))
		...
		if can_invoke and player:getMark("MOD_AngelsRelics_dev") > 0 then --触发事件
			...
			local DEVs = { "dev_cheshen", "dev_hmqgg", "dev_jiaqi", "dev_funima", "dev_tak", "dev_zhangzheng", "dev_tan",
				"dev_lzx", "dev_fsu", "dev_rara",
				"dev_db", "dev_zy", "dev_jiaoshen", "dev_36li", "dev_para", "dev_yuanjiati", "dev_dudu", "dev_chongmei",
				"dev_yizhiyongheng", "dev_duguanhe",
	...
	MOD_AngelsRelics_xrnsBuff_s = sgs.CreateTriggerSkill { --伤害量与回复量+1
		...
		can_trigger = function(self, player)
			return player and (player:hasSkill("MOD_AngelsRelics_xrns_s") or player:hasSkill("MOD_AngelsRelics_xrns_ex"))
	...
	MOD_AngelsRelics_givebuffs = sgs.CreateTriggerSkill {
		...
		can_trigger = function(self, player)
			return player:isLord() and player:getGeneralName() == "devilDaybit" and
				player:getMark("MOD_AngelsRelics") > 0
...
dk_guanceCard = sgs.CreateSkillCard { --星夜
	...
	on_use = function(self, room, player, targets)
		...
		if z >= 1000 and z < 2000 then
			z = z - 1000
		elseif z >= 2000 and z < 3000 then
			z = z - 2000
		elseif z >= 3000 and z < 4000 then
			z = z - 3000
		elseif z >= 4000 and z < 5000 then
			z = z - 4000
		elseif z >= 5000 and z < 6000 then
			z = z - 5000
		elseif z >= 6000 and z < 7000 then
			z = z - 6000
		elseif z >= 7000 and z < 8000 then
			z = z - 7000
		elseif z >= 8000 and z < 9000 then
			z = z - 8000
		elseif z >= 9000 and z < 10000 then
			z = z - 9000
		end
		--观测值上百-->处理成百以下的数字
		if z >= 100 and z < 200 then
			z = z - 100
		elseif z >= 200 and z < 300 then
			z = z - 200
		elseif z >= 300 and z < 400 then
			z = z - 300
		elseif z >= 400 and z < 500 then
			z = z - 400
		elseif z >= 500 and z < 600 then
			z = z - 500
		elseif z >= 600 and z < 700 then
			z = z - 600
		elseif z >= 700 and z < 800 then
			z = z - 700
		elseif z >= 800 and z < 900 then
			z = z - 800
		elseif z >= 900 and z < 1000 then
			z = z - 900
		end
		--再提取出个位
		if z >= 10 and z < 20 then
			z = z - 10
		elseif z >= 20 and z < 30 then
			z = z - 20
		elseif z >= 30 and z < 40 then
			z = z - 30
		elseif z >= 40 and z < 50 then
			z = z - 40
		elseif z >= 50 and z < 60 then
			z = z - 50
		elseif z >= 60 and z < 70 then
			z = z - 60
		elseif z >= 70 and z < 80 then
			z = z - 70
		elseif z >= 80 and z < 90 then
			z = z - 80
		elseif z >= 90 and z < 100 then
			z = z - 90
		end
		-----
		if z == 7 then
