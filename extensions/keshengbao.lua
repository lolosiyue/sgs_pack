...
keshengganen = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if sunquan and (count > 0) and (sunquan:hasLordSkill(self:objectName())) and (player:objectName() ~= sunquan:objectName()) then
			if player:isYourFriend(sunquan) and (sunquan:getHp() < player:getHp()) then room:setPlayerFlag(player,"wantusekeshengganen") end
			if player:askForSkillInvoke(self:objectName()) then
				count = count - 1
				data:setValue(count)
				sunquan:drawCards(1)
			end
			room:setPlayerFlag(player,"-wantusekeshengganen")
		end
	end,
	can_trigger = function(self, player)
		return player
...
keshengqizuoclear = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		if event == sgs.EventPhaseStart then
			if (player:getPhase() == sgs.Player_RoundStart) and (player:getMark("&keshengqizuo")>0) then
				local room = player:getRoom()
...
kejieshengganen = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		local sunquan = room:findPlayerBySkillName(self:objectName())
		if sunquan and (count > 0) and (sunquan:hasLordSkill(self:objectName())) and (player:objectName() ~= sunquan:objectName()) then
			if player:isYourFriend(sunquan) and (player:getHp() > sunquan:getHp()) then room:setPlayerFlag(player,"wantusekejieshengganen") end
			if player:askForSkillInvoke(self:objectName()) then
				room:broadcastSkillInvoke(self:objectName())
				count = count - 1
				data:setValue(count)
			...
			room:setPlayerFlag(player,"-wantusekejieshengganen")
			--[[else
				if (sunquan:isYourFriend(player) and (player:isYourFriend(sunquan))) then
					count = count - 1
					data:setValue(count)
					local result = room:askForChoice(sunquan,self:objectName(),"huixue+mopai")
					if result == "huixue" then
		...
		if event == sgs.SlashMissed then
			...
			if effect.to:isAlive() and effect.from:hasSkill(self:objectName()) then
				if player:askForSkillInvoke(self:objectName(), data) then
					...
					if not effect.from:isYourFriend(effect.to) then room:setPlayerFlag(effect.from,"wantusekejieshenghuju") end
					...
					room:setPlayerFlag(effect.from,"-wantusekejieshenghuju")
				end
			end
		end
		if event == sgs.ConfirmDamage then
			local damage = data:toDamage()
			if damage.card and damage.card:isKindOf("Slash") and damage.from:hasFlag("kejieshenghuju") then
				local hurt = damage.damage
				damage.damage = hurt + 1
				room:setPlayerFlag(player,"-kejieshenghuju")
				data:setValue(damage)
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.TargetSpecified then
			...
			if use.card:isKindOf("Slash") then
				if player:askForSkillInvoke(self:objectName(), data) then
					local askforcardpattern = "."
					if not player:isYourFriend(use.to:at(0)) then room:setPlayerFlag(player,"wantusekejieshenghujutwo") end
					...
					room:setPlayerFlag(player,"-wantusekejieshenghujutwo")
				end
			end	
		end
		if event == sgs.ConfirmDamage then
			local damage = data:toDamage()
			if damage.card and damage.card:isKindOf("Slash") and (damage.card:getSkillName() == "kejieshenghujutwo") then
				local hurt = damage.damage
				damage.damage = hurt + 1
				data:setValue(damage)
			end
...
kejieshenghuixueCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room, player, targets)
		...
		if (player:getMark("&useshenghuixue") > 0) then
			local result = room:askForChoice(player,"kejieshenghuixue","shanghai")
			if result == "shanghai" then
		...
		if (player:getMark("&useshengshanghai") > 0) and not (player:hasFlag("useshenghuixue")) then
			local result = room:askForChoice(player,"kejieshenghuixue","huixue")
			if result == "huixue" then
		...
		room:setPlayerFlag(player,"-useshenghuixue")
		if player:isYourFriend(target) then room:setPlayerFlag(player,"huixuehuixue") end
		if (player:getMark("&useshengshanghai") == 0) and (player:getMark("&useshenghuixue") == 0) then
			local result = room:askForChoice(player,"kejieshenghuixue","huixue+shanghai")
			if result == "huixue" then
				room:setPlayerMark(player,"&useshenghuixue",1)
				if not (player:isKongcheng()) then
					if not room:askForDiscard(player, self:objectName(), 1, 1, false, false, "shenghuixue-discard") then
...
kejieshengqizuo = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
        ...
        for i = 0, damage.damage - 1, 1 do
			...
			if judge.card:isRed() then
				local one = room:askForPlayerChosen(player, room:getAllPlayers(), self:objectName(), "shengqizuoxuanpai-ask", true, true)
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.Damaged then
			...
			if damage.from and player:canDiscard(damage.from, "he") and (damage.from:objectName() ~= player:objectName()) then
				if not player:isYourFriend(damage.from) then room:setPlayerFlag(player,"wantusekejieshengliema") end
				...
				room:broadcastSkillInvoke(self:objectName())
				room:setPlayerFlag(player,"-wantusekejieshengliema")
			end
...
--界圣孙权
	...
	[":kejieshengxionglve"] = "<font color='green'><b>出牌阶段结束时，</b></font>你可以亮出牌堆顶的等同于你体力上限的牌并获得其中X张，然后将其余的牌放回牌堆顶。",
