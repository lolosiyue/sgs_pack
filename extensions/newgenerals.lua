...
enabled_at_play = function(self, player)
	...
	or (player:getMark("banchuifeng-Clear") > 0)) then return false end
...
kehuiyaoCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room, source, targets)
		...
		local other = room:askForPlayerChosen(source, room:getOtherPlayers(source), self:objectName(), "kehuiyao-ask", false, false)
		if other then
			local another = room:askForPlayerChosen(source, room:getOtherPlayers(other), self:objectName(), "kehuiyaotwo-ask", false, false)
			if another then
	...
	on_trigger = function(self, event, player, data)
		...
		if (event == sgs.EventPhaseStart) then
			...
			--先选择攻击范围
				...
				if (choice == "shanduanone") then
					room:setPlayerMark(player,"shanduanchooseone-Clear",1)
					room:setPlayerMark(player,"keolshanduancpgjfw-Clear",1+player:getMark("&keolshanduandamage-SelfClear"))
				elseif (choice == "shanduantwo") then
					room:setPlayerMark(player,"shanduanchoosetwo-Clear",1)
			...
			--再选择杀的次数
				...
				if (schoice == "shanduanone") then
					room:setPlayerMark(player,"shanduanchooseone-Clear",1)
					room:setPlayerMark(player,"keolshanduancpslash-Clear",1+player:getMark("&keolshanduandamage-SelfClear"))
	...
	on_trigger = function(self, event, player, data)
		...
		if (event == sgs.TargetConfirmed) then
			...
			and not player:isNude() then
				local askdis = room:askForDiscard(player, self:objectName(), player:getMaxHp(), 1, true, true, "keolhuanfudis")
				if askdis then
					room:broadcastSkillInvoke(self:objectName())
	...
	on_trigger = function(self, event, player, data)
		...
		if (event == sgs.EventPhaseChanging) then
			...
			if (change.to == sgs.Player_NotActive) then
				...
				if (player:getMark("diskeyijin_guxiong-SelfClear") > 0) then room:removePlayerMark(player,"@keyijin_guxiong") end
				if (player:getMark("diskeyijin_tongshen-SelfClear") > 0) then room:removePlayerMark(player,"@keyijin_tongshen") end
				if (player:getMark("diskeyijin_yongbi-SelfClear") > 0) then room:removePlayerMark(player,"@keyijin_yongbi") end
				if (player:getMark("diskeyijin_houren-SelfClear") > 0) then room:removePlayerMark(player,"@keyijin_houren") end
			end
		end
		--拥蔽:写在金迷里
...
kemobileguanzongCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room, player, targets)
		...
		local to = room:askForPlayerChosen(player, room:getOtherPlayers(target), self:objectName(), "kemobileguanzong-ask", false, false)
		if to then
			local log = sgs.LogMessage()
			log.type = "$kehuiyaolog"
			log.from = target
	...
	on_trigger = function(self, event, player, data)
		...
		if (event == sgs.EventPhaseEnd) and (player:getPhase() == sgs.Player_Play) then
			if (player:getMark("keoltongdu") > 0) then
				local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_DISMANTLE, player:objectName(), nil, "keoltongdu", nil)
				room:moveCardTo(sgs.Sanguosha:getCard(player:getMark("keoltongdu")), player, nil, sgs.Player_DrawPile, reason, true)
		...
		elseif event == sgs.EventPhaseEnd then
			if player:getPhase() ~= sgs.Player_NotActive and player:getPhase() ~= sgs.Player_RoundStart and player:getMark("keolzhuri_changed") > 0 and player:getMark("keolzhuri_lose") == 0 then
				...
				if targets:length() > 0 then
					...
					if target then room:broadcastSkillInvoke(self:objectName()) player:pindian(target, self:objectName()) end
				end
			end
					...
					if eny then
						...
						for _, p in sgs.qlist(room:getAlivePlayers()) do
							if p:objectName() ~= qh:objectName() and p:objectName() ~= eny:objectName() then
								room:doNotify(p, sgs.CommandType.S_COMMAND_EXCHANGE_KNOWN_CARDS, json.encode({qh:objectName(), eny:objectName()}))
							end
						end
						local exchangeMove = sgs.CardsMoveList()
...
keolbotu = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if (event == sgs.EventPhaseStart) and player:hasSkill(self:objectName()) then
			local num = player:getMark("@keolspade") + player:getMark("@keolclub") + player:getMark("@keolheart") + player:getMark("@keoldiamond")
...
keolshandaoex = sgs.CreateProhibitSkill{
	name = "#keolshandaoex",
	is_prohibited = function(self, from, to, card)
		return to and card and (card:getSkillName() == "keolshandao") and (((to:getMark("keolshandaowj") == 0) and card:isKindOf("ArcheryAttack")) or ((to:getMark("keolshandaowg") == 0) and card:isKindOf("AmazingGrace")))
	end
}
