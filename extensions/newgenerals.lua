...
Qizhengxiangsheng = sgs.CreateTrickCard {
	...
	on_effect = function(self, effect)
		...
		if not effect.no_respond then
			...
			card = room:askForCard(to, "Slash,Jink", "@_qizhengxiangsheng-card:", data, sgs.Card_MethodResponse, from,
				false, "", false, self)
		end
...
oldimengCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		local ok, error_msg = pcall(oldimeng_func, a, b)
		if not ok then
			a:setFlags("-OLDimengTarget")
			b:setFlags("-OLDimengTarget")
		end
...
local newgenerals_skills = { nhyinbing, nhhuoqi, nhguizhu, nhxianshou, nhlundao, nhguanyue, nhyanzheng, zuoxing, shenzhuo,
	shenzhuoSlash }
