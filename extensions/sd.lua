...
f_wuqin_MarkClear = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getMark("&f_wuqin_tiger") == 0 and player:getMark("&f_wuqin_deer") == 0 and player:getMark("&f_wuqin_bear") == 0
			and player:getMark("&f_wuqin_ape") == 0 and player:getMark("&f_wuqin_bird") == 0 then
			return false
		end
	...
	on_trigger = function(self, event, player, data)
		...
		if card then
			...
			room:obtainCard(player, card,
				sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, player:objectName(), damage.to:objectName(),
...
ks_YQWWCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		if choice == "YQWW_lastalive" then
			room:setPlayerFlag(source, "YQWW_lastalive")
		elseif choice == "YQWW_nextalive" then
...
ks_QNLXWCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		if source:getMark("&ks_QNLXW") > 0 then
			...
			local xiaoheizi = room:askForPlayerChosen(source, room:getAllPlayers(), "ks_QNLXW", "ks_QNLXW_xhzYBSBS:" .. t,
				true, true)
			if xiaoheizi then
	...
	on_effect = function(self, effect)
		...
		while not effect.from:hasFlag("CCYD_END") do
			...
			if sgs.Sanguosha:getCard(card):isRed() then
				judge.pattern = ".|red"
			elseif sgs.Sanguosha:getCard(card):isBlack() then
				judge.pattern = ".|black"
			end
			judge.good = true
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.DrawNCards then
			...
			for _, cxk in sgs.qlist(room:findPlayersBySkillName("f_ikun")) do
				...
				local lizhi = room:askForExchange(cxk, "f_ikun", count, count, true,
					"#f_ikunGiveCards:" .. player:getGeneralName())
				if lizhi then
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.DamageCaused then --判定蓝色妖姬
			...
			if damage.card and damage.card:isKindOf("Slash") and damage.from:objectName() == player:objectName()
				...
				room:setPlayerFlag(damage.to, "f_xingyuan_bluckTarget")
			end
...
f_huanxing = sgs.CreateZeroCardViewAsSkill {
	...
	enabled_at_play = function(self, player)
		return not player:hasUsed("#f_huanxingCard") and not player:isNude()
			and
			not (player:getMark("@CP_Red_unlock") == 0 and player:getMark("@CP_Bluck_unlock") == 0 and player:getMark("@CP_CrimsonTyphoon_unlock") == 0)
	end,
}
	...
	on_trigger = function(self, event, player, data)
				...
				for _, dz in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
					...
					if card then
						...
						if choice1 == "cancel" then
							room:setPlayerMark(dz, self:objectName(), 0)
							continue
						end
						--<基本牌>
						if choice1 == "Basic" then
							...
							if choice21 == "slash" then
								...
								room:setPlayerProperty(dz, "SkillDescriptionRecord_f_dingzheng",
									sgs.QVariant(table.concat(YYDZ, "+")))
							...
							elseif choice21 == "fire_slash" then
								...
								room:sendLog(log)
								local fire_slash = sgs.Sanguosha:cloneCard("fire_slash", use.card:getSuit(),
									use.card:getNumber())
							...
							elseif choice21 == "thunder_slash" then
								...
								room:sendLog(log)
								local thunder_slash = sgs.Sanguosha:cloneCard("thunder_slash", use.card:getSuit(),
									use.card:getNumber())
							...
							elseif choice21 == "analeptic" then
								...
								room:sendLog(log)
								local analeptic = sgs.Sanguosha:cloneCard("analeptic", use.card:getSuit(),
									use.card:getNumber())
							...
							elseif choice21 == "ice_slash" then
								...
								room:sendLog(log)
								local ice_slash = sgs.Sanguosha:cloneCard("ice_slash", use.card:getSuit(),
									use.card:getNumber())
						...
						elseif choice1 == "Trick" then
							...
							if choice22 == "Trick_BZ" then
								...
								if choice31 == "dismantlement" then
									...
									if not useto:isEmpty() then
										local dismantlement = sgs.Sanguosha:cloneCard("dismantlement", use.card:getSuit(),
											use.card:getNumber())
								...
								elseif choice31 == "snatch" then
									...
									if not useto:isEmpty() then
										local snatch = sgs.Sanguosha:cloneCard("snatch", use.card:getSuit(),
											use.card:getNumber())
								...
								elseif choice31 == "collateral" then
									...
									if can_jd then
										...
										room:askForUseCard(player, "@@f_dingzhengCollateral!",
											"@f_dingzhengCollateral-card")
										room:setPlayerFlag(player, "-f_dingzhengCollateral")
										room:setCardFlag(use.card, "-f_dingzhengCollateral")
									end
								...
								elseif choice31 == "ex_nihilo" then
									...
									room:sendLog(log)
									local ex_nihilo = sgs.Sanguosha:cloneCard("ex_nihilo", use.card:getSuit(),
										use.card:getNumber())
								...
								elseif choice31 == "savage_assault" then
									...
									room:sendLog(log)
									local savage_assault = sgs.Sanguosha:cloneCard("savage_assault", use.card:getSuit(),
										use.card:getNumber())
								...
								elseif choice31 == "archery_attack" then
									...
									room:sendLog(log)
									local archery_attack = sgs.Sanguosha:cloneCard("archery_attack", use.card:getSuit(),
										use.card:getNumber())
								...
								elseif choice31 == "amazing_grace" then
									...
									room:sendLog(log)
									local amazing_grace = sgs.Sanguosha:cloneCard("amazing_grace", use.card:getSuit(),
										use.card:getNumber())
								...
								elseif choice31 == "god_salvation" then
									...
									room:sendLog(log)
									local god_salvation = sgs.Sanguosha:cloneCard("god_salvation", use.card:getSuit(),
										use.card:getNumber())
							...
							elseif choice22 == "Trick_JY" then
								...
								if choice32 == "iron_chain" then
									...
									room:sendLog(log)
									local iron_chain = sgs.Sanguosha:cloneCard("iron_chain", use.card:getSuit(),
										use.card:getNumber())
								...
								elseif choice32 == "fire_attack" then
									...
									if not useto:isEmpty() then
										local fire_attack = sgs.Sanguosha:cloneCard("fire_attack", use.card:getSuit(),
											use.card:getNumber())
								...
								elseif choice32 == "drowning" then
									...
									room:sendLog(log)
									local drowning = sgs.Sanguosha:cloneCard("drowning", use.card:getSuit(),
										use.card:getNumber())
								...
								elseif choice32 == "dongzhuxianji" then
									...
									room:sendLog(log)
									local dongzhuxianji = sgs.Sanguosha:cloneCard("dongzhuxianji", use.card:getSuit(),
										use.card:getNumber())
								...
								elseif choice32 == "zhujinqiyuan" then
									...
									if not useto:isEmpty() then
										local zhujinqiyuan = sgs.Sanguosha:cloneCard("zhujinqiyuan", use.card:getSuit(),
											use.card:getNumber())
								...
								elseif choice32 == "chuqibuyi" then
									...
									if not useto:isEmpty() then
										local chuqibuyi = sgs.Sanguosha:cloneCard("chuqibuyi", use.card:getSuit(),
											use.card:getNumber())
						...
						elseif choice1 == "Equip" then
							...
							local choice23 = room:askForChoice(dz, self:objectName(),
								"Equip_Weapon+Equip_Armor+Equip_Defen+Equip_Offen+Equip_Treasure+cancel")
							if choice23 == "cancel" then
								room:setPlayerMark(dz, self:objectName(), 0)
								continue
							end
		...
		elseif event == sgs.CardFinished then --使用延时锦囊
			...
			elseif SSuf and SSut then
				local supply_shortage = sgs.Sanguosha:cloneCard("supply_shortage", use.card:getSuit(),
					use.card:getNumber())
	...
	on_use = function(self, room, source, targets)
		...
		for i = 0, 4 do
			if source:hasEquipArea(i) then
				...
				for _, p in sgs.qlist(room:getOtherPlayers(source)) do
					local zmb_card = nil
					if i == 0 then
						zmb_card = p:getWeapon()
					elseif i == 1 then
						zmb_card = p:getArmor()
					elseif i == 2 then
						zmb_card = p:getDefensiveHorse()
					elseif i == 3 then
						zmb_card = p:getOffensiveHorse()
					elseif i == 4 then
						zmb_card = p:getTreasure()
					end
		...
		for i = 0, 4 do
			if source:hasEquipArea(i) then
				...
				local wgyz_card = nil
				if i == 0 then
					wgyz_card = source:getWeapon()
				elseif i == 1 then
					wgyz_card = source:getArmor()
				elseif i == 2 then
					wgyz_card = source:getDefensiveHorse()
				elseif i == 3 then
					wgyz_card = source:getOffensiveHorse()
				elseif i == 4 then
					wgyz_card = source:getTreasure()
				end
...
f_lingyuan:addRelateSkill("f_lingyuangouCD")
if not sgs.Sanguosha:getSkill("f_lingyuangouCD") then skills:append(f_lingyuangouCD) end
		...
		if event == sgs.CardUsed then
			...
			if use.card:isKindOf("BasicCard") and use.from:objectName() == player:objectName() then
				if use.card:getSkillName() == "ollongdan" then
					...
					if me == 0 then
						local victim = room:askForPlayerChosen(player, room:getAllPlayers(), self:objectName(),
							"f_SuperChongzhen", true, true)
...
dg_linghann = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.TargetConfirming then
			...
			local names, name = player:property("SkillDescriptionRecord_dg_linghan"):toString():split("+"),
				use.card:objectName()
			if table.contains(names, name) then return false end
			table.insert(names, name)
			...
			local record, other, all, dg_linghan, tricks = sgs.IntList(), sgs.IntList(), sgs.IntList(),
				player:property("SkillDescriptionRecord_dg_linghan"):toString():split("+"), {}
			...
			if choice == "remove" then
				...
				room:setPlayerProperty(player, "SkillDescriptionRecord_dg_linghan",
					sgs.QVariant(table.concat(dg_linghan, "+")))
			...
			if use.card and use.card:isDamageCard() and not use.card:isKindOf("SkillCard") then
				...
				for _, t in sgs.qlist(use.to) do
					...
					if n > 0 and player:askForSkillInvoke(self:objectName(), _data) then
						...
						local discard_n = tonumber(room:askForChoice(player, self:objectName() .. "_num",
							table.concat(dis_num, "+"))) - 1
						local orig_places = sgs.PlaceList()
						local cards = sgs.IntList()
						t:setFlags("f_pojun_InTempMoving")
						for i = 0, discard_n do
							local id = room:askForCardChosen(player, t, "he", self:objectName() .. "_dis", false,
								sgs.Card_MethodNone)
			...
			for _, p in sgs.qlist(room:getAllPlayers()) do
				if not p:getPile("f_pojun"):isEmpty() then
					...
					room:obtainCard(p, dummy,
						sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_EXCHANGE_FROM_PILE, p:objectName(),
							self:objectName(), ""), false)
				end
		...
		if event == sgs.TargetSpecified then
			...
			if use.from and use.from:objectName() == player:objectName() and use.card and use.card:isKindOf("Slash") then
				if player:getMark("f_kaigong_d1") > 0 and player:getMark("f_kaigong_d2") > 0
					and player:getMark("f_kaigong_d3") > 0 and player:getMark("f_kaigong_d4") > 0 then
					return false
				end
				for _, p in sgs.qlist(use.to) do
					local up_damage = 0
		...
		elseif event == sgs.MarkChanged then
			...
			if mark.name == "&f_gonghun" and mark.gain > 0 and mark.who and mark.who:objectName() == player:objectName() then
				...
				if player:getMark("&f_gonghun") > 1 then
					...
					if choice == "1" then
						room:addPlayerMark(player, "f_kaigong_d1")
					elseif choice == "2" then
						room:addPlayerMark(player, "f_kaigong_d2")
					elseif choice == "3" then
						room:addPlayerMark(player, "f_kaigong_d3")
					elseif choice == "4" then
						room:addPlayerMark(player, "f_kaigong_d4")
					end
				end
			end
		elseif event == sgs.ConfirmDamage then
...
f_mouliegong = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.TargetConfirmed then
			local use = data:toCardUse()
			if not use.from or not use.from:hasSkill(self:objectName()) or player:objectName() ~= use.from:objectName() or (use.to:length() ~= 1 and not player:hasFlag("f_mouliegong_RMtargetlimit")) or not use.card:isKindOf("Slash") then return false end
			if room:askForSkillInvoke(player, self:objectName(), data) then
				room:broadcastSkillInvoke(self:objectName())
				local x = -1
			...
			if choice ~= "cancel" then
				for _, air in sgs.qlist(room:getAllPlayers(true)) do
					if air:isDead() and air:getGeneralName() == choice then --判断死亡的人的名字，跟选择的人是否符合，令其复活
						...
						if dc > 0 then
							room:drawCards(air, dc, "f_shanmengHS")
						end
						local mhp = source:getMaxHp()
						local hp = source:getHp()
						--代价：
	...
	enabled_at_play = function(self, player)
		...
		for _, lvr in sgs.qlist(player:getAliveSiblings()) do
			if (isSpecialOne(player, "神黄忠") and isSpecialOne(lvr, "神徐盛"))
				or (isSpecialOne(player, "神徐盛") and isSpecialOne(lvr, "神黄忠")) then
				return false
			end
		end
