	...
	about_to_use = function(self, room, use)
		...
		then
			...
			for i = 1, 2 do
				...
				local id = room:askForCardChosen(use.from, use.to:at(0), r, "ov_lingbao", false, sgs.Card_MethodDiscard,
					b, b:length() > 0)
	...
	on_use = function(self, room, source, targets)
		for c, to in sgs.list(targets) do
			...
			then
				...
				then
					room:broadcastSkillInvoke("ov_beini")
					continue
				end
			else
					...
					then
						...
						then
							if bit32.band(move.reason.m_reason, sgs.CardMoveReason_S_MASK_BASIC_REASON) == sgs.CardMoveReason_S_REASON_USE
								and sgs.Sanguosha:getCard(move.card_ids:at(fp)):isKindOf("EquipCard") then
								continue
							end
							n = n + 1
						end
		...
		for _, owner in sgs.list(room:findPlayersBySkillName(self:objectName())) do
			...
			then
				if use.from:objectName() ~= player:objectName()
					or player:getCardCount() < 1 then
					continue
				end
				local c = room:askForCard(player, "..", "ov_chenshi0:" .. owner:objectName(), ToData(owner),
					sgs.Card_MethodNone)
				...
				local c = room:askForCard(player, "..", "ov_chenshi1:" .. owner:objectName(), ToData(owner),
					sgs.Card_MethodNone)
				if not c then continue end
				ToSkillInvoke(self, player, owner, true)
				room:giveCard(player, owner, c, self:objectName())
	...
	on_trigger = function(self, event, player, data)
		...
		then
			...
			then
				...
				then
					use.card:setTag("ov_fengpo", ToData(x))
				else
					player:drawCards(x, "ov_fengpo")
				end
			end
		...
		then
			...
			then
				player:addMark("ov_fengpo_deathdamage")
			end
		else
			local damage = data:toDamage()
			if damage.card
	...
	on_trigger = function(self, event, player, data)
		...
		then
			...
			if damage and damage.from
				and damage.from:objectName() == player:objectName()
			then
				dying.who:setFlags("ov_moukui_dying")
			end
		else
			local use = data:toCardUse()
			if use.card:isKindOf("Slash")
			then
				for _, to in sgs.list(use.to) do
					...
					then
						...
						then
							local id = room:askForCardChosen(to, player, "he", "ov_moukui", false, sgs
								.Card_MethodDiscard)
	...
	on_use = function(self, room, source, targets)
		for _, to in sgs.list(targets) do
			...
			for c, p in sgs.list(tos) do
				...
				then
					c = room:askForCard(p, "..", "ov_mouzhu0:" .. source:objectName(), ToData(source),
						sgs.Card_MethodNone)
					if c then
						room:giveCard(p, source, c, "ov_mouzhu")
						n = n + 1
					end
				end
			end
			...
			then
				...
				then
					table.insert(tos, "mz_slash=" .. source:objectName() .. ":" .. n)
				end
				dc = dummyCard("duel")
				dc:setSkillName("_ov_mouzhu")
				if not source:isProhibited(to, dc)
				then
					table.insert(tos, "mz_duel=" .. source:objectName() .. ":" .. n)
				end
	...
	on_trigger = function(self, event, player, data)
		...
		then
			...
			for i, id in sgs.list(move.card_ids) do
				...
				then
					table.insert(i, "ov_yujue1")
				end
				if from:getMark("ov_yujue2-Clear") < 1
				then
					table.insert(i, "ov_yujue2")
				end
	...
	on_trigger = function(self, event, player, data)
		...
		for _, owner in sgs.list(room:findPlayersBySkillName(self:objectName())) do
			...
			then
				...
				then
					room:changeTranslation(player, "ov_yujuevs", 2)
		...
		for _, owner in sgs.list(room:findPlayersBySkillName(self:objectName())) do
			...
			then
				...
				then
					...
					if player:getCardCount() > 0
					then
						c = room:askForCard(player, "..", "ov_lingfa2:" .. owner:objectName(), ToData(owner),
							sgs.Card_MethodNone)
					end
					if c then
						room:giveCard(player, owner, c, "ov_lingfa")
					else
						room:damage(sgs.DamageStruct("ov_lingfa", owner, player))
					end
				end
		...
		for _, owner in sgs.list(room:findPlayersBySkillName(self:objectName())) do
			...
			then
				...
				then
					...
					then
						table.insert(choice, "ov_zhian1=" .. use.card:objectName())
					end
					...
					then
						...
						then
							room:throwCard(use.card, to, owner)
	...
	on_trigger = function(self, event, player, data)
		...
		then
			...
			local to = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(),
				"ov_budao0:" .. choice, true, true)
			if to
			then
				room:acquireSkill(to, choice)
				if to:getCardCount() < 1 then return end
				local c = room:askForCard(to, "..!", "ov_budao1:" .. player:objectName(), ToData(player),
					sgs.Card_MethodNone)
		...
		then
			...
			then
				...
				if use.card:isVirtualCard() then
					...
					fs:addSubcard(use.card:getEffectiveId())
				end
				use.card = fs
				local tos = sgs.SPlayerList()
		...
		then
			...
			then
				...
				then
					...
					if tos
					then
						local c = room:askForExchange(player, "ov_chunlaobf", 1, 1, true,
							"ov_chunlaobf1:" .. tos:objectName())
						if c
						then
				...
				for i, id in sgs.list(move.card_ids) do
					...
					then
						...
						then
							...
							then
								room:damage(sgs.DamageStruct("ov_ejian", player, to))
							else
...
ov_shanli = sgs.CreateTriggerSkill {
	...
	can_trigger = function(self, target)
		return target and target:hasSkill(self) and target:getMark(self:objectName()) < 1 and
			target:getPhase() == sgs.Player_Start
			and
			(target:getTag("BaiyiUsed"):toBool() and #target:getTag("JinglveTargets"):toString():split("+") > 1 or target:canWake(self:objectName()))
	end,
	on_trigger = function(self, event, player, data)
		local room = player:getRoom()
		if event == sgs.EventPhaseProceeding
...
ov_fuman = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		for i, owner in sgs.list(room:findPlayersBySkillName("ov_fuman")) do
			...
			if event == sgs.CardResponded
			then
				i = data:toCardResponse().m_card
			else
				i = data:toCardUse().card
			end
...
ov_feifu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		local use = data:toCardUse()
		if use.card:isKindOf("Slash")
			and use.to:length() == 1 then else
			return
		end
		local n = room:getChangeSkillState(player, self:objectName())
		if event == sgs.TargetSpecified then
			if n < 2 then return end
		elseif use.to:contains(player) and n < 2 then else
			return
		end
		...
		local c = room:askForCard(to, "..!", "ov_feifu0:" .. use.from:objectName(), ToData(use.from), sgs
			.Card_MethodNone)
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			...
			then
				...
				then
					player:addMark("ov_fenghan-Clear")
				end
			end
		end
		return false
	...
	on_trigger = function(self, event, player, data)
		...
		then
			...
			then
				...
				local to = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "ov_congji0:",
					true, true)
				if to then
					room:broadcastSkillInvoke(self:objectName())
					room:obtainCard(to, d)
				end
			end
		end
		return false
...
ov_chuhai = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			...
			then
				dying.who:setTag("ov_chuhai" .. player:objectName(), ToData(true))
			end
			...
			then
				...
				local c = room:askForCard(player, table.concat(ids, ",") .. "!", "ov_chuhai0:", data, sgs
					.Card_MethodNone)
	...
	on_trigger = function(self, event, player, data)
		...
		then
			...
			for c, to in sgs.list(room:getOtherPlayers(player)) do
				...
				c = room:askForCard(to, "..!", "ov_shanghe0:" .. player:objectName(), ToData(player), sgs
					.Card_MethodNone)
				if c
				then
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			...
			if use.card:getSkillName() ~= "ov_sidai"
				and not use.card:hasFlag("ov_sidai") then
				return
			end
		...
		then
			...
			for c, to in sgs.list(use.to) do
				...
				then else
					table.insert(nsl, to:objectName())
				end
			end
...
ov_liewei = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			...
			then
				choice = "ov_liewei1+ov_liewei2"
			end
			if room:askForChoice(player, "ov_liewei", choice, data) ~= "ov_liewei1"
			then
				room:addPlayerMark(player, "@ov_cuorui")
			else
...
ov_zhuiduCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		for i, to in sgs.list(targets) do
			...
			then
				table.insert(i, "beishui_choice=ov_zhuidu3")
			end
			i = table.concat(i, "+")
...
ov_shigong = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		then
			...
			then
				table.insert(dying, "ov_shigong2=" .. player:objectName())
			end
...
ov_shuangren = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			to = room:askForPlayerChosen(player, tos, "ov_shuangren", "ov_shuangren0:", event == sgs.EventPhaseStart,
				true)
		end
		if not to then return end
		...
		then
			...
			to_s = room:askForPlayersChosen(player, to_s, "ov_shuangren", 0, 2, "ov_shuangren1:" .. to:objectName(), true,
				true)
...
ov_xiafengbf = sgs.CreateMaxCardsSkill {
	...
	extra_func = function(self, target)
		...
		then
			return target:getMark("&ov_xiafeng-Clear")
		end
	end
}
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			...
			then
				...
				then
					table.insert(choice, "beishui_choice=ov_liechi3")
				end
				choice = table.concat(choice, "+")
				...
				then
					...
					then
						...
						if choice then
							room:throwCard(choice:at(0), player)
						else
							return
						end
					end
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			...
			then
				...
				local choice = "ov_fenming1=" ..
					to:objectName() .. "+ov_fenming2=" .. to:objectName() .. "+beishui_choice=ov_fenming3"
...
ov_xingqi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		if player:getTag("ov_mibei_shiming"):toBool()
		then
			room:addPlayerMark(player, "ov_xingqibf")
		else
			local dc = dummyCard()
			for c, id in sgs.list(room:getDrawPile()) do
				c = sgs.Sanguosha:getCard(id)
		...
		then
			...
			for _, ap in sgs.list(room:getAlivePlayers()) do
				...
				then
					...
					elseif move:targetFixed() then
						room:useCard(sgs.CardUseStruct(move, ap, sgs.SPlayerList()))
					else
		...
		then
			...
			then
				...
				if n == "ov_chayi1" then
					...
					room:addPlayerMark(to, "ov_chayidebf")
				end
		...
		then
			...
			then
				...
				then
					...
					then
						room:addPlayerMark(player, "ov_chayidebf")
					else
...
ov_fubiCard = sgs.CreateSkillCard {
	...
	on_use = function(self, room, source, targets)
		...
		if self:subcardsLength() > 0
		then
			room:addPlayerMark(targets[1], "ov_fubibf")
		else
			for i = 1, 4 do
		...
		then
			...
			for i = 1, damage.damage do
				...
				then
					...
					if i
					then
						i = room:askForExchange(damage.from, "ov_enyuan", 1, 1, false, "ov_enyuan0:" ..
							player:objectName())
					end
					if i
					then
		...
		then
			...
			then
				...
				then
					...
					then
						room:recover(from, sgs.RecoverStruct(player))
					else
						from:drawCards(1, "ov_enyuan")
					end
				end
				player:setFlags("-CardsMoveOneTime")
			end
		...
		for i, target in sgs.list(targets) do
			...
			then
				...
				for n = 1, 2 do
					...
					n = room:askForCardChosen(source, target, "he", "ov_xuanhuo", false, sgs.Card_MethodNone,
						i:getSubcards())
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			...
			then
				...
				tos = room:askForPlayerChosen(player, tos, "ov_zenhui", "ov_zenhui0:" .. use.card:objectName(), true,
					true)
				if not tos then return end
				...
				then
					...
					then
						...
						tos = room:askForPlayerChosen(player, tos, "ov_qirang1",
							"ov_qirang1:" .. can:objectName() .. ":ov_zenhui", true)
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			...
			then
				...
				then
					room:addPlayerMark(player, "ov_zhilvedebf-Clear")
				else
					room:loseHp(player)
				end
			else
			...
			then
				...
				then
					...
					then
						...
						local c = room:askForExchange(player, "ov_zhengjian", 1, 1, true,
							"ov_zhengjian0:" .. owner:objectName())
		...
		then
			...
			then
				for _, owner in sgs.list(room:findPlayersBySkillName("ov_bingzhao")) do
					...
					then
						owner:setTag("ov_bingzhao", ToData(damage.to))
					end
				end
			end
		elseif event == sgs.GameStart
		...
		then
			...
			for c, owner in sgs.list(room:getAllPlayers(true)) do
				if owner:isDead() and owner:objectName() ~= death.who:objectName()
					or not owner:hasLordSkill(self) then
					continue
				end
				if death.damage
...
ov_dingzhenbf = sgs.CreateProhibitSkill {
	...
	is_prohibited = function(self, from, to, card)
		...
		then
			self:setObjectName("ov_dingzhen")
			return true
		end
	end
}
	...
	on_trigger = function(self, event, player, data)
		...
		local function ov_youyePREVIEW()
			...
			local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, player:objectName(), self:objectName(),
				nil)
			room:askForYiji(player, xu, self:objectName(), true, true, false, xu:length(), sp, reason,
				"ov_youye0:" .. sp:at(0):objectName())
		...
		then
			...
			then
				player:addMark(damage.to:objectName() .. "ov_youye-Clear")
			end
	...
	on_trigger = function(self, event, player, data)
		...
		then
			...
			SetShifa("ov_xingzhui", player).effect = function(owner, x)
				...
				local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_TURNOVER, owner:objectName(), "ov_xingzhui",
					nil)
				...
				then
					...
					local to = room:askForPlayerChosen(owner, room:getOtherPlayers(owner), "ov_xingzhui", "ov_xingzhui0:",
						true, true)
					room:clearAG(owner)
					if to
					then
...
ov_jichouCard = sgs.CreateSkillCard {
	...
	about_to_use = function(self, room, use)
		...
		then
			table.insert(choice, "ov_jichou-Clear")
		end
		local d = dummyCard()
		...
		then
			table.insert(choice, "ov_jichoucard-PlayClear")
		end
		choice = table.concat(choice, "+")
...
ov_jichouvs = sgs.CreateViewAsSkill {
	...
	enabled_at_response = function(self, player, pattern)
		...
		for c, p in sgs.list(pattern:split("+")) do
			...
			then
				return player:getMark("ov_jichou-Clear") < 1
			end
		end
	end,
	enabled_at_nullification = function(self, player)
					...
					then
						...
						then
							...
							then
								...
								tos = room:askForPlayerChosen(player, tos, "ov_qirang1",
									"ov_qirang1:" .. can:objectName() .. ":ov_qirang", true)
								if tos then can:setTag("collateralVictim", ToData(tos)) else return end
							end
							use.to:append(can)
							room:sortByActionOrder(use.to)
		...
		then
			...
			then
				...
				then
					...
					then
						player:drawCardsList(n, "ov_yuhua")
					end
				end
			end
		elseif event == sgs.EventPhaseChanging
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			...
			then
				...
				then
					room:setPlayerMark(effect.to, "&ov_xuhe+#" .. player:objectName() .. "-Clear", 1)
				else
					room:damage(sgs.DamageStruct("ov_xuhe", player, effect.to))
				end
		...
		then
			...
			then
				...
				for _, p in sgs.list(room:getAlivePlayers()) do
					...
					then
						table.insert(no_respond, p:objectName())
					end
...
ov_fupan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data)
		...
		then
			if event == sgs.Damaged
			then
				room:broadcastSkillInvoke(self, 1)
			else
				room:broadcastSkillInvoke(self, 2)
			end
			local damage = data:toDamage()
	...
	on_use = function(self, room, source, targets)
		...
		for c, to in sgs.list(targets) do
			...
			for i = 1, n do
				...
				if c then
					room:giveCard(to, to1, c, "ov_mutao", true)
				else
					for _, h in sgs.list(to:getHandcards()) do
						if h:isKindOf("Slash") then
							room:giveCard(to, to1, h, "ov_mutao", true)
							break
						end
					end
				end
	...
	on_trigger = function(self, event, player, data)
		...
		then
			...
			for i, owner in sgs.list(room:findPlayersBySkillName(self:objectName())) do
				...
				then
					table.insert(i, "ov_yimou2")
				end
				if #i > 1 and owner:getHandcardNum() > 0
					and player:objectName() ~= owner:objectName()
				then
					table.insert(i, "beishui_choice=ov_yimou3")
				end
				i = table.concat(i, "+")
...
ov_tanfeng = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			...
			then
				table.insert(sp, "ov_tanfeng2=" .. player:objectName())
			end
...
ov_xiaweivs = sgs.CreateViewAsSkill {
	...
	enabled_at_response = function(self, player, pattern)
		if string.sub(pattern, 1, 1) == "." or string.sub(pattern, 1, 1) == "@"
			or player:getPile("&ov_wei"):isEmpty() then
			return
		end
		for c, p in sgs.list(pattern:split("+")) do
			c = dummyCard(p)
		...
		then
			...
			for i, owner in sgs.list(room:findPlayersBySkillName("ov_jianwei")) do
				...
				then
					...
					if pindian.from_number > 13 then pindian.from_number = 13 end
					Log_message("$ov_niju10", owner, nil, pindian.from_card:getEffectiveId(), "+" .. n,
						pindian.from_number)
					data:setValue(pindian)
				end
				...
				for _, owner in sgs.list(room:findPlayersBySkillName("ov_jianwei")) do
					...
					then
						...
						then
							...
							while pindian.from:getCards("hej"):length() > b:length() do
								...
								local id = room:askForCardChosen(owner, pindian.from, flags, "ov_jianwei", false,
									sgs.Card_MethodNone, b)
					...
					then
						...
						while pindian.to:getCards("hej"):length() > b:length() do
							owner:setTag("askForCardChosen_ForAI", ToData(b))
							local id = room:askForCardChosen(owner, pindian.to, flags, "ov_jianwei", false,
								sgs.Card_MethodNone, b)
							owner:removeTag("askForCardChosen_ForAI")
							if id < 0 then break end
							local cp = room:getCardPlace(id)
...
ov_shengxi = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.Damage
			and player:getPhase() ~= sgs.Player_NotActive
		then
			player:addMark("ov_shengxiDamage-Clear")
		elseif event == sgs.CardFinished
			and player:getPhase() ~= sgs.Player_NotActive
		then
			local use = data:toCardUse()
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			...
			then
				...
				room:fillAG(ids, player)
				local to = room:askForPlayerChosen(player, room:getOtherPlayers(player), "ov_kuanji", "ov_kuanji0:", true,
					true)
				room:clearAG(player)
				if not to then return end
...
ov_tiaojiyanmei = sgs.CreateTrickCard {
	...
	about_to_use = function(self, room, use)
		if use.to:length() < 2
		then
			UseCardRecast(use.from, self, "ov_tiaojiyanmei")
		else
			self:cardOnUse(room, use)
		end
	end,
	on_use = function(self, room, source, targets)
		...
		for i, to in sgs.list(targets) do
			...
			effect.nullified = table.contains(nullified_list, "_ALL_TARGETS") or
				table.contains(nullified_list, to:objectName())
					...
					then
						...
						then
							...
							then
								...
								tos = room:askForPlayerChosen(player, tos, "ov_qirang1",
									"ov_qirang1:" .. i:objectName() .. ":ov_yizhu", true)
	...
	on_use = function(self, room, source, targets)
		for i, to in sgs.list(room:getAlivePlayers()) do
			if to:getMark("ov_luanchou_" .. source:objectName()) < 1
				or table.contains(targets, to) then
				continue
			end
			...
			for i, owner in sgs.list(room:findPlayersBySkillName(self:objectName())) do
				...
				then
					...
					then
						...
						then
							room:setPlayerFlag(i, "Global_PlayPhaseTerminated")
						else
							i:setTag("FinishPhase", ToData(i:getPhase()))
						end
					end
				end
			end
		...
		then
			...
			for i, owner in sgs.list(room:findPlayersBySkillName("ov_chuanshu")) do
				...
				then
					...
					if pindian.from_number > 13 then pindian.from_number = 13 end
					Log_message("$ov_niju10", pindian.from, nil, pindian.from_card:getEffectiveId(), "+3",
						pindian.from_number)
					data:setValue(pindian)
				end
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			...
			for i, owner in sgs.list(room:findPlayersBySkillName("ov_lvren")) do
				...
				then
					...
					if pindian.from_number > 13 then pindian.from_number = 13 end
					Log_message("$ov_niju10", pindian.from, nil, pindian.from_card:getEffectiveId(), "+" .. i,
						pindian.from_number)
					data:setValue(pindian)
				...
				then
					...
					if pindian.to_number > 13 then pindian.to_number = 13 end
					Log_message("$ov_niju10", pindian.to, nil, pindian.to_card:getEffectiveId(), "+" .. i,
						pindian.to_number)
					data:setValue(pindian)
				end
			end
		...
		then
			...
			then
				...
				local to = room:askForPlayerChosen(player, sp, "ov_lvren", "ov_lvren0:" .. use.card:objectName(), true,
					true)
				if not to then return end
				room:broadcastSkillInvoke(self:objectName())
				use.to:append(to)
...
ov_shelie = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		elseif event == sgs.RoundStart
		then
			player:removeMark("ov_shelieUse")
		elseif event == sgs.EventPhaseChanging
		then
			local change = data:toPhaseChange()
			if change.to == sgs.Player_NotActive
	...
	on_use = function(self, room, source, targets)
		for c, to in sgs.list(targets) do
			...
			then
				...
				if c == "ov_gongxin1"
				then
					room:moveCardsInToDrawpile(source, id, "ov_gongxin", 1, true)
				else
					room:throwCard(id, to, source)
				end
	...
	on_trigger = function(self, event, player, data, room)
		for i, owner in sgs.list(room:findPlayersBySkillName("ov_suizheng")) do
			...
			then
				...
				i = room:askForDiscard(owner, "ov_suizheng", 2, 2, true, true, "ov_suizheng1:" .. player:objectName(),
					"BasicCard")
				if i
				then
		...
		then
			...
			then
				for i, to in sgs.list(use.to) do
					...
					then
						Skill_msg(self, player)
						i = room:askForExchange(player, "ov_gongge", i, i, true, "ov_gongge01:" ..
							i .. ":" .. to:objectName())
		...
		then
			...
			then
				...
				then
					...
					local can = player:getMark("ov_jingceDRAW-PlayClear") > 0 or
						player:getMark("ov_jingceDamage-Clear") > 0
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			...
			then
				...
				if change.to == 2 then
					to_phase = "Player_Judge"
				elseif change.to == 3 then
					to_phase = "Player_Draw"
				elseif change.to == 4 then
					to_phase = "Player_Play"
				elseif change.to == 5 then
					to_phase = "Player_Discard"
				elseif change.to == 6 then
					to_phase = "Player_Finish"
				end
	...
	on_use = function(self, room, source, targets)
		for _, to in sgs.list(targets) do
			...
			then
				...
				room:damage(sgs.DamageStruct("ov_liexi", to, source))
			end
			for c, id in sgs.list(self:getSubcards()) do
				c = sgs.Sanguosha:getCard(id)
				if c:isKindOf("Weapon")
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			...
			then
				...
				tos = room:askForPlayersChosen(player, room:getOtherPlayers(player), "ov_shezhong", 0, tos,
					"ov_shezhong0:" .. tos, true)
		...
		then
			...
			if damage.to:objectName() ~= player:objectName()
			then
				player:addMark("ov_shezhongUse-Clear")
			end
		elseif event == sgs.Damaged
		then
			local damage = data:toDamage()
...
ov_kaizengCard = sgs.CreateSkillCard {
	...
	about_to_use = function(self, room, use)
		...
		local dc = room:askForExchange(ls, "ov_kaizeng", ls:getCardCount(), 1, true,
			"ov_kaizeng0:" .. use.from:objectName() .. ":" .. msg, true)
		if dc
		then
...
ov_zhiqu = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		if event == sgs.EventPhaseProceeding
			and player:getPhase() == sgs.Player_Finish
		then
			local to = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "ov_zhiqu0:",
				true, true)
			if not to then return end
			local n = 0
			for i, p in sgs.list(room:getAlivePlayers()) do
			...
			while cs:length() > 0 do
				...
				then
					if n:targetFixed()
					then
						room:useCard(sgs.CardUseStruct(n, player, sgs.SPlayerList()))
					else
						room:useCard(sgs.CardUseStruct(n, player, to))
					end
				else
					room:throwCard(n, nil)
				end
				room:setCardFlag(n, "-ov_zhiqubf")
				cs:removeOne(n)
			end
...
ov_xinghan = sgs.CreateTriggerSkill {
	...
	on_trigger = function(self, event, player, data, room)
		...
		then
			if player:getPhase() ~= sgs.Player_Start
				or player:getPile("ov_xiayi"):length() <= player:aliveCount() then
				return
			end
