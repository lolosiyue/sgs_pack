...
xj_tonewlonghun = sgs.CreateTriggerSkill{
	...
	can_trigger = function(self, player)
		return (player:getGeneralName() == "xj_gaodayihao" or player:getGeneral2Name() == "xj_gaodayihao") and player:hasSkill("xj_longhun")
	end,
}
----
		...
		elseif event == sgs.Damaged then
			...
			for _, c in sgs.qlist(room:getAllPlayers()) do
				...
				if c:getPile(self:objectName()):length() > 0 then
					...
					local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_PUT, c:objectName(), nil, self:objectName(), nil)
...
xj_dunshi = sgs.CreateZeroCardViewAsSkill{
	...
	enabled_at_play = function(self, player)
		...
		and not (player:getMark("xj_dunshiCantUseSlash") > 0 and player:getMark("xj_dunshiCantUsePeach") > 0 and player:getMark("xj_dunshiCantUseAnaleptic") > 0)
	end,
	enabled_at_response = function(self, player, pattern)
		if player:hasFlag("xj_dunshiUsed") then return false end
		return (pattern == "slash" and player:getMark("xj_dunshiCantUseSlash") == 0)
		or (string.find(pattern, "peach") and (player:getMark("Global_PreventPeach") == 0 and player:getMark("xj_dunshiCantUsePeach") == 0) or player:getMark("xj_dunshiCantUseAnaleptic") == 0)
...
wmzgl_SkinChange = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getGeneralName() == "wumiao_zhugeliang" and room:askForSkillInvoke(player, self:objectName(), data) then
			...
			if not player:hasSkill("wmzgl_SkinChange_Button") then room:attachSkillToPlayer(player, "wmzgl_SkinChange_Button") end
		end
...
wmzgl_hideskin = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		and not table.contains(sgs.Sanguosha:getBanPackages(), "wmzgl_better") then
			...
			if player:getMaxHp() ~= mhp then room:setPlayerProperty(player, "maxhp", sgs.QVariant(mhp)) end
			if player:getHp() ~= hp then room:setPlayerProperty(player, "hp", sgs.QVariant(hp)) end
			if player:hasSkill("wmzgl_SkinChange_Button") then room:detachSkillFromPlayer(player, "wmzgl_SkinChange_Button", true) end
		end
	end,
	can_trigger = function(self, player)
...
WmChushibiaos = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getPhase() == sgs.Player_Start then
			if room:askForSkillInvoke(player, self:objectName(), data) then
				...
				local n = player:getHp() if n > 6 then n = 6 end
				...
				while not to_use:isEmpty() do
					...
					if uscard:isKindOf("Jink") or uscard:isKindOf("Nullification") or uscard:isKindOf("JlWuxiesy") then --不能主动使用的直接丢入弃牌堆
						...
						if #pattern > 0 then
							...
							local csb = room:askForUseCard(player, table.concat(pattern, ","), "@WmChushibiao_use:"..uscard:objectName(), -1)
							if csb then
								if not player:hasFlag("WmChushibiao_anaNoLimit") then room:setPlayerFlag(player, "WmChushibiao_anaNoLimit") end --专门处理【酒】的不计次
							else
								room:throwCard(uscard, nil)
							end
							room:setPlayerFlag(player, "-WmChushibiao_useNoLimit")
	...
	can_trigger = function(self, player)
		return player:getWeapon():isKindOf("WmChushibiao") and player:getMark("WmChushibiao") > 0
	end,
}
				...
				if room:askForSkillInvoke(player, self:objectName(), data) then
					...
					if suit == sgs.Card_NoSuit then --随机阵
						...
						if num >= 8 and num <= 13 then
							if suit == sgs.Card_Heart then room:setPlayerMark(player, "wmBZT_zf", 1)
							elseif suit == sgs.Card_Diamond then room:setPlayerMark(player, "wmBZT_zf", 2)
							elseif suit == sgs.Card_Club then room:setPlayerMark(player, "wmBZT_zf", 3)
							elseif suit == sgs.Card_Spade then room:setPlayerMark(player, "wmBZT_zf", 4) end
						elseif num >= 1 and num <= 7 then
							if suit == sgs.Card_Heart then room:setPlayerMark(player, "wmBZT_zf", 5)
							elseif suit == sgs.Card_Diamond then room:setPlayerMark(player, "wmBZT_zf", 6)
							elseif suit == sgs.Card_Club then room:setPlayerMark(player, "wmBZT_zf", 7)
							elseif suit == sgs.Card_Spade then room:setPlayerMark(player, "wmBZT_zf", 8) end
						end
					end
					if player:getMark("wmBZT_zf") == 1 then --☯天覆阵☯
						...
						if use.card:objectName() == "slash" then
							if player:isMale() then sgs.Sanguosha:playAudioEffect("audio/card/male/slash.ogg", false)
							elseif player:isFemale() then sgs.Sanguosha:playAudioEffect("audio/card/female/slash.ogg", false) end
							if use.card:isRed() then room:setEmotion(player, "slash_red")
							elseif use.card:isBlack() then room:setEmotion(player, "slash_black") end
						elseif use.card:isKindOf("FireSlash") then
							if player:isMale() then sgs.Sanguosha:playAudioEffect("audio/card/male/fire_slash.ogg", false)
							elseif player:isFemale() then sgs.Sanguosha:playAudioEffect("audio/card/female/fire_slash.ogg", false) end
							room:setEmotion(player, "fire_slash")
						elseif use.card:isKindOf("ThunderSlash") then
							if player:isMale() then sgs.Sanguosha:playAudioEffect("audio/card/male/thunder_slash.ogg", false)
							elseif player:isFemale() then sgs.Sanguosha:playAudioEffect("audio/card/female/thunder_slash.ogg", false) end
							room:setEmotion(player, "thunder_slash")
						elseif use.card:isKindOf("IceSlash") then
							if player:isMale() then sgs.Sanguosha:playAudioEffect("audio/card/male/ice_slash.ogg", false)
							elseif player:isFemale() then sgs.Sanguosha:playAudioEffect("audio/card/female/ice_slash.ogg", false) end
							if use.card:isRed() then room:setEmotion(player, "slash_red")
							elseif use.card:isBlack() then room:setEmotion(player, "slash_black") end
						end
						data:setValue(use)
					...
					elseif player:getMark("wmBZT_zf") == 3 then --☯风扬阵☯
						if not use.from:isNude() and player:canDiscard(use.from, "he") then
							local fytt = room:askForCardChosen(player, use.from, "he", self:objectName(), false, sgs.Card_MethodDiscard)
					...
					elseif player:getMark("wmBZT_zf") == 6 then --☯虎翼阵☯
						...
						local hy_slash = room:askForUseSlashTo(player, use.from, "@wmBZT_hyz-slash:" .. use.from:objectName(), true)
...
WmKongmingdeng = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		if player:getPhase() == sgs.Player_Play and player:isWounded() and not player:isKongcheng() then
			if room:askForSkillInvoke(player, self:objectName(), data) then
				local yuanjun = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(), "WmKongmingdeng-invoke", true, true) --改成可不选是为了AI
				if yuanjun then
					local kmd = room:askForExchange(player, self:objectName(), 1, 1, false, "#WmKongmingdeng:".. yuanjun:getGeneralName())
					if kmd then
						room:obtainCard(yuanjun, kmd, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE, yuanjun:objectName(), player:objectName(), self:objectName(), ""), false)
					end
				end
			end
		end
...
WmQixingdengs = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_RoundStart then
			for _, p in sgs.qlist(room:getOtherPlayers(player)) do
				...
				if #pattern > 0 then
					...
					local qxds = room:askForUseCard(p, table.concat(pattern, ","), "@WmQixingdeng_use:"..qxd_card:objectName(), -1)
					if qxds then
		...
		elseif event == sgs.DamageInflicted then
			...
			or player:getPile("WmQixingdeng"):length() == 0 then return false end
			local random_get = {}
			for _, c in sgs.qlist(player:getPile("WmQixingdeng")) do
				local qxd = sgs.Sanguosha:getCard(c)
				table.insert(random_get, qxd)
			...
			if #pattern > 0 then
				...
				local qxds = room:askForUseCard(player, table.concat(pattern, ","), "@WmQixingdeng_use:"..qxd_card:objectName(), -1)
				if qxds then
					if not player:hasFlag("WmQixingdeng_anaNoLimit") then room:setPlayerFlag(player, "WmQixingdeng_anaNoLimit") end --专门处理【酒】的不计次
				end
				room:setPlayerFlag(player, "-WmQixingdeng_useNoLimit")
			end
		elseif event == sgs.EventPhaseChanging then
				...
				if not cards:isEmpty() then
					...
					if equipA then
						...
						if wzg:getEquip(equip_index) == nil and wzg:hasEquipArea(equip_index) then
							...
							Moves:append(sgs.CardsMoveStruct(equipA:getId(), wzg, sgs.Player_PlaceEquip, sgs.CardMoveReason()))
						else
							Moves:append(sgs.CardsMoveStruct(equipA:getId(), wzg, sgs.Player_PlaceHand, sgs.CardMoveReason()))
						end
...
xj_pandiCard = sgs.CreateSkillCard{
	...
	target_fixed = false,
	filter = function(self, targets, to_select)
		return #targets == 0 and to_select:objectName() ~= sgs.Self:objectName() and not to_select:hasFlag("xj_lingyue_FD")
	end,
