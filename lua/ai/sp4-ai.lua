...
sgs.ai_skill_use["@@guanxu1"] = function(self, prompt)
	...
	for c, id in sgs.list(guanxuhand) do
		...
		then
			suits[c:getSuitString()] = suits[c:getSuitString()] + 1
		else
...
addAiSkills("juguan").getTurnUseCard = function(self)
	...
	for d, c in sgs.list(cards) do
		...
		then
			return sgs.Card_Parse("@JuguanCard=" .. c:getEffectiveId() .. ":duel")
		end
	end
	for d, c in sgs.list(cards) do
		...
		if fs:isAvailable(self.player) and d.card and d.to
		then
			return sgs.Card_Parse("@JuguanCard=" .. c:getEffectiveId() .. ":slash")
		end
	end
end
...
sgs.ai_fill_skill.tenyearshuhe = function(self)
	...
	for _, c in sgs.list(cards) do
		...
		then
			return sgs.Card_Parse("@TenyearShuheCard=" .. c:getId())
		end
	end
...
function sgs.ai_cardsview.jibing(self, class_name, player)
	...
	if class_name == "Jink"
	then
		return ("jink:jibing[no_suit:0]=" .. ids:at(0))
	elseif class_name == "Slash"
	then
		return ("slash:jibing[no_suit:0]=" .. ids:at(0))
	end
end
