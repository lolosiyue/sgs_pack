...
function SmartAI:findLeijiTarget(player,leiji_value,slasher,latest_version)
	...
	local getCmpValue = function(enemy)
		...
		then return 100 end
		if not self:isGoodTarget(enemy,self.enemies) then value = value+50 end
		...
		if enemy:isChained() and self:isGoodChainTarget(enemy,sgs.DamageStruct_Thunder,player,latest_version==1 and 1 or 2) and #(self:getChainedEnemies(player))>1 then value = value-25 end
