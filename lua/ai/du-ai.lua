...
sgs.ai_skill_choice.jinfan = function(self, choices)
	...
	if self:isFriend(self.room:getCurrent()) and allow then return allow end
	return disallow
end
...
jfTakeCard_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("#jfTakeCard") then return end
...
sgs.ai_skill_use_func["#jfTakeCard"] = function(card, use, self)
	...
	if sgs.ai_role[self.player:objectName()] == "neutral" then sgs.ai_use_priority["#jfTakeCard"] = 0 end
	if self.player:hasUsed("jfTakeCard") then return end
	local zhanglu
	local cards
	for _, player in sgs.qlist(self.room:getAlivePlayers()) do
...
sgs.ai_event_callback[sgs.ChoiceMade].jinfan = function(self, player, data)
	...
	if datastr == "skillChoice:jinfan:jinfanTake_allow" then
...
sgs.ai_skill_use["@@duYinling"] = function(self, prompt)
	...
	if jiangwei and self:isFriend(jiangwei) and jiangwei:getMark("zhiji") == 0 and jiangwei:getHandcardNum() == 1
		...
		if add_player(jiangwei, 1) == 1 then return ("#duYinlingCard:.:->%s"):format(targets[1]) end
	end

	if dengai and self:isFriend(dengai) and (not self:isWeak(dengai) or self:getEnemyNumBySeat(self.player, dengai) == 0)
		and dengai:hasSkill("zaoxian") and dengai:getMark("zaoxian") == 0 and dengai:getPile("field"):length() == 2 and add_player(dengai, 1) == 1 then
	...
	if zhugeliang and self:isFriend(zhugeliang) and zhugeliang:getHandcardNum() == 1 and self:getEnemyNumBySeat(self.player, zhugeliang) > 0 then
		if zhugeliang:getHp() <= 2 then
			if add_player(zhugeliang, 1) == 1 then return ("#duYinlingCard:.:->%s"):format(targets[1]) end
		else
	...
	if luxun and self:isFriend(luxun) and luxun:getHandcardNum() == 1 and self:getEnemyNumBySeat(self.player, luxun) > 0 then
		...
		if #cards == 1 and (cards[1]:hasFlag("visible") or cards[1]:hasFlag(flag)) then
			if cards[1]:isKindOf("TrickCard") or cards[1]:isKindOf("Slash") or cards[1]:isKindOf("EquipCard") then
				if add_player(luxun, 1) == 1 then return ("#duYinlingCard:.:->%s"):format(targets[1]) end
			end
		end
	end
	...
	if zhijiangwei and self:isFriend(zhijiangwei) and zhijiangwei:getHandcardNum() == 1 and #self.enemies > 0 and
		...
		if isGood and add_player(zhijiangwei, 1) == 1 then return ("#duYinlingCard:.:->%s"):format(targets[1]) end
	end
	...
	for i = 1, #self.enemies, 1 do
		...
		for _, card in ipairs(cards) do
			if (card:hasFlag("visible") or card:hasFlag(flag)) and (card:isKindOf("Peach") or card:isKindOf("Nullification") or card:isKindOf("Analeptic")) then
				if add_player(p) == 1 then return ("#duYinlingCard:.:->%s"):format(targets[1]) end
			end
		end
	end
	...
	for i = 1, #self.enemies, 1 do
		local p = self.enemies[i]
		if p:hasSkills("jijiu|qingnang|xinzhan|leiji|nosleiji|olleiji|jieyin|beige|kanpo|liuli|qiaobian|zhiheng|guidao|longhun|xuanfeng|tianxiang|ol_tianxiang|noslijian|lijian") then
			if add_player(p) == 1 then return ("#duYinlingCard:.:->%s"):format(targets[1]) end
		end
	end
	...
	for i = 1, #self.enemies, 1 do
		...
		if good_target and add_player(p) == 1 then return ("#duYinlingCard:.:->%s"):format(targets[1]) end
	end
	...
	if dengai and self:isFriend(dengai) and dengai:hasSkill("zaoxian") and (not self:isWeak(dengai) or self:getEnemyNumBySeat(self.player, dengai) == 0) and add_player(dengai, 1) == 1 then
		return ("#duYinlingCard:.:->%s"):format(targets[1])
	end

	local others = self.room:getOtherPlayers(self.player)
	for _, other in sgs.qlist(others) do
		if self:objectiveLevel(other) >= 0 and not (other:hasSkill("tuntian") and other:hasSkill("zaoxian")) and add_player(other) == 1 then
			return ("#duYinlingCard:.:->%s"):format(targets[1])
		end
	end
...
sgs.ai_skill_invoke.duXiaoguo = function(self, data)
	...
	if damage.to:hasArmorEffect("silver_lion") or damage.to:hasSkill("DSTP") then return false end
...
du_zhouxuan_skill.getTurnUseCard = function(self)
	if self.player:getMark("@yaochong") == 0 or not self.player:canDiscard(self.player, "h") then return end
	local card_id
...
sgs.ai_skill_invoke["tongpao_jink"] = function(self, data)
	...
	for _, p in sgs.qlist(lieges) do
		if self:hasEightDiagramEffect(p) and self:isFriend(p) then return true end
	end
...
local tongpaoSlash_skill = { name = "tongpaoSlash" }
...
sgs.ai_skill_use_func["#tongpaoSlashCard"] = function(card, use, self)
	if self.player:hasUsed("#tongpaoSlashCard") and self.player:hasFlag("Global_tongpaoFailed") then return end
...
sgs.ai_skill_invoke["tongpao_slash"] = function(self, data)
	...
	local lieges = self.room:getLieges("wu", self.player)
	for _, p in sgs.qlist(lieges) do
		if p and p:getPhase() ~= sgs.Player_NotActive
			and self:isFriend(p) and self:getOverflow(p) > 2 and not self:hasCrossbowEffect(p) then
			return true
		end
	end
	local cards = self.player:getHandcards()
