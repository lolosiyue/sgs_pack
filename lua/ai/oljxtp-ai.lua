...
ollongdan_skill.getTurnUseCard = function(self)
	...
	for _, c in ipairs(cards) do
		if c:isKindOf("Analeptic") then
			return sgs.Card_Parse(("peach:ollongdan[%s:%s]=%d"):format(c:getSuitString(), c:getNumberString(),
				c:getEffectiveId()))
		end
	end
	for _, c in ipairs(cards) do
		if c:isKindOf("Peach") then
			...
			sgs.Card_Parse(("analeptic:ollongdan[%s:%s]=%d"):format(c:getSuitString(), c:getNumberString(),
				c:getEffectiveId()))
		end
	end
	for _, c in ipairs(cards) do
		if c:isKindOf("Jink") then
			return sgs.Card_Parse(("slash:ollongdan[%s:%s]=%d"):format(c:getSuitString(), c:getNumberString(),
				c:getEffectiveId()))
		end
	end
end
		...
		for i = 1, 5 do
			...
			then
				...
				then
					...
					if user:isKindOf("ExNihilo") and math.random(1, 3) <= ratio then
						return guhuo_str
					elseif math.random(1, 4) <= ratio then
						return guhuo_str
					end
				end
			end
...
sgs.ai_guhuo_card.olguhuo = function(self, toname, class_name)
	...
	then
		return "@OLGuhuoCard=" .. fake[1]:getEffectiveId() .. ":" .. toname
	end
	...
	then
		...
		then
			return "@OLGuhuoCard=" .. hcards[1]:getEffectiveId() .. ":" .. toname
...
sgs.ai_choicemade_filter.pindian.olzhiba_pindian = function(self, from, promptlist)
	...
	if lord_max_card and lord_max_card:getNumber() >= number then
		sgs.updateIntention(from, lord, -60)
	elseif lord_max_card and lord_max_card:getNumber() < number then
		sgs.updateIntention(from, lord, 60)
