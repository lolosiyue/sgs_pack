...
sgs.ai_playerchosen_intention.zuilun = function(self, from, to)
	if self:needToLoseHp(friend, self.player, nil, false, true) then return end
...
sgs.ai_skill_playerchosen.liangyin = function(self, targets)
	...
	for _, p in sgs.list(tos) do
		...
		elseif p:getHandcardNum() < self.player:getHandcardNum() and p:canDiscard(p, "he") then
			if self:isEnemy(p) and not self:needToThrowCard(p) and not (not self:hasLoseHandcardEffective(p) and not p:isKongcheng()) then return
				p end
...
sgs.ai_skill_discard.kongsheng = function(self, discard_num, min_num, optional, include_equip)
	...
	for _, card in sgs.list(cards) do
		...
		elseif card:isKindOf("Slash") then
			...
			if slash > 1 and not self:hasCrossbowEffect() and not table.contains(to_discard, card:getId()) then table
					.insert(to_discard, card:getId()) end
		elseif card:isKindOf("Analeptic") then
			if (not useSlash or ana > 0) and not table.contains(to_discard, card:getId()) then table.insert(to_discard,
					card:getId()) end
			ana = ana + 1
...
sgs.ai_skill_playerchosen.zhengu = function(self, targets)
	...
	for _, who in sgs.list(self.friends_noself) do
		if who:getHandcardNum() < f and not self:needKongcheng(who, true) and not hasManjuanEffect(who) then
			f = who:getHandcardNum()
			fri = who
		end
	...
	for _, who in sgs.list(self.enemies) do
		if who:getHandcardNum() > e and not (n == 0 and self:needKongcheng(who, true)) then
			e = who:getHandcardNum()
			ene = who
		end
...
sgs.ai_playerchosen_intention.zhengu = function(self, from, to)
	if to:getHandcardNum() < from:getHandcardNum() and self:canDraw(to, from) then sgs.updateIntention(from, to, -10) end
	if to:getHandcardNum() > from:getHandcardNum() and not self:needToThrowCard(to, "h") then sgs.updateIntention(from,
			to, 10) end
end
