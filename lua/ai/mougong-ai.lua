...
moukejii_skill.getTurnUseCard = function(self)
	...
	or (self.player:getMark("moudujiangg") > 0 and self.player:hasUsed("moukejiiCard")) then return end
...
sgs.ai_skill_choice.moukejii = function(self, choices, data)
	...
	if self.player:getHp() <= 1 and self:getCardsNum("Peach") == 0 and self:getCardsNum("Analeptic") == 0 then return "cancel" end
	return "2"
end
	...
	for _, card in ipairs(cards) do
		if card:isBlack() and ((self:getUseValue(card) < sgs.ai_use_value.Dismantlement) or inclusive or self:getOverflow() > 0) then
			...
			if card:isKindOf("Armor") then
				...
				elseif self.player:hasEquip(card) and not self:needToThrowArmor() then shouldUse = false end
			end
			if card:isKindOf("Weapon") then
				if not self.player:getWeapon() then shouldUse = false
				elseif self.player:hasEquip(card) and not has_weapon then shouldUse = false end
			end
			if card:isKindOf("Slash") then
				local dummy_use = {isDummy = true}
...
mouluanjii_skill.getTurnUseCard = function(self)
			...
			if not fvalueCard then
				...
				for _, scard in ipairs(cards) do
					...
					if first_card ~= scard and not svalueCard then
						local card_str = ("archery_attack:mouluanjii[%s:%s]=%d+%d"):format("to_be_decided", 0, first_card:getId(), scard:getId())
	...
	if first_found and second_found then
		...
		local archeryattack = sgs.Card_Parse(card_str)
		assert(archeryattack) return archeryattack
	end
end
