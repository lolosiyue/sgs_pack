...
sgs.ai_skill_askforag.gongxin = function(self, card_ids)
	...
	if self:isEnemy(target) and hasTuntianEffect(target) then
...
sgs.ai_skill_use["@@kuangfeng"] = function(self, prompt)
	...
	if usecard then
		...
		if target[1] then
			return "@KuangfengCard=" .. self.player:getPile("stars"):first() ..
				"->" .. target[1]:objectName()
		else
			return "."
		end
...
sgs.ai_skill_use["@@dawu"] = function(self, prompt)
	...
	if self.player:getPile("stars"):length() > #targets and self:isWeak() then
...
sgs.ai_skill_use_func.ShenfenCard = function(card, use, self)
	...
	for _, player in sgs.list(self.room:getOtherPlayers(self.player)) do
		if self:damageIsEffective(player, sgs.DamageStruct_Normal) then
			...
			if self:needToLoseHp(player, self.player) or getBestHp(player) == player:getHp() - 1 then
				value_d = value_d *
					0.8
			end
...
function SmartAI:needBear(player)
	...
	return (player:hasSkills("renjie+baiyin") and not player:hasSkill("jilve") and player:getMark("&bear") < 4)
		or (player:hasSkills("sprenjie+spbaiyin") and player:getMark("spbaiyin") == 0 and player:getMark("&spren") < 4)
end
...
sgs.ai_skill_playerchosen.cuike = function(self, targets)
	...
	if mark % 2 == 1 then
		...
		for _, enemy in sgs.list(self.enemies) do
			if self:doDisCard(enemy) and not enemy:isChained() and not enemy:hasSkills("qianjie|jieying") then
				return enemy
			end
		end
		for _, enemy in sgs.list(self.enemies) do
			if self:doDisCard(enemy) then
				return enemy
			end
		end
		for _, friend in sgs.list(self.friends_noself) do
...
sgs.ai_skill_use_func.ZhanhuoCard = function(card, use, self)
	...
	local targets = {}
	for _, enemy in sgs.list(self.enemies) do
		if #targets < self.player:getMark("&junlve") and enemy:isChained() and self:doDisCard(enemy, "e") and
...
sgs.ai_skill_playerchosen.jieyingg = function(self, targets)
	...
	for _, enemy in sgs.list(self.enemies) do
		if enemy:faceUp() and not enemy:hasSkills("tenyearliegong|tieji") and not self:needKongcheng(enemy)
			...
			if not enemy:inMyAttackRange(self.player) or enemy:getHandcardNum() > 3 or self:hasSkills(sgs.notActive_cardneed_skill, enemy) then
				return
					enemy
			end
...
newlonghun_skill.getTurnUseCard = function(self, inclusive)
	...
	if #two_diamond_cards == 2 and self:slashIsAvailable() and self:getOverflow() > 1 then
		return sgs.Card_Parse(("fire_slash:newlonghun[%s:%s]=%d+%d"):format("to_be_decided", 0, two_diamond_cards[1],
			two_diamond_cards[2]))
	end
	for _, c in sgs.list(usable_cards) do
		if c:getSuit() == sgs.Card_Diamond and self:slashIsAvailable() and not c:isKindOf("Peach") and not (c:isKindOf("Jink") and self:getCardsNum("Jink") < 3) and not (c:isKindOf("WoodenOx") and self.player:getPile("wooden_ox"):length() > 0) then
			return sgs.Card_Parse(("fire_slash:newlonghun[%s:%s]=%d"):format(c:getSuitString(), c:getNumberString(),
				c:getEffectiveId()))
		end
	end
	for _, c in sgs.list(usable_cards) do
		if c:getSuit() == sgs.Card_Heart and self.player:getMark("Global_PreventPeach") == 0 and not c:isKindOf("Peach") then
			return sgs.Card_Parse(("peach:newlonghun[%s:%s]=%d"):format(c:getSuitString(), c:getNumberString(),
				c:getEffectiveId()))
		end
	end
end
...
sgs.ai_skill_invoke.chuyuan = function(self, data)
	...
	if self.player:getPile("cychu"):length() == 2 then
		...
		if self.player:hasSkill("tianxing") and self.player:getMark("tianxing") <= 0 then
...
sgs.ai_skill_playerchosen.shenfu_ji = function(self, targets)
	...
	if not self.player:isChained() or not self:isWeak() or not self:damageIsEffective(self.player, sgs.DamageStruct_Thunder, self.player) then
		return
			targets_table[1]
	end
...
function CanUsejiwu(self)
	...
	if not self.player:hasSkill("lieren", true) and self:getCardsNum("Slash") > 0 and
		(self.player:getHandcardNum() - self:getCardsNum("Peach") - self:getCardsNum("Slash") > 0) then
		return true
	end
	local candis = false
...
sgs.ai_skill_invoke.cuixin = function(self, data)
	...
	then
		...
		local from = self.player:getNextAlive(self.player:aliveCount() - 1)
		--if items:match("shangjia") then to = self.player:getNextAlive(self.player:aliveCount()-1) end
		if table.contains(items, "shangjia") then
			to = self.player:getNextAlive(self.player:aliveCount() - 1)
			from = self.player:getNextAlive()
		end
		if d:isDamageCard()
			and not self:isFriend(from)
			and self:isEnemy(to) then
			return self:canCanmou(to, { from = self.player, card = d, to = {} })
		elseif not self:isFriend(to)
		then
