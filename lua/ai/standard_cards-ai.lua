...
function hasExplicitRebel(room)
	...
	for _,player in sgs.list(room:getAllPlayers())do
		if sgs.isRolePredictable() and sgs.ai_role[player:objectName()]=="rebel" then return true end
...
function SmartAI:isGoodTarget(to,targets,card)
	...
	and sgs.playerRoles["loyalist"]>0 then return end
	...
	then
		...
		return sgs.compareRoleEvaluation(to,"rebel","loyalist")=="rebel"
	end
...
function sgs.getDefenseSlash(player,self)
	...
	attacker = self and self.player or global_room:getCurrent()
	defense = getCardsNum("Jink",player,attacker)
	knownJink = getKnownCard(player,attacker,"Jink",true)
	if sgs.card_lack[player:objectName()]["Jink"]==1 and knownJink<1 then defense = 0 end
	...
	if player:hasLordSkill("hujia") then
		for _,liege in sgs.list(global_room:getLieges("wei",player))do
	...
	if attacker:canSlashWithoutCrossbow() and attacker:getPhase()==sgs.Player_Play
	then
	...
	need_double_jink = attacker:hasSkills("wushuang|drwushuang")
	...
	and getCardsNum("Jink",player)<1.5 and (not player:hasLordSkill("hujia") or hujiaJink<2)
	then defense = 0 end
	...
	and not (player:hasLordSkill("hujia") and hujiaJink>=1)
	then defense = 0 end
	if player:isCardLimited(dummyCard("jink"),sgs.Card_MethodUse) then defense = 0 end
	if player:hasFlag("QianxiTarget")
	then
		if player:getMark("@qianxi_red")>0
		then
			if player:hasSkill("qingguo")
			or player:hasSkill("longhun") and player:isWounded()
	...
	if attacker and not attacker:hasSkill("jueqing")
	then
		for _,masochism in sgs.list(sgs.masochism_skill:split("|"))do
			if player:hasSkill(masochism) and self:isGoodHp(player)
			then defense = defense+1 end
		end
	...
	if not (self and self:isGoodTarget(player)) then defense = defense+10 end
	...
	if (player:getSeat()-attacker:getSeat())%playernum>=playernum-2 and playernum>3
	and player:getHandcardNum()<=2 and player:getHp()<=2
	then defense = defense-0.4 end
	if player:hasSkill("tianxiang") then defense = defense+player:getHandcardNum()*0.5 end
	if player:getHandcardNum()+hujiaJink<1 and not player:hasSkill("kongcheng")
	then
		...
		if attacker:hasWeapon("guding_blade") and player:getHandcardNum()<1
		and not (player:hasArmorEffect("silver_lion") and not IgnoreArmor(attacker,player))
		then defense = defense-2 end
	end
	for _,c in sgs.list(attacker:getHandcards())do
		if (attacker:hasWeapon("fan") and c:objectName()=="slash"
		and not c:isKindOf("ThunderSlash")) or c:isKindOf("FireSlash")
		and player:hasArmorEffect("vine") and not IgnoreArmor(attacker,player)
	...
	if (attacker:hasSkill("roulin") and player:isFemale()) or (attacker:isFemale() and player:hasSkill("roulin"))
...
function SmartAI:slashProhibit(card,enemy,from)
	if self.room:getMode():find("_mini_36")
...
function SmartAI:slashIsEffective(slash,to,from,ignore_armor)
	...
	use.to:append(to)
	ignore_armor = ignore_armor or slash:hasFlag("Qinggang") or slash:hasFlag("SlashIgnoreArmor") or IgnoreArmor(self.player,to)
	for tr,sk in sgs.list(aiConnect(to,ignore_armor))do
...
function SmartAI:useCardSlash(card,use)
	...
	function slashNoTarget(target)
		if use.current_targets and table.contains(use.current_targets,target:objectName()) then return end
		if use.to
		then
			...
			then
				...
				then
					...
					if use.isDummy or use.card:subcardsLength()>1 then
						...
						if wt then use.card = wt return end
						fa = self:searchForAnaleptic(use.card,target)
						if fa then use.card = fa return end
							...
							then
								...
								then
									for _,slash in sgs.list(self:getCards("Slash"))do
										...
										and self.player:canUse(slash,target) then use.card = slash break end
									end
								end
							else
								for _,ns in sgs.list(self:getCards("NatureSlash"))do
									if not self:slashProhibit(ns,target) and self.player:canUse(ns,target)
									and self:isGoodChainTarget(target,card) then use.card = ns break end
								end
							end
						end
					end
	...
	for _,friend in ipairs(self.friends_noself)do
		if self:isPriorFriendOfSlash(friend,use.card or card)
	...
	for _,friend in ipairs(self.friends_noself)do
		if (use.card or card):getSkillName()~="lihuo"
...
sgs.ai_skill_use.slash = function(self,prompt)
	...
	then
		...
		then
			...
			then
				local targets,use = {},{to=sgs.SPlayerList(),current_targets={}}
				for _,p in sgs.list(self.room:getOtherPlayers(target))do
					table.insert(use.current_targets,p:objectName())
				end
				...
				then
					use.card = nil
					use.current_targets = {}
					table.insert(use.current_targets,target:objectName())
					self:useCardSlash(callback,use)
...
sgs.ai_card_intention.Slash = function(self,card,from,tos)
	...
	for _,to in sgs.list(tos)do
		...
		and self:ajustDamage(from,to,1,card)<2 and (hasExplicitRebel(self.room) or sgs.explicit_renegade)
		and not self:canLiegong(to,from) then value = 0 end
...
sgs.ai_skill_cardask["slash-jink"] = function(self,data,pattern,target)
	...
	function getJink()
		if target:hasSkill("dahe") and self.player:hasFlag("dahe")
		then
	...
	then
		...
		then
			...
			for _,card in sgs.list(self:getCards("Jink"))do
				...
				then use = true break end
			end
			if not use then return not isdummy and "." end
		end
		if self.player:getHandcardNum()==1 and self:needKongcheng()
		or not (self:hasLoseHandcardEffective() or self.player:isKongcheng())
		then return getJink() end
		...
		then
			...
			or self.player:getCardCount()<2 and not self.player:getArmor()
 			then return getJink() end
		...
		then
			...
			then
				...
				then return not isdummy and "." end
		...
		then
			...
			if isCard("Jink",current[1],self.player) then if self:getUseValue(current[2])>6 then return "." end
			elseif self:getUseValue(current[1])>6 then return "." end
		end
	end
	return getJink()
end
...
sgs.ai_skill_cardask["@axe"] = function(self,data,pattern,target)
	...
	or effect.to:getHp()<2 and not effect.to:hasSkill("buqu")
	or self:needKongcheng() and self.player:getHandcardNum()>0
	or self:ajustDamage(self.player,target,1,effect.slash)
	...
	then
		...
		and self.player:getHandcardNum()>0
		then
		...
		then
			...
			if #cards<2 and ec then
				table.insert(cards,ec:getEffectiveId())
			end
		end
		if #cards<2 and self.player:getHandcardNum()>2
		then
			for _,card in sgs.list(hcards)do
				if #cards>1 then break end
				if table.contains(cards,card:getEffectiveId())
				then continue end
				table.insert(cards,card:getEffectiveId())
			end
		end
		...
		if #cards<2 and ec and not table.contains(cards,ec:getEffectiveId())
		then table.insert(cards,ec:getEffectiveId()) end
		if #cards<2 and self.player:getHandcardNum()>0
		then
			for _,card in sgs.list(hcards)do
				if #cards>1 then break end
				if table.contains(cards,card:getEffectiveId())
		...
		if #cards>1 then
			...
			then return "$"..table.concat(cards,"+") else return "." end
		end
	end
end
...
function turnUse_spear(self,skill_name)
	...
	if #newcards<2
	or #cards<self.player:getHp()
	and not self:hasSkills("kongcheng|lianying|noslianying|paoxiao|shangshi|noshangshi|zhiji|benghuai")
	and self.player:getHp()<5
	then return end
	local slashs = {}
	...
	for _,c1 in sgs.list(newcards)do
		...
		for _,c2 in sgs.list(newcards2)do
			...
			then table.insert(slashs,dc)
			else dc:deleteLater() end
...
sgs.ai_skill_cardask.aoe = function(self,data,pattern,target,name)
	...
	then aoe = data:toCardEffect().card
	else aoe = sgs.Sanguosha:cloneCard(name) end
	assert(aoe)
...
function SmartAI:useCardSnatchOrDismantlement(card,use)
	...
	local using_2013 = card:isKindOf("Dismantlement") and self.room:getMode()=="02_1v1" and sgs.GetConfig("1v1/Rule","Classical")~="Classical"
	...
	local function addTarget(to,cid)
		...
		if use.current_targets and table.contains(use.current_targets,to:objectName())
		then return end
		if use.to and cid
		then
	...
	then
		...
		for _,p in sgs.list(players)do
			...
			then
				...
				then
					local kingdoms = {}
					if self.player:getGeneral():isLord() then table.insert(kingdoms,self.player:getGeneral():getKingdom()) end
					if self.player:getGeneral2() and self.player:getGeneral2():isLord() then table.insert(kingdoms,self.player:getGeneral2():getKingdom()) end
					if table.contains(kingdoms,p:getKingdom()) or lord:hasSkill("yongsi")
					then else table.insert(enemies,p) end
				elseif lord
				then
					if p:objectName()~=lord:objectName()
					then table.insert(enemies,p) end
				else table.insert(enemies,p) end
			end
		end
		local temp = {}
		for _,enemy in sgs.list(enemies)do
			if enemy:hasSkills("tuntian+guidao")
			and enemy:hasSkills("zaoxian|jixi|zhiliang|leiji|nosleiji")
			then else table.insert(temp,enemy) end
		end
		enemies = temp
	...
	for _,enemy in sgs.list(enemies)do
		...
		then
			for _,h in sgs.list(enemy:getHandcards())do
				...
				and isCard("Peach,Analeptic",h,enemy) and addTarget(enemy,self:getCardRandomly(enemy,"h")) then return end
			end
		end
	end
	for _,enemy in sgs.list(enemies)do
		...
		then
			...
			then
				for _,equip in sgs.list({enemy:getDefensiveHorse(),enemy:getArmor(),enemy:getOffensiveHorse(),enemy:getWeapon(),enemy:getTreasure()})do
					if (not enemy:hasSkill("jijiu") or equip:isRed())
					and addTarget(enemy,equip) then return end
				end
				...
				and addTarget(enemy,self:getCardRandomly(enemy,"h")) then return end
	...
	for _,enemy in sgs.list(enemies)do
		...
		and addTarget(enemy,enemy:getArmor()) then return end
	end
...
function SmartAI:useCardCollateral(card,use)
	...
	for _,enemy in sgs.list(self.enemies)do
		...
		then needCrossbow = true break end
	end
	function useToCard(to)
		return not (use.current_targets and table.contains(use.current_targets,to:objectName()))
		and not (use.to and use.to:contains(to))
	...
	if needCrossbow
	and self:getCardsNum("Slash")>2 and not self:hasCrossbowEffect()
	then
	...
	for _,friend in sgs.list(fromList)do
		if useToCard(enemy) and self:objectiveLevel(friend)<0
...
function SmartAI:useCardIndulgence(card,use)
	...
	local getvalue = function(enemy)
		if enemy:containsTrick("YanxiaoCard")
		or not CanToCard(card,self.player,enemy)
	...
	table.sort(self.enemies,cmp)
	if getvalue(self.enemies[1])>-100
	and use.to
	then
		use.card = card
...
function SmartAI:willUseLightning(card)
	...
	if self:getFinalRetrial(self.player)==2 then return
	elseif self:getFinalRetrial(self.player)==1 then return true
...
sgs.ai_skill_askforag.amazing_grace = function(self,card_ids)
	...
	local cards = {}
	local trickNum = 0
	for c,card_id in sgs.list(card_ids)do
		c = sgs.Sanguosha:getCard(card_id)
	...
	then
		...
		and not self:getSameEquip(card) then return cid.eightdiagram end
		if NextPlayerisEnemy
		and self:hasSkills("tiandu|leiji|nosleiji|noszhenlie|gushou|hongyan",NextPlayer)
		and not self:getSameEquip(card,NextPlayer) then return cid.eightdiagram end
		...
		then return cid.eightdiagram end
	end
	...
	then
		...
		for _,aplayer in sgs.list(self.room:getOtherPlayers(self.player))do
			...
			then return cid.silverlion end
	...
	then
		...
		for _,enemy in sgs.list(self.enemies)do
			...
			then inAttackRange = true break end
		end
	...
	for _,card in sgs.list(cards)do
		...
		then return card:getEffectiveId() end
	end
	for _,card in sgs.list(cards)do
		if table.contains(cid,c) then continue end
		return card:getEffectiveId()
