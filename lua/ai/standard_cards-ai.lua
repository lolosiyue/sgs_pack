...
function sgs.getDefenseSlash(player, self)
	...
	need_double_jink = attacker:hasSkills("wushuang|drwushuang")
		...
		or
		(jiangqin and jiangqin:isAdjacentTo(player) and attacker:isAdjacentTo(player) and self and self:isFriend(jiangqin, attacker))
	...
	if not (self and self:isGoodTarget(player, self.enemies)) then defense = defense + 10 end
...
sgs.ai_skill_cardask["slash-jink"] = function(self, data, pattern, target)
	...
	then
		...
		if target:hasSkill("jieyin") and not self.player:isWounded() and self.player:isMale() and not self.player:hasSkills("leiji|nosleiji") then
			return
			"."
		end
		...
		then
			...
			if isCard("Jink", current[1], self.player) then
				if self:getUseValue(current[2]) > 6 then return "." end
			elseif self:getUseValue(current[1]) > 6 then
				return "."
			end
		end
	end
