			...
			if not fvalueCard then
				...
				for _, scard in ipairs(cards) do
					...
					if first_card ~= scard and not svalueCard then
						local card_str = ("archery_attack:lol_hbss_w[%s:%s]=%d+%d"):format("to_be_decided", 0,
...
sgs.ai_skill_invoke.lol_bzll_e = function(self, data)
	...
	local dest
	if move.from and move.from:objectName() == self.player:objectName() then
		dest = self.room:getCurrent()
	elseif move.from then
		dest = self.room:findPlayer(move.from:getGeneralName())
	end
