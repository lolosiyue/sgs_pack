...
function GoldenWaveStart(self)
	...
	if self.player:hasSkill("kurou")
	and self:hasCrossbowEffect()
	then
		local guojia,huatuo
		for _,friend in pairs(self.friends_noself)do
			if friend:hasSkill("yiji")
			and self.player:canSlash(friend)
			and not hasJueqingEffect(self.player,friend)
			then guojia = friend end
			if friend:hasSkill("jijiu")
			and getCardsNum("Peach",friend,self.player)>1
			then huatuo = friend end
		end
		if guojia and huatuo
		then
			sgs.GoldenWaveDetail.KurouActor = {self.player:objectName()}
			sgs.GoldenWaveDetail.YijiActor = {guojia:objectName()}
			sgs.GoldenWaveDetail.JijiuActor = {huatuo:objectName()}
			self.room:setPlayerMark(self.player,"GoldenWaveFlow",1)
			self.room:setPlayerMark(guojia,"GoldenWaveFlow",1)
			self.room:setPlayerMark(huatuo,"GoldenWaveFlow",1)
			return true
		else
			sgs.GoldenWaveDetail.KurouActor = {}
			sgs.GoldenWaveDetail.YijiActor = {}
			sgs.GoldenWaveDetail.JijiuActor = {}
			room:setPlayerMark(self.player,"GWF_Forbidden",1)
			return false
		end
	end
	self.room:setPlayerMark(self.player,"GWF_Forbidden",1)
	return false
end
--黄金苦肉
...
function RuthlessStart(self)
	if self.player:hasSkill("jianxiong")
	then
		local xiaoqiao,guojia
		for _,friend in pairs(self.friends_noself)do
			if friend:hasSkill("yiji")
			then guojia = friend
			elseif friend:hasSkill("tianxiang")
			then xiaoqiao = friend
			else
				self.room:setPlayerMark(friend,"RL_Forbidden",1)
			end
		end
		if xiaoqiao and guojia
		then
			sgs.RuthlessDetail.JianxiongActor = {self.player:objectName()}
			sgs.RuthlessDetail.TianxiangActor = {xiaoqiao:objectName()}
			sgs.RuthlessDetail.YijiActor = {guojia:objectName()}
			self.room:setPlayerMark(self.player,"Ruthless",1)
			self.room:setPlayerMark(guojia,"Ruthless",1)
			self.room:setPlayerMark(xiaoqiao,"Ruthless",1)
			return true
		else
			sgs.RuthlessDetail.JianxiongActor = {}
			sgs.RuthlessDetail.TianxiangActor = {}
			sgs.RuthlessDetail.YijiActor = {}
			self.room:setPlayerMark(self.player,"RL_Forbidden",1)
			return false
		end
	end
	self.room:setPlayerMark(self.player,"RL_Forbidden",1)
...
function RLTianxiangSkillUse(self,data)
	...
	if damage then
		...
		if aoe and aoe:isKindOf("AOE") then
			...
			for _,id in sgs.list(handcards)do
				local suit = card:getSuit()
				if suit==sgs.Card_Heart
				then
...
function KOFKurouTurnUse(self)
		...
		if self:getCardsNum("Analeptic")+self:getCardsNum("Peach")>0
		then
			return sgs.Card_Parse("@KurouCard=.")
		end
	end
...
function KOFControlStart(player)
	...
	or player:hasSkill("super_guanxing")
	then
		...
		then
			...
			if followList then
				...
				then
					local follow = 1
					for _,name in sgs.list(followList)do
...
sgs.classical_func["kurou"] = function(self,data)
	...
	if mode=="02_1v1"
	then
		if self.player:getMark("KOFKurouRush")>0
		then return KOFKurouTurnUse
		elseif self.player:getMark("KKR_Forbidden")==0
		then
			if KOFKurouStart(self)
			then
	...
	elseif mode=="06_3v3"
	then
		if self.player:getMark("GoldenWaveFlow")>0
		then return GWFKurouTurnUse
		elseif self.player:getMark("GWF_Forbidden")==0
		then
...
sgs.classical_func["yiji"] = function(self,data)
	...
	then
		...
		then
			return GWFYijiAsk
		elseif self.player:getMark("Ruthless")>0
		then
			return RLYijiAsk
		end
	end
...
sgs.classical_func["jijiu"] = function(self,data)
	...
	then
		if self.player:getMark("GoldenWaveFlow")>0
		then return GWFJijiuSignal end
	end
end
--天香
sgs.classical_func["tianxiang"] = function(self,data)
	local mode = string.lower(self.room:getMode())
	if mode=="06_3v3" then
		if self.player:getMark("Ruthless")>0
		then return RLTianxiangSkillUse end
	end
end