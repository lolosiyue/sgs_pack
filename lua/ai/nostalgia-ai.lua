...
sgs.ai_skill_cardask["@nosenyuan-heart"] = function(self, data)
	...
	if self:needToLoseHp(self.player, damage.to, damage.card, true) and not self:hasSkills(sgs.masochism_skill) then
		return
		"."
	end
...
function sgs.ai_slash_prohibit.nosenyuan(self, from, to, card)
	if hasJueqingEffect(from, to, getCardDamageNature(from, to, card)) then return false end
...
sgs.ai_need_damaged.nosenyuan = function(self, attacker, player)
	if attacker and self:isEnemy(attacker, player) and self:isWeak(attacker)
		and not (self:needToLoseHp(attacker) and not self:hasSkills(sgs.masochism_skill, attacker))
	then
		return true
	end
	return false
...
function sgs.ai_cardsview_valuable.nosjiefan(self, class_name, player)
	...
	if current:objectName() == player:objectName() or (current:hasSkill("wansha") and player:objectName() ~= dying:objectName())
		or (self:isEnemy(current) and self:findLeijiTarget(current, 50, player)) then
		return nil
	end
...
sgs.ai_skill_playerchosen.nosmiji = function(self, targets)
	...
	if self.player:getPhase() == sgs.Player_Start then
		if self.player:getHandcardNum() - n < 2 and not self:needKongcheng() and not self:willSkipPlayPhase() then
			return
				self.player
		end
...
sgs.ai_skill_use_func.NosRendeCard = function(card, use, self)
	...
	for i = 1, self.player:getHandcardNum() do
		...
		if friend:objectName() == self.player:objectName()
			or not self.player:getHandcards():contains(card)
		then
			continue
		end
		...
		then
			...
			then
				...
				elseif (card:isKindOf("Snatch") or card:isKindOf("Dismantlement"))
...
sgs.ai_use_revises.nosrende = function(self, card, use)
	...
	then
		use.card = card
		if use.to then use.to:append(xy) end
	end
end
...
sgs.ai_skill_choice.nosguhuo = function(self, choices)
	...
	if nosguhuotype and self:getRestCardsNum(nosguhuotype, yuji) < 1 and self.player:getHp() > 0 then return "question" end
	if nosguhuotype and nosguhuotype == "AmazingGrace" then return "noquestion" end
	if nosguhuotype:match("Slash") then
		if yuji:getState() ~= "robot" and math.random(1, 4) == 1 and not sgs.questioner and self:isEnemy(yuji) then
			return
			"question"
		end
	...
	if yuji:hasFlag("NosGuhuoFailed") and math.random(1, 6) == 1 and self:isEnemy(yuji) and self.player:getHp() >= 3
		and self.player:getHp() > self.player:getLostHp() then
		return "question"
	end
	local x = math.random(1, 5)
	self:sort(self.friends, "hp")
	if self.player:getHp() < 2 and self:getCardsNum("Peach") < 1 and self.room:alivePlayerCount() > 2 then
		return
		"noquestion"
	end
	if self:isFriend(yuji) or sgs.questioner then
		return "noquestion"
	elseif self.player:getHp() < self.friends[#self.friends]:getHp()
	then
		return "noquestion"
	end
	if self:needToLoseHp(self.player) and not self.player:hasSkills(sgs.masochism_skill) and x > 1 and self:isEnemy(yuji) then
		return
		"question"
	end
	local questioner
...
nosguhuo_skill.getTurnUseCard = function(self)
	...
	then
		...
		if peach_str and math.random(0, #cards) > 2
		then
	...
	elseif phase == sgs.Player_Play then
		...
		elseif self:getOverflow() >= 2 then
			...
			if (damage.card and damage.card:isKindOf("AOE")) or (self.player:hasFlag("ShenfenUsing") and self.player:faceUp()) then
				if damage.to:getNextAlive():objectName() == self.player:objectName() then
					return true
				else
					local dmg_val = 0
					local p = damage.to
	...
	elseif phase == sgs.Player_NotActive then
		...
		if self:isFriend(current) then
			...
			if self:getOverflow(current) <= 2 then
				...
				if self:slashIsAvailable(current) and getCardsNum("Slash", current, self.player) > 0 then
					threat = threat +
						math.min(1, getCardsNum("Slash", current, self.player))
				end
				return threat >= 1
			end
		end
...
sgs.ai_skill_playerchosen.nosqiuyuan = function(self, targets)
	...
	for _, p in ipairs(targetlist) do
		if self:isEnemy(p) and not (p:getHandcardNum() == 1 and (p:hasSkill("kongcheng") or (p:hasSkill("zhiji") and p:getMark("zhiji") == 0))) then
			if p:hasSkills(sgs.cardneed_skill) then
				return p
			elseif not enemy and not self:canLiuli(p, self.friends_noself) then
				enemy = p
			end
		end
	end
	...
	for _, p in ipairs(targetlist) do
		if self:isFriend(p) then
			if (p:hasSkill("kongcheng") and p:getHandcardNum() == 1) or (p:getCardCount() >= 2 and self:canLiuli(p, self.enemies)) then
				return p
			elseif not friend and getCardsNum("Jink", p, self.player) >= 1 then
				friend = p
			end
		end
	end
...
sgs.ai_skill_invoke.nosjuece = function(self, data)
	...
	return from and
		((self:isFriend(from) and self:needToLoseHp(from, self.player)) or (not self:isFriend(from) and self:canAttack(from)))
end
...
local function getNosFenchengValue(self, player)
	...
	if self.player:hasSkill("juece|nosjuece") and self:isEnemy(player)
		...
		and not (player:isChained() or not self:isGoodChainTarget(player, "F")) then
		return self:isWeak(player) and 1.5 or 1
	end
	...
	if self:needKongcheng(player) or self:getLeastHandcardNum(player) > num then
		value = value - 0.15
	elseif num == 0 then
		value = value + 0.1
	end
...
nosfencheng_skill.getTurnUseCard = function(self)
	...
	for _, player in sgs.qlist(self.room:getOtherPlayers(self.player)) do
		if self:isFriend(player) then
			value = value - getNosFenchengValue(self, player)
...
sgs.ai_skill_discard.nosfencheng = function(self, discard_num, min_num, optional, include_equip)
	...
	if juece_effect and self:isEnemy(liru) and self.player:getEquips():isEmpty() and self.player:getHandcardNum() == 1 and not self:needKongcheng()
		and not (self.player:isChained() or not self:isGoodChainTarget(self.player, "F", liru)) then
		return {}
	end
	...
	for _, id in ipairs(to_discard) do
		if isCard("Peach", sgs.Sanguosha:getCard(id), self.player) then
			return {}
		elseif 1 == self.player:getHp() and isCard("Analeptic", sgs.Sanguosha:getCard(id), self.player) then
			return {}
		end
	end
	if not juece_effect then
		return to_discard
	else
		...
		if self:needToThrowArmor() and not table.contains(to_discard, arm_id) then
			table.insert(to_discard, arm_id)
		else
			if self.player:getOffensiveHorse() and not table.contains(to_discard, off_id) then
				table.insert(to_discard, off_id)
			elseif self.player:getWeapon() and not table.contains(to_discard, wep_id) then
				table.insert(to_discard, wep_id)
			elseif self.player:getDefensiveHorse() and not table.contains(to_discard, def_id) then
				if self:isWeak() then
					table.insert(to_discard, def_id)
				else
					return {}
				end
			elseif self.player:getArmor() and not table.contains(to_discard, arm_id) then
				if self:isWeak() or (not hasJueqingEffect(liru, self.player, sgs.DamageStruct_Fire) and self.player:hasArmorEffect("vine")) then
					table.insert(to_discard, arm_id)
				else
					return {}
				end
...
function sgs.ai_cardsview.nosrenxin(self, class_name, player)
	...
	if player:getHp() < 2 and getCardsNum("Jink,Analeptic", player, self.player) > 0 then return nil end
...
sgs.ai_skill_invoke.nosjianxiong = function(self, data)
	if self.nosjianxiong then
		self.nosjianxiong = nil
		return true
	end
...
sgs.ai_choicemade_filter.cardChosen.nosfankui = function(self, player, promptlist)
	...
	if damage.from then
		...
		if self:needToThrowArmor(target) and self.room:getCardPlace(id) == sgs.Player_PlaceEquip and card:isKindOf("Armor") then
			intention = -intention
		elseif not self:doDisCard(target, "he") then
			intention = -intention
		elseif self:hasSkills(sgs.lose_equip_skill, target) and not target:getEquips():isEmpty() and
			...
			intention = -intention
		elseif sgs.ai_need_damaged.nosfankui(self, target, player) then
			intention = 0
		elseif self:getOverflow(target) > 2 then
			intention = 0
		end
...
sgs.ai_skill_invoke.nosganglie = function(self, data)
	...
	if self:needToLoseHp(damage.from, self.player, damage.card) then
		if self:isFriend(damage.from) then
			sgs.ai_nosganglie_effect = string.format("%s_%s_%d", self.player:objectName(), damage.from:objectName(),
				sgs.turncount)
			return true
		end
		return false
...
sgs.ai_need_damaged.nosganglie = function(self, attacker, player)
	if not attacker then return end
	if not attacker:hasSkill("nosganglie") and self:needToLoseHp(attacker, player) then
		return self:isFriend(attacker,
			player)
	end
...
sgs.ai_skill_use["@@nostuxi"] = function(self, prompt)
			...
			if #cards == 1 and (cards[1]:hasFlag("visible") or cards[1]:hasFlag(flag)) then
				if cards[1]:isKindOf("TrickCard") or cards[1]:isKindOf("Slash") or cards[1]:isKindOf("EquipCard") then
					if add_player(zhugeliang, 1) == 2 then
						return ("@NosTuxiCard=.->%s+%s"):format(targets[1], targets
							[2])
					end
				end
			end
	...
	if zhijiangwei and self:isFriend(zhijiangwei) and zhijiangwei:getHandcardNum() == 1 and
		...
		if isGood and add_player(zhijiangwei, 1) == 2 then
			return ("@NosTuxiCard=.->%s+%s"):format(targets[1], targets
				[2])
		end
	end
	...
	for _, other in sgs.qlist(others) do
		if self:objectiveLevel(other) >= 0 and not (hasTuntianEffect(other, true)) and add_player(other) == 2 then
			return ("@NosTuxiCard=.->%s+%s"):format(targets[1], targets[2])
		end
	end

	for _, other in sgs.qlist(others) do
		if self:objectiveLevel(other) >= 0 and not (hasTuntianEffect(other, true)) and add_player(other) == 1 and math.random(0, 5) <= 1 and not self:hasSkills("qiaobian") then
...
sgs.ai_skill_askforyiji.nosyiji = function(self, card_ids)
	...
	for _, player in sgs.qlist(self.room:getAlivePlayers()) do
		if player:hasFlag("ShenfenUsing")
		then
			Shenfen_user = player
			break
		end
	end
	for i, c in ipairs(self:poisonCards(cards)) do
		i = c:getEffectiveId()
		if self.player:getEquips():contains(c)
			or self.player:getJudgingArea():contains(c)
		then
			for _, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do
				if self:isFriend(p) then return p, i end
			end
			for _, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do
				if not self:isEnemy(p) then return p, i end
			end
			for _, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do
				return p, i
			end
			...
			for _, p in ipairs(self.enemies) do
				if self:isEnemy(p) then return p, i end
			end
			for _, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do
				if not self:isFriend(p) then return p, i end
			end
		end
	end
	if Shenfen_user
	then
		if self:isFriend(Shenfen_user)
		then
			if Shenfen_user:objectName() ~= self.player:objectName()
			then
				for _, id in ipairs(card_ids) do
	...
	for _, friend in ipairs(self.friends) do
		if not hasManjuanEffect(friend)
			and not self:isLihunTarget(friend)
			and not self:needKongcheng(friend, true)
			and (not Shenfen_user or friend:objectName() == Shenfen_user:objectName() or friend:getHandcardNum() >= 4)
		then
			if friend:objectName() ~= self.player:objectName()
			then
	...
	if #new_friends > 0 then
		...
		if card and target then
			for _, friend in ipairs(new_friends) do
				if target:objectName() == friend:objectName()
				then
	...
	elseif CanKeep then
		...
		local other = {}
		for _, player in sgs.qlist(self.room:getOtherPlayers(self.player)) do
			if not (self:isFriend(player) and self:isLihunTarget(player))
				and (self:isFriend(player) or not player:hasSkill("lihun"))
			then
...
function sgs.ai_skill_suit.nosfanjian(self)
	...
	for _, c in sgs.qlist(tg:getHandcards()) do
		...
		if c:hasFlag(flag) or c:hasFlag("visible") then
			if not suits[c:getSuitString()] then
				suits[c:getSuitString()] = 1
			else
				suits[c:getSuitString()] = suits
					[c:getSuitString()] + 1
			end
...
sgs.ai_view_as.gdlonghun = function(card, player, card_place)
	...
	then
		if card:getSuit() == 2
		then
			return ("peach:gdlonghun[no_suit:0]=" .. card:getEffectiveId())
		elseif card:getSuit() == 0
		then
			return ("nullification:gdlonghun[no_suit:0]=" .. card:getEffectiveId())
		elseif card:getSuit() == 3
		then
			return ("fire_slash:gdlonghun[no_suit:0]=" .. card:getEffectiveId())
		elseif card:getSuit() == 1
		then
			return ("jink:gdlonghun[no_suit:0]=" .. card:getEffectiveId())
		end
	end
end
