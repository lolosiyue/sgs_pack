...
sgs.ai_skill_use_func.NosRendeCard = function(card,use,self)
	...
	for i = 1,self.player:getHandcardNum()do
		...
		if card and friend then cards = self:resetCards(cards,card)
		...
		else use.card = sgs.Card_Parse("@NosRendeCard="..card:getId()) end
		...
		for _,p in sgs.qlist(self.room:getOtherPlayers(self.player))do
			...
			then
				...
				for _,card in ipairs(cards)do
					...
					then table.insert(to_give,card:getId()) end
				...
				then
					...
					if use.to then use.to:append(p) break end
				end
			end
		end
	end
	...
	if not juece_effect then return to_discard
		...
		if self:needToThrowArmor() and not table.contains(to_discard,arm_id) then table.insert(to_discard,arm_id)
			...
			elseif self.player:getArmor() and not table.contains(to_discard,arm_id) then
				if self:isWeak() or (not hasJueqingEffect(liru, self.player, sgs.DamageStruct_Fire) and self.player:hasArmorEffect("vine")) then table.insert(to_discard,arm_id)
				else return {} end
			end
...
sgs.ai_need_damaged.nosganglie = function(self,attacker,player)
	if not attacker then return end
	if not attacker:hasSkill("nosganglie") and self:needToLoseHp(attacker,player) then return self:isFriend(attacker,player) end
...
sgs.ai_skill_askforyiji.nosyiji = function(self,card_ids)
	...
	for _,player in sgs.qlist(self.room:getAlivePlayers())do
		...
		then Shenfen_user = player break end
	end
	...
	then
		...
		then
			...
			then return friend,card:getId() end
		end
	...
	for _,friend in ipairs(self.friends)do
		...
		then
			if friend:objectName()~=self.player:objectName()
			then table.insert(new_friends,friend)
	...
	if #new_friends>0 then
		...
		if card and target then
			for _,friend in ipairs(new_friends)do
				...
				then return friend,card:getEffectiveId() end
			end
		end
		if Shenfen_user and self:isFriend(Shenfen_user)
		then return Shenfen_user,cards[1]:getEffectiveId() end
	...
	elseif CanKeep then
		...
		for _,player in sgs.qlist(self.room:getOtherPlayers(self.player))do
			...
			then table.insert(other,player) end
		end
