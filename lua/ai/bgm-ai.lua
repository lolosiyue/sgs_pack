...
lihun_skill.getTurnUseCard = function(self)
	...
	return sgs.Card_Parse("@LihunCard=" .. cards[1]:getId())
end
...
sgs.ai_skill_use_func.LihunCard = function(card, use, self)
	...
	for _, enemy in sgs.list(self.enemies) do
		if enemy:isMale()
			and not enemy:hasSkill("kongcheng")
		then
	...
	then
		for _, enemy in sgs.list(self.enemies) do
			if enemy:isMale()
				and enemy:getHandcardNum() >= enemy:getHp()
			then
				use.card = card
				if use.to
				then
...
function SmartAI:isLihunTarget(player, drawCardNum)
	...
	then
		for _, ap in sgs.list(player) do
			if self:isLihunTarget(ap, drawCardNum)
			then
				return true
			end
		end
	...
	if sb_diaochan:getPhase() == sgs.Player_Play
	then
		if (handCardNum - player:getHp() >= 2)
			or (handCardNum > 0 and handCardNum - player:getHp() >= -1 and not sb_diaochan:faceUp())
		then
			return true
		end
	else
		if sb_diaochan:faceUp() and not self:willSkipPlayPhase(sb_diaochan)
			and self:playerGetRound(player) > self:playerGetRound(sb_diaochan) and handCardNum >= player:getHp() + 2
		then
			return true
		end
	end
...
sgs.ai_skill_askforag.manjuan = function(self, card_ids)
	...
	for _, card in sgs.list(cards) do
		if card:isKindOf("Peach") and self.player:isWounded() and self:getCardsNum("Peach") < self.player:getLostHp() then return
			card:getEffectiveId() end
	end
	for _, card in sgs.list(cards) do
...
function hasManjuanEffect(player, manjuan_only)
	if player:hasSkill("manjuan")
		and player:getPhase() == sgs.Player_NotActive
	then
		return true
	end
	if not manjuan_only and player:hasSkill("zishu")
		and player:getPhase() == sgs.Player_NotActive
	then
		return true
	end
...
local function will_discard_zhaolie(self, nobasic)
	...
	if self.player:hasSkill("manjuan") then
		if self:isFriend(spliubei) then
			return true
		else
...
sgs.ai_skill_use_func.YanxiaoCard = function(card, use, self)
	...
	for _, friend in sgs.list(self.friends_noself) do
		if (friend:containsTrick("lightning") and self:getFinalRetrial(friend) == 2
...
sgs.ai_skill_cardask["@anxian-discard"] = function(self, data)
	...
	if self:ajustDamage(from, self.player, 1, use.card) and self:canHit(to, from, true) then
		...
		if card:isBlack() and ((self:getUseValue(card) < sgs.ai_use_value.YinlingCard) or inclusive or self:getOverflow() > 0) then
			...
			if card:isKindOf("Armor") then
				if not self.player:getArmor() then
					shouldUse = false
				elseif self.player:hasEquip(card) and not (card:isKindOf("SilverLion") and self.player:isWounded()) then
					shouldUse = false
				end
			end
...
sgs.ai_skill_use_func.YinlingCard = function(card, use, self)
	self:useCardDismantlement(card, use)
end
...
sgs.ai_skill_use["@@junwei"] = function(self, data, method)
	...
	if #pile >= 3 then
		...
		if #tos > 0 then
			...
			if (tos[1]) then
				return "@JunweiCard=" ..
				tostring(pile[1]) .. "+" .. tostring(pile[2]) .. "+" .. tostring(pile[3]) .. "->" .. tos[1]:objectName()
			end
		end
	end
	return "."
...
sgs.ai_skill_choice.xuehen = function(self, choices)
	...
	if self:isEnemy(current) then
		...
		if self:hasSkills("jijiu|tuntian+zaoxian|beige", current) and n >= 2 and current:getCardCount(true) >= 2 then return
			"discard" end
	end
	self:sort(self.enemies, "defenseSlash")
	for _, enemy in sgs.list(self.enemies) do
	...
	if self:isEnemy(current) then
		for _, enemy in sgs.list(self.enemies) do
			...
			if self.player:canSlash(enemy, nil, false) and not self:slashProhibit(nil, enemy) and self:ajustDamage(self.player, enemy, slash) > 1 then
				self.xuehentarget = enemy
				return "slash"
			end
		end
		local armor = current:getArmor()
		if armor and self:evaluateArmor(armor, current) >= 3 and self:doDisCard(current, "e") and n <= 2 then return
			"discard" end
	end
	if self:isFriend(current) then
			...
			if not isCard("Peach", fcard, self.player) and not isCard("ExNihilo", fcard, self.player) then
				...
				for _, scard in sgs.list(cards) do
					if first_card ~= scard and scard:getSuit() == first_card:getSuit()
						...
						second_card = scard
						second_found = true
						for _, tcard in sgs.list(cards) do
...
hantong_skill.getTurnUseCard = function(self)
	if self.player:hasLordSkill("jijiang") then return end
	local can_invoke = false
	for _, friend in sgs.list(self.friends_noself) do
...
function sgs.ai_cardsview_valuable.hantong(self, class_name, player)
	if player:getPile("edict"):length() > 0 and not player:hasSkill("jijiang")
	then
