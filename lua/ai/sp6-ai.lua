	...
	for _, c in sgs.list(n1) do
		...
		for _, h in sgs.list(cards) do
			...
			if table.contains(poisons, c)
				...
				or self:getKeepValue(c) <= self:getKeepValue(h) then
				continue
			end
			table.insert(n2, c:getEffectiveId())
...
sgs.ai_skill_askforag.diting = function(self, ids)
	...
	local to_obtain = {}
	for card_id in ipairs(ids) do
		table.insert(to_obtain, sgs.Sanguosha:getCard(card_id))
	end
	local cs = self:sortByUseValue(to_obtain, self:isEnemy(self.diting_to))

	for card_id in ipairs(cs) do
	...
	then
		...
		for d, c in sgs.list(getCardList(pile)) do
			...
			then
				...
				if d.card and d.to
				then
					self.shilie_n:append(c:getEffectiveId())
				end
			end
		end
