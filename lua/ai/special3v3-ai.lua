...
sgs.ai_skill_playerchosen.vsganglie = function(self,targets)
	...
	for _,enemy in sgs.list(prior_enemies)do
	...
	for _,friend in sgs.list(self.friends_noself)do
		if self:damageIsEffective(friend,sgs.DamageStruct_Normal,friend) and not self:cantbeHurt(friend) and self:needToLoseHp(friend,self.player) then
...
sgs.ai_need_damaged.vsganglie = function(self,attacker,player)
	for _,enemy in sgs.list(self.enemies)do
		if self:isEnemy(enemy,player) and enemy:getHp()+enemy:getHandcardNum()<=3
			and not (enemy:hasSkills(sgs.need_kongcheng) and not hasBuquEffect(enemy) and enemy:getHandcardNum()>1) and self:isGoodTarget(enemy,self.enemies) then
			return true
		end
	end
	return false
...
zhongyi_skill.getTurnUseCard = function(self)
	if self.player:isKongcheng() or sgs.turncount<=1 then return end
	...
	for _,friend in sgs.list(friends)do
		local local_value = 0
		for _,enemy in sgs.list(self.enemies)do
			if friend:canSlash(enemy) and not self:slashProhibit(slash,enemy) and self:slashIsEffective(slash,enemy) and self:isGoodTarget(enemy,self.enemies,slash) then
...
function sgs.ai_cardsview.jiuzhu(self,class_name,player)
	if class_name=="Peach" and player:getHp()>1 and not player:isNude()
	then
