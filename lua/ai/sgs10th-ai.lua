...
ny_10th_lifeng_skill.getTurnUseCard = function(self, inclusive)
    ...
    for _, card in ipairs(cards) do
        ...
        if (not card:isRed()) and (not card:isBlack())
            and self.player:getMark("ny_10th_lifeng_nocolor-Clear") == 0 then
            canuse = true
        end
...
sgs.ai_view_as.ny_10th_lifeng = function(card, player, card_place)
    ...
    if card_place == sgs.Player_PlaceHand then
        ...
        if (not card:isRed()) and (not card:isBlack())
            and player:getMark("ny_10th_lifeng_nocolor-Clear") == 0 then
            canuse = true
        end
        if canuse then
            ...
            for _, c in ipairs(cards) do
                ...
                for _, ctype in ipairs(ctypes) do
                    if s:isKindOf(ctype) and c:isKindOf(ctype) then
                        local card_str = string.format("#ny_tenth_xiaoyin_buff:%s+%s:", s:getEffectiveId(),
                            c:getEffectiveId())
                        return card_str
                    end
                end
...
sgs.ai_skill_choice["ny_10th_miyi"] = function(self, choices, data)
    ...
    for _, p in sgs.qlist(self.room:getAlivePlayers()) do
        ...
        if self:isFriend(p) then
            if p:isWounded() and (p:objectName() == self.player:objectName() or self:isWeak(p)) then
                recover = recover +
                    1
            end
        end
...
sgs.ai_skill_use_func["#ny_10th_tongwei"] = function(card, use, self)
    ...
    if target then
        local card_str = string.format("#ny_10th_tongwei:%s+%s:->%s", cards[1]:getEffectiveId(),
            cards[2]:getEffectiveId(), target:objectName())
        local acard = sgs.Card_Parse(card_str)
        use.card = acard
        if use.to then use.to:append(target) end
    end
